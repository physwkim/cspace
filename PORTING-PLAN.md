...
1. **`moveit_octomap::OcTree` 위에 parry의 도형/질의 트레잇을 직접
   ...
   Sync`와 `QueryDispatcher`(`parry3d-f64-0.30.0/src/query/query_dispatcher.rs`의 모듈
...
| Phase 3 | `distance: f64` 가 분리 분기(오라클 값 > 0)에서 `1e-4` 이내 일치; 관통 분기는 상류 결함 3건이라 비교 불가 | PARTIAL | §260 | 2026-08-06 |
...
`main.rs:3125-3135`의 `oracle_only`/`rust_only` 독코멘트가 바로 그
구분을 위해 존재한다고 적고 있다.
...
`main.rs:4217`·`main.rs:4335`에 있고, 둘 다
...
(`main.rs:3650`). 두 필드 쪽은 여전히 참이다: `stats.rust_degenerate`나
...
고정했다(`tools/moveit-diff/src/main.rs:3687`, 내가 확인했다).
...
- `cost_sources` — upstream `PlanningScene::getCostSources(const RobotState&,
  ...
  비대칭 자체를 검증 대상으로 만든다.
- `path_cost_sources` — trajectory 오버로드(`planning_scene.cpp:2457-2491`). 추가 요청 필드
...
남은 둘은 `tools/moveit-diff/src/main.rs:4217,4138` —
...
- **다른 쌍** (pair-flip) — 이긴 쌍이 서로 다르다. pr2의 self가
  9,821/10,000으로 여기에 지배된다. **"최소값이 근접한 쌍들 사이의
  승자 교체"라고 적었던 이 줄의 이전 판은 값에 대한 가설이었고, §247가
  그 두 번째 값을 재어 반증했다** — fanuc robot 2,302건 전부에서 오라클이
  고른 쌍은 포트의 최소값보다 `2.264e-4`~`2.897e-1` 위에 있고 `1e-4`
  안쪽은 0건이다.
...
> **이 절의 "UNMET"은 이 절이 얻은 것이 아니다.** 아래 세 STEP 중 조건을
...
**이 소절의 진단은 뒤에 오는 §251 절이 대체한다.** 위 결론("상류에 규약이
...
값 발산이 **0**건이고 전부 pair-flip이다. 이 절의 표·불변량 시험은
fanuc의 2,897배를 설명하지 않는다. pr2의 mesh-vs-box 최악값
`3.218e-1`(§21.4, 이 절 이전 판)은 같은 쌍/다른 쌍 구분이 **한 번도
시행되지 않았다** — 근거 없이 이 기전으로 분류해서는 안 된다.
`doc/upstream-bugs.md`의 해당 항목도 이 구분에 맞춰 고쳤다.

**(§247에서 이어짐)** 그 pair-flip을 "근접 동점"이라 읽은 것 역시
측정되지 않은 가설이었고, §247가 두 후보를 한 metric에 놓고 재어
반증했다. fanuc·pr2의 원인은
`distance-callback-threshold-suppresses-deeper-pairs` — 상류가 질의 안에
있던 더 깊은 쌍을 임계값 게이트로 버리고 그보다 얕은 값을 최소값으로
발표하는 것 — 이며, fanuc self와 pr2 self에서는 이 절의 기전과 겹쳐
나타난다. pr2는 "세 번째 원인"이 아니다.
...
**(§260에서 이어짐) 이 절의 "제외 가능한 좌표에 갇혀 있지 않다"는 거짓이다.**
좌표를 상태 공간에서 찾으면 없는 것이 맞지만, 상류 자신의 코드 경로에서 찾으면
있다 — `moveit_core/collision_detection_fcl/src/collision_common.cpp:636`이
`distance` 필드의 의미를 분리 거리에서 접촉 깊이로 바꾸고, 이 절을 포함한 등재
결함 셋이 전부 그 아래에서만 발생한다. 분리 쪽 발산은 5로봇 × 10,000상태에서
0건이다. §5 표의 `distance: f64` 행은 그 측정에 따라 **PARTIAL**로 바뀌었고,
근거 열도 §260을 가리킨다.
...
(`planning_scene.cpp:457-463`, `:502-510`). 이름과 반대로 패딩된 검사다.
`doc/upstream-bugs.md`의
...
## §247 `distance` 절의 fanuc·pr2 원인 — 근접 동점이 아니라 상류가 최소값을 발표하지 않는다 (2026-08-06)

§229.3은 panda의 미충족을 `distance-callback-max-contact-depth`로 닫으면서
fanuc은 "이 기전이 아니다"(§218.4의 pair-flip), pr2는 "구분을 시행한 적이
없다"고 명시적으로 열어두었다. 이 절이 그 둘을 닫는다. 결론은 하나다:
**두 로봇 모두 상류가 자기 질의 안에 있던 더 깊은 쌍을 버리고 그보다 얕은
값을 최소값으로 발표한다.** 근접 동점이 아니었고, 포트가 구현하지 않은
tie-break 규칙도 존재하지 않는다.

### §247.1 "pair-flip = 근접 동점"은 값에 대한 주장이었고, 값은 측정된 적이 없다

`DistancePairStats`가 셀 수 있는 것은 "이긴 쌍이 서로 다르다"뿐이다.
§218.4가 그것을 "최소값이 근접한 서로 다른 쌍 사이의 승자 교체"라고 읽은
것은 **두 후보의 값**에 대한 가설이고, 어느 라운드도 두 번째 값을 재지
않았다. 오라클은 자기 쌍의 값만, 포트는 자기 쌍의 값만 보고하기 때문이다.

빠져 있던 양을 `tools/moveit-diff`에 `--pair-probe-json`으로 넣었다. 실패
사례마다 **오라클이 고른 쌍 하나만 남기고 나머지를 전부 마스크한 ACM**으로
포트 자신의 `distance_robot`/`distance_self`를 다시 부른다. 두 후보가 한
metric 안에 놓이므로 "동점"이 비로소 수치가 된다:

- `candidate_gap = (오라클 쌍에서의 포트 값) − (포트의 최소값)`
  — 오라클이 고른 쌍이 포트의 최소값보다 얼마나 위에 있는가. 동점 가설이
  성립하려면 이 값이 절의 허용오차 `1e-4` 아래여야 한다. 구성상 `>= 0`이다.
- `same_pair_delta = |오라클 값 − 오라클 쌍에서의 포트 값|`
  — **같은 쌍**에서의 값 발산, 즉 `distance-callback-max-contact-depth`.
  쌍 선택과 독립이다.

프로브는 `DistanceRequestType::Global`을 쓴다. `Single`이면 한 번에 전체
맵을 받지만 pr2 self에서 호출당 ~340s(Global은 ~0.11s)이므로 10,000상태
스윕 위에서는 아예 돌지 않는다. fanuc `collision[9651]`에서 `Single` 맵
7쌍과 대조해 검증한 뒤 썼다 — 마스크 프로브 `link_4/floor`
`-2.89703002516375319e-1`, `base_link/floor` `+0.0`, `link_5/floor`
`+1.12707096491874478e-1`이 `Single` 맵의 같은 세 항목과 17자리까지 같고,
`check_robot_collision`의 접촉 깊이(`base_link/floor` `+0.0`, `floor/link_4`
`+2.89703002516375319e-1`)와도 같다.

fanuc 10,000상태(seed 1), pair-flip 3,888건 전수:

| 측면 | 건수 | `candidate_gap` min | p1 | median | max | `< 1e-4` |
|---|---|---|---|---|---|---|
| robot | 2,302 | `2.264205e-4` | `4.563071e-3` | `1.164680e-1` | `2.897030e-1` | **0 / 2,302** |
| self  | 1,586 | `1.924534e-5` | `2.829453e-4` | `1.568689e-2` | `6.191058e-2` | 2 / 1,586 |

robot 쪽은 **2,302건 전부**가 허용오차 밖이고, 가장 가까운 것조차
`2.264e-4` — 절의 `1e-4`의 2.26배다. 동점은 없다. 같은 2,302건에서
오라클이 고른 쌍은 예외 없이 `base_link/floor`이고 그 값은 단일 상수
`-9.930136612989092e-16`, 포트가 그 쌍에서 재는 값은 정확히 `0.0`이므로
`same_pair_delta`도 `9.930137e-16` 하나뿐이다 — **두 구현은 그 쌍에 대해
동의한다.** 갈리는 것은 값이 아니라 *어느 쌍을 발표하는가*다.

### §247.2 상류 자신의 질의가 자기 발표값을 반증한다 (기하는 그대로, ACM만 바뀐다)

`scene_diff_collision` + `set_acm_entry`로 오라클에게 **쌍 하나만 고려에서
빼고** 같은 상태를 다시 물었다. 옮긴 기하도, 더하거나 뺀 물체도 없다.

fanuc `collision[9651]` (§218.4가 지목한 fanuc 최악 상태, seed 1):

| 고려에서 뺀 쌍 | 오라클 `robot_collision` | 오라클 `robot_distance` |
|---|---|---|
| (없음)            | `true`  | `-9.93013661298909247e-16` |
| `base_link/floor` | `true`  | `-1.39489505627222377e0` |
| `+ link_4/floor`  | `false` | `+1.12707096491874714e-1` |

발표값은 `-9.9e-16`인데, 접선 쌍 하나를 빼자 **자기 질의가** `-1.39`를
답한다 — 자기가 최소라고 부른 값보다 `1.39` 아래다. 셋째 행은
`link_4/floor`가 진짜 최소였음을 보인다(둘 다 빼면 접촉이 없다). 그 셋째
행의 `+1.12707096491874714e-1`은 포트의 `Single` 맵이 `floor/link_5`에
대해 내는 `+1.12707096491874478e-1`과 상대오차 `2e-16`으로 같다 — 두
구현은 억압이 없는 쌍에 대해서는 일치한다. 같은 요청의 오라클
`robot_contacts`는 이미 두 쌍을 다 적고 있다(`base_link/floor` 깊이
`4.440892098500626e-16`, `link_4/floor` 깊이 `0.018272381609339305`) —
버려진 쌍은 "닿지 않는 쌍"이 아니라 같은 상태의 충돌 질의가 보고하는
쌍이다.

pr2 self, 실패 8상태. 여기서는 **포트가 고른 쌍만 남기고 나머지 1,430개
링크 쌍을 전부 마스크**해 오라클에게 물었다 — 포트에 돌리던 그 프로브를
오라클에 돌린 것이다:

| case | 오라클 발표값 | 그 쌍만 물었을 때의 오라클 값 | 쌍 |
|---|---|---|---|
...
8건 전부에서 발표값이 자기 자신이 그 쌍에 대해 내는 값보다 `7.79`~`21.18`배
얕다.

억압하는 주체가 *이긴* 쌍이 아니라 *처음 방문된 관통 쌍*이라는 것도
측정했다. 오라클이 **고른** 쌍만 빼는 실험은 위 8건 중 3건만 값을 더
깊게 만들고(`4.5e-3` / `2.1e-2` / `3.1e-3`), 3건은 전혀 움직이지 않으며
(`<= 5.2e-17`; pr2 그리퍼는 좌우가 mimic으로 대칭 관통하므로 같은 깊이의
쌍이 하나 더 있다), 1건은 오히려 얕아진다(`-1.1e-3`). 그래서 이긴 쌍만
빼서는 억눌린 쌍이 드러나지 않고, 위 표가 "포트의 쌍만 남기는" 형태여야
한다.

### §247.3 기전 — `:608`의 두 항이 서로 다른 양이다

`distanceCallback`(`moveit_core/collision_detection_fcl/src/collision_common.cpp`
@ `e017c91ee`):
...
- `:636`~`:663` — 통과한 경우에만 `fcl::collide`(`num_max_contacts = 200`)를
  돌려 `dist_result.distance = -contact.penetration_depth`.
- `:694` — `if (dist_result.distance < cdata->res->minimum_distance.distance)`로
  누적. 즉 `:574`가 다음 쌍에 넘겨주는 임계값은 **관통 깊이**다.

`:608`의 왼쪽은 `fcl::distance`의 원값(분리 거리)이고 오른쪽은 관통
깊이다. 분리 거리는 기록된 관통 깊이 아래로 내려오지 않으므로, 어떤
쌍이든 한 번 누적 최소값을 음수로 만들면 그 뒤의 모든 관통 쌍이 `:608`에서
탈락하고 자기 깊이는 계산조차 되지 않는다. 살아남는 것은 가장 깊은 쌍이
아니라 브로드페이즈가 **처음 방문한** 관통 쌍이다. fanuc에서 그 쌍이 항상
`base_link/floor`인 이유는 §218.3과 같다 — `base_link`는 월드에 고정되어
있고 바닥 상판이 정확히 `z = 0`이라 10,000상태 전부에서 접선이다.

포트에서 대응되는 자리는 `crates/moveit-collision/src/parry.rs`의
`accumulate_distance`다. `:2242`가 같은 `Global` 임계값을 읽고 `:2268`이
같은 폐기를 하지만, 비교되는 `contact.dist`는 `:2294`가
`minimum_distance`에 넣는 바로 그 부호 있는 양이다 — 양쪽이 한 metric이므로
더 깊은 쌍이 게이트에서 탈락할 수 없다. `bounded_prediction`(`:1258`)이
임계값을 `[0, ...]`로 clamp하므로 음수 임계값에서도 관통 접촉은 그대로
돌아온다.

**포트가 구현하지 않은 tie-break 규칙은 존재하지 않는다.** 상류가 하는
비교는 `:694`의 평범한 strict `<` 하나뿐이고(동점이면 먼저 온 쪽이 남는다),
포트도 `:2294`에서 같은 strict `<`를 쓴다. 승자를 가르는 것은 `:608`의
게이트이지 동점 처리 규칙이 아니다.

이 항목을 `doc/upstream-bugs.md`에
`distance-callback-threshold-suppresses-deeper-pairs`로 등재했다
(not-reproduced). 포트 쪽 불변량은 오라클·docker 없이 재실행 가능한 형태로
`crates/moveit-collision/tests/minimum_distance_is_the_minimum.rs`에 고정했다
(3건, 빌드 후 3회 반복 실측 `0.037`~`0.043s`; `570cf96`의 커밋 메시지에 적힌
`0.031s`는 이 재측정 범위 아래이므로 이 줄의 숫자를 쓸 것).

### §247.4 pr2도 같은 기전이다 — 세 번째 원인이 아니다

pr2 10,000상태(seed 1), pair-flip 전수를 §247.1의 두 양으로 분류:

| 측면 | pair 불일치 | 그중 `1e-4` 초과 | 억압만 | 둘 다 | 값 발산만 |
|---|---|---|---|---|---|
| robot | 8,831 | 28 | 21 | 6 | 1 |
| self  | 9,821 | 9,820 | 515 | 9,141 | 164 |

self의 `candidate_gap`은 min `3.053113e-16`, p1 `3.485406e-14`, median
`4.659153e-2`, max `5.292844e-2`. 즉 self 실패 9,820건 중 **9,656건
(98.3%)** 이 "상류가 최소가 아닌 값을 발표"를 안고 있고, 그중 9,141건은
같은 쌍 값 발산(`distance-callback-max-contact-depth`)까지 함께 안고 있다.
pr2는 fanuc과 같은 기전이며, panda의 기전이 그 위에 겹친 것이다.

§21.4가 적어둔 pr2 mesh-vs-box 최악값 `3.218e-1`이 이 스윕에서 그대로
재현됐고(`worst_distance_deviation` `0.3217869257926942`), 이제 분류된다 —
`collision[1584]` self: 오라클 `base_link/torso_lift_link` `-3.747158e-1`,
포트 `base_bellow_link/torso_lift_link` `-5.292891e-2`, 포트가 오라클의
쌍에서 재는 값 `-1.761972e-2`. 같은 쌍 값 발산이 `3.571e-1`, 쌍 선택
격차가 `3.531e-2` — **둘 다**이고, 이 한 건에서는 값 발산 쪽이 지배적이다.

pr2의 robot 쪽은 반대로 **진짜 근접 동점 모집단**이다. 네 캐스터 바퀴가
바닥 위에서 대칭이라 쌍은 갈리지만 값이 같다: 쌍 불일치 8,831건 중 절을
깨는 것은 28건뿐이고 8,803건은 허용오차 안이다. "쌍이 갈리는 것"과 "절이
깨지는 것"이 같은 사건이 아니라는 증거가 같은 로봇 안에 있다 — §218.4의
pair-flip 계수만으로는 원인을 지목할 수 없다는 뜻이기도 하다.

### §247.5 판정 — 두 절 다 이 픽스처 위에서 닫히지 않고, 재측정이 그것을 바꾸지 않는다

`distance: f64` 절(§5 Phase 3)의 로봇별 원인이 이제 전부 이름을 가진다:

| 로봇 | 원인 | 근거 |
|---|---|---|
| panda | `distance-callback-max-contact-depth` | §229.3 |
| fanuc | `distance-callback-threshold-suppresses-deeper-pairs` (robot 2,302건 단독, self 1,586건 중 1,542건은 겹침) | §247.1~3 |
| pr2 | 같음 (self 9,820건 중 9,656건, 그중 9,141건은 겹침) | §247.4 |
| prbt | §218.3/§229.1의 센티널 | §229.1 |

재측정이 이 판정을 바꾸지 않는 이유는 셋 다 측정이기 때문이다.

1. **발산의 크기가 상수가 아니다.** 그것은 억눌린 쌍의 깊이이고 상태마다
   다르다 — fanuc `collision[9651]`에서 `2.897e-1`, pr2 `collision[1584]`
   에서 `3.218e-1`, pr2 `collision[153]`에서 `3.77e-2`. 그러므로 이것을
   받아들이면서 더 작은 무엇인가를 탐지하는 고정 허용오차는 존재하지
   않는다. 알려진 상수는 오프셋이지 허용오차가 아니며, §11.10이 미리
   적어둔 이유가 그대로 적용된다.
2. **오라클 쪽 반증이 시드·표본에 걸려 있지 않다.** §247.2의 실험이 바꾼
   것은 상태가 아니라 ACM 한 항목, 즉 고려 집합이다. 같은 상태에서 상류가
   자기 발표값보다 깊은 값을 자기 입으로 내놓는다.
3. **포트 쪽은 불변량이지 수치가 아니다.**
   `minimum_distance_is_the_minimum.rs`의
   `masking_the_tangent_pair_does_not_move_the_answer`가 §247.2 표의 첫 두
   행을 포트에 그대로 돌리고, 포트에서는 답이 움직이지 않는다. 상류 수치와
   일치하려면 "질의가 고려한 쌍들의 최소값을 발표한다"는 성질 자체를
   버려야 하고, 그것이 절의 이름이 주장하는 바로 그 성질이다.

따라서 `distance: f64` 행은 **미충족**으로 남으며, 그 사유는 "포트의 오차"가
아니라 "상류가 다른 값을 발표한다"이다. `collision: bool` 행은 §229.1의
판정 그대로다 — 정확 접촉에서 맞출 상류의 단일 규약이 없다. 이 절은 `bool`
행을 건드리지 않는다.
...
- §5 완료 조건 현황표의 `distance: f64` 행 근거를 `§229.3`에서 이 절로
  ...
  아직 번호가 없는 절을 그 자리에 쓰면 그 게이트가 "no such heading"으로
  빨개진다 — 미배정 자리표시자를 의도적으로 잡아주는
...
- `doc/assertion-discrimination-ledger-p1-fixtures.md`의 `main.rs` 인용
  ...
  커밋이 건드려 `3890` → `4051`행이 되었고, main 쪽 원장은 이미 다른
  번호로 재도출되어 있다(`78`/`2826`/`2856`/`2873`/… 15개). 병합하면 이
  문서는 충돌하며, `+247`을 그대로 받으면 안 된다. **병합 후 합쳐진
  `main.rs`에 대해 다시 도출해야 하고**, 방법은 줄 번호를 옮기는 것이
  아니라 인용된 줄의 *내용*을 앵커로 삼는 것이다 — `difflib.SequenceMatcher`
...
§219가 Phase 7의 세 조건을 명령으로 만들었다. 이 절은 그 명령이 **통과하면서도
포트가 틀려 있을 수 있는 경로**를 조건별로 세고, 닫은 것은 게이트 안에서
닫는다. Phase 8 둘째 조건("CHOMP/STOMP가 Phase 7과 같은 속성 기반 검증을
통과")이 이 게이트의 모양을 물려받으므로, 여기서 못 보는 것은 그쪽도 못 본다.

닫은 변경은 하나씩 **판별 뮤테이션**으로 증명했다 — 그 검사가 잡는다고 주장하는
것을 실제로 깨뜨려 실패를 보이고 되돌린다(§248.8). 판별하지 못하는 검사는
검사가 아니다.

### §248.1 세 조건이 전부 참인데 포트가 틀릴 수 있는 경로

| # | 조건 | 조건이 참인데 포트가 틀릴 수 있는 방식 | 판정 |
|---|---|---|---|
| 1 | 1 | fanuc이 붕괴해도 게이트는 panda만 보고 통과 — fanuc 판정은 **인쇄만** 되고 `failed`에 들어가지 않았다 | 닫음 §248.6 |
| 2 | 1 | `floor_wall`이 무너지고 `cage`가 버티면 stratum pooled가 상쇄한다 | 닫음 §248.6 (set별 게이트) |
| 3 | 1 | C++ 기준선이 퇴행하면 문턱 자체(`0.9 × cpp_rate`)가 내려간다 | 닫음 §248.7 (`cpp_solved_floor`) |
| 4 | 1 | RNG 스트림 하나에 대한 비교다 (§221은 스트림 차이로 IK 성공이 30건 움직였다) | 측정 후 닫지 않음 §248.2 |
| 5 | 2 | 포트 자신의 체커가 판정한다 — 플래너와 같은 방향으로 틀린 체커는 100%를 보고한다 | 닫음 §248.3 |
| 6 | 2 | 500개 중 1개만 검사해도 `condition2_checked > 0`은 만족된다 | 닫음 §248.6 (`condition2-covers-every-solved-path`) |
| 7 | 2 | densify가 플래너 정점으로 되돌아가면 탐색이 이미 승인한 상태만 다시 읽는다 | 닫음 §248.6 (`condition2-densified`) |
| 8 | 2 | 아무 문제도 못 푼 injection 실행이 체커를 보증한다 | 닫음 §248.4 |
| 9 | 3 | 각 진영이 **자기** 해결 집합에 대한 median을 쓴다 — 어려운 문제를 놓치면 통과가 쉬워진다 | 닫음 §248.6 (paired median) |
| 10 | 3 | pooled median이 robot별 퇴행을 숨긴다 | 닫음 §248.5 |
| 11 | 3 | 조건 3은 단측이다 — median이 붕괴하면(경로가 우회를 그만두면) 더 잘 통과한다 | 닫음 §248.7 (`ratio_floor`) |
| 12 | 1·2·3 | `solved`는 경로가 돌아왔다는 말이지 **요청한 두 상태 사이의** 경로라는 말이 아니다 | 닫음 §248.6 (`endpoints`) |
...
§221은 restart-RNG 스트림 차이가 IK 성공을 30건 움직였다고 기록했다. 같은 크기의
움직임이 여기서도 있으면 게이트가 재는 것은 포트가 아니라 스트림이다. 그래서 쟀다.

문제 집합은 고정이다(요청 파일 동일). 포트 쪽은 자기 RNG의 `seed_base`만,
C++ 쪽은 요청의 OMPL `seed`만 바꿨다. panda 500문제(`floor_wall` 250 +
`cage` 250):

| 바꾼 것 | 값 | solved / exact | pooled median |
|---|---|---|---|
| 포트 `seed_base` | 424242 (핀) | 497 | 2.668003737362192 |
| | 1 | 498 | 2.7153680126656896 |
| | 20260806 | 497 | 2.6694923028806663 |
| | 999983 | 497 | 2.6576020990273617 |
| C++ 요청 `seed` | 900001+900002 (핀) | 498 (250+248) | 2.6597767032746464 |
| | 1 | 497 (250+247) | 2.6683930312879918 |
| | 20260806 | 498 (249+249) | 2.654415371365775 |
| | 999983 | 496 (249+247) | 2.6680746022802833 |

fanuc 500문제(성공률이 81%로 문턱에 훨씬 가까운 집합이라 스트림에 더 민감할
자리다):

| 바꾼 것 | 값 | solved / exact | pooled median |
|---|---|---|---|
| 포트 `seed_base` | 424242 (핀) | 405 | 1.8928516465801457 |
| | 20260806 | 405 | 1.8604968009756002 |
| | 999983 | 405 | 1.8310179645102211 |
| C++ 요청 `seed` | 900021+900022 (핀) | 406 (206+200) | 1.873377471827347 |
| | 1 | 405 (205+200) | 1.8369360502021963 |
| | 20260806 | 405 (205+200) | 1.8579512498919681 |
| | 999983 | 405 (205+200) | 1.8392462872821487 |

**§221의 30건은 여기서 재현되지 않는다.** 움직임은 panda 포트 1건 / C++ 2건,
fanuc 포트 0건 / C++ 1건이고, median 비율의 전 범위는 panda
0.99596–1.02296, fanuc 0.97739–1.03044이다. 조건 1의 마진은 실측값에서 49문제
(panda: 497 vs 문턱 448.2), 조건 3의 마진은 약 27%다. 두 자릿수 작은 섭동이다.

이유는 두 측정의 성질이 다르다는 데 있다. §221에서 스트림이 움직인 것은
**재시작 위치**가 성공을 좌우하는 IK였다. 여기서 RRTConnect가 이 문제들에
대해 갖는 성공/실패는 한계적이지 않다 — panda 99.4%는 거의 모든 문제가 어떤
스트림에서도 풀린다는 뜻이고, 성공률이 문턱에 가까운 fanuc(81%)에서조차 세
스트림이 405/405/405로 같았다.

그래서 게이트에 둘째 스트림을 넣지 않았다. 비용은 두 배인데 방어 대상이
마진의 1/50이다. **대신 같은 여유를 핀으로 닫았다**(§248.7): 관측 최소값보다
5문제 아래의 바닥은 스트림 잡음(1–2문제)을 통과시키고 6문제 이상의 퇴행을
잡는다. 스트림을 늘리는 것과 달리 비용이 0이다.

부수적으로, 양쪽 다 **바이트 재현**된다: 핀된 seed 쌍으로 C++는 exact 498
(250/248)과 pooled median `2.6597767032746464`를, 포트는 `seed_base` 424242로
497과 `2.668003737362192`를 재현했다. 재현되지 않는 값에는 핀을 걸 수 없다.

### §248.3 조건 2는 포트 자신의 체커가 판정했다

`is_path_valid`(보고), `DiscreteMotionValidator`(탐색 중 사용),
`build_injected_state`의 탐색은 **같은 `ParryCollisionEnv`로 가는 세 입구**다.
injection 게이트는 "체커가 나쁜 waypoint를 거부한다"를 증명하지만, 거부하는
주체가 플래너와 같은 백엔드다. 백엔드가 한 방향으로 관대하면 세 입구가 모두
같이 관대하고, 조건 2는 100%를 보고한다.

닫는 방법은 논증이 아니라 두 번째 구현이다. 오라클의 `is_state_valid` op은
상류 MoveIt 자신의 `PlanningScene::isPathValid`(FCL 위)를 부른다. 그래서 포트가
조건 2로 센 **모든** 경로의 densify된 waypoint 목록을 그 op에 그대로 넘기고,
`valid`와 **invalid-index 집합까지** 일치할 것을 요구한다. 개수가 아니라 인덱스인
이유: 서로 다른 waypoint를 같은 **개수**만큼 거부한 두 체커는 같은 답을 낸 것이
아니다.

경로를 한 컨테이너 호출에 묶어도 되는 근거는 상류 루프가 waypoint 단위라는 것이다
(`moveit_core/planning_scene/src/planning_scene.cpp:2365-2424`, 고정 커밋
`e017c91e`) — 유일한 waypoint 간 요소는 마지막 waypoint의 goal 검사이고
`goal_constraints`가 비면 발동하지 않는다.

이 교차검증이 실패할 수 있다는 것은 injection 실행으로 증명된다: 주입된 경로에서
양쪽이 `valid:false`를 내고 invalid-index 배열이 동일했다(측정: 7/192/321이 양쪽
동일). 정상 경로 3개(975 waypoint, 1.44s)에서는 양쪽이 `valid:true`였다.

### §248.4 체커를 한 번도 부르지 않은 injection 실행이 체커를 보증했다

`assert_eq!(condition2_pass, 0)`은 **아무 문제도 못 푼 실행이 보고하는 값과
같다** — 그 실행은 `is_path_valid`를 부르지 않았으므로 비교 대상 개수도 0이다.
측정: 플래너 데드라인을 1ns로 깎은 injection 실행이

```
robot=panda problems=4 solved=0 timeouts=4 failures=0 cond2=0/0 ...
inject=collision rejected all 0 paths, as required
```

를 찍고 exit 0으로 끝났다. 조건 2의 체커가 실패할 수 있음을 증명하는 **바로 그
검사**가, 체커를 부르지 않은 채 그것을 보증한 것이다. `condition2_checked > 0`을
같은 자리에 함께 단언해 닫았다. 같은 뮤테이션에서 이제 exit 101이다(§248.8 M5).

### §248.5 pooled median은 robot별 퇴행을 숨긴다 — fanuc을 따로 둔 판단과 그 규칙

**따로 둔 것은 옳았다. 다만 절반만 옳았다.** fanuc은 panda 숫자에 섞이지 않았지만
(옳음) `failed`에도 들어가지 않았다(틀림, §248.6에서 닫음).

섞지 않는 것이 왜 옳은지는 수치로 말할 수 있다. 실측값(panda C++ 498 / 포트 497,
fanuc C++ 406 / 포트 405):

| 게이트 단위 | 조건 1 문턱 | 포트 실측 | 여유 |
|---|---|---|---|
| panda 500 (§5가 명명한 집단) | 448.2 | 497 | 49문제 |
| fanuc 500 (자기 이름으로) | 365.4 | 405 | 40문제 |
| 1000문제 pooled | 813.6 | 902 | 88문제 |

pooled에서는 panda의 잉여가 fanuc의 결손을 대신 낸다: panda가 497로 유지되면
fanuc은 **317까지 떨어져도** pooled 게이트를 통과한다 — 88문제 손실 허용이다.
fanuc을 자기 이름으로 재면 366에서 실패한다 — 40문제. **평균을 내는 것만으로
허용 퇴행이 2.2배가 된다.**

그래서 규칙은 이렇게 쓴다. **stratum은 집단이다. §5가 명명한 집단은 하나
(panda의 500)이고 그것이 완료 판정을 낸다. 여기서 측정되는 다른 모든 집단도
집단이므로 각각 자기 이름으로 게이트되고, 어떤 집단도 다른 집단에 평균되지
않는다.** 이 규칙은 robot 사이에만 적용되는 것이 아니다 — `floor_wall`과
`cage`를 pooled하는 것도 두 집단을 평균하는 것이므로 set도 자기 이름으로
게이트된다. 뮤테이션이 그 필요를 직접 보였다: fanuc만 퇴행시켰을 때
`fanuc_floor_wall/condition1`은 실패하고 `fanuc_cage/condition1`은
통과했다(4/8 vs 6/8) — set을 합치면 상쇄되는 크기의 퇴행이다.

### §248.6 인쇄된 판정과 종료 코드가 두 계산이었다

`verify-phase7-benchmark.sh`는 세 조건을 **두 번** 계산했다 — 한 번은 보고용
`jq -r` 안에서, 한 번은 그 아래 `c1=$(jq ...)`에서 — 그리고 자기 주석이 그것을
인정했다: "Must match the verdict computed below exactly". 맞을 **의무**가 없었다.
fanuc의 세 조건은 `jq -r` 문자열 안에서 PASS/FAIL로 인쇄되고 `failed`에 닿는
경로가 없었다. 결과: 둘째 robot의 퇴행이 FAIL로 인쇄되고 스크립트는 exit 0.

측정(§248.8 M1, fanuc만 퇴행시킨 뮤테이션, 라운드 전 스크립트):

```
    C++ 12/16 = 75%, port 10/16 = 62.5% (timeouts 0, other failures 6)
    condition 1: FAIL   condition 2: 10/10 PASS   condition 3: ... PASS
OK conditions 1/2/3 pass (mode=pilot)
PRE-ROUND GATE exit=0
```

구조적으로 닫았다: 인쇄와 종료 코드가 **같은 목록 하나**에서 나온다. jq가
`[{name, ok, detail}]`를 만들고 bash가 그 목록을 한 번 돌면서 각 줄을 인쇄하고
`ok=false`를 전부 `failed`에 넣는다. 동기화해야 할 사본이 없으니 어긋날 수 없다.

같은 목록에 §248.1의 나머지를 검사로 넣었다. 조건 2가 참인데 포트가 틀릴 수
있는 방식들:

- `condition2-covers-every-solved-path` — `condition2_checked == port_solved`.
  `condition2_checked > 0`은 500개 중 1개로도 만족된다.
- `condition2-densified` — `waypoints_checked > raw_waypoints`. densify가 정점으로
  붕괴하면 조건 2는 탐색이 이미 승인한 상태만 다시 읽으면서 100%를 보고한다.
- `endpoints` — 해결된 모든 경로에서 요청한 start/goal까지의 최대 거리가 **정확히
  0**. 허용오차가 아니라 0인 이유: `rrt_connect`는 건네받은 끝점 상태를 그대로
  돌려준다.
- `no-timeouts` — 타임아웃이 하나라도 있는 실행은 재현 불가다(같은 트리가 느린
  기계에서 다른 답을 낸다). 조건 1의 여유는 몇 개를 흡수해 버린다.
- `condition3-paired` — 같은 1.3x를 **양쪽이 모두 푼 문제들**에 대해. 조건 3을
  글자대로 읽으면 각 진영이 자기 해결 집합의 median을 쓰는데, 어려운 문제를 놓친
  포트는 그 긴 경로들을 자기 median에서 떨어뜨려 **나빠질수록 쉽게** 통과한다.
- `condition1`/`condition3`을 stratum(panda pooled, fanuc pooled)과 set 단위로
  각각(§248.5의 규칙).
- 빈 목록은 통과가 아니다: 검사 개수가 0이면 그 자체가 실패다.

### §248.7 회귀 핀 — §5의 문턱은 큰 퇴행을 통과시킨다

§5의 세 문턱은 완료 조건이지 회귀 검출기가 아니다. 실측값에서:

- 조건 1은 panda 포트가 `0.9 × 498/500`, 즉 500문제 중 449를 풀 것을 요구한다.
  포트는 497을 푼다. **48문제를 잃고도 통과한다.**
- 조건 3은 C++ median의 1.3배를 허용하고 실측 비율은 1.023이다. **경로가 27%
  길어져도 통과한다.**

그래서 실측값 아래에 핀을 깔았다. 값은 고르지 않고 §248.2의 스트림 간 폭에서
크기를 얻었다 — 잡음보다 촘촘한 핀은 무고한 변경에 실패하고, 퇴행보다 느슨한
핀은 아무것도 못 본다.

| 핀 | full/panda | full/fanuc | 근거 |
|---|---|---|---|
| `problems` | 500 | 500 | n=500에서 잰 값이 다른 크기의 실행과 비교되는 일을 불가능하게 만든다 |
| `port_solved_floor` | 492 | 400 | 관측 최소(497, 405)보다 5문제 아래 = 관측 최대 스트림 차이(1문제)의 5배 |
| `cpp_solved_floor` | 491 | 400 | 조건 1의 문턱이 **C++ 비율 자체**다 — 퇴행한 기준선은 실패하는 대신 문턱을 낮춘다 |
| `ratio_ceiling` | 1.10 | 1.10 | 관측 전 범위(≤1.031) 위, §5의 1.30보다 3배 촘촘 |
| `ratio_floor` | 0.90 | 0.90 | 조건 3은 단측이다 — median이 붕괴하면 더 잘 통과한다. 관측 최저 0.977의 약 4배 여유 |

pilot 모드는 별도 값이고 full의 값을 축소한 것이 아니다: 8문제/config에서 500
기준의 바닥은 아무 말도 하지 않는다. `pin-population`이 그 혼동을 구조적으로
막는다 — 핀은 자기가 측정된 `problems`를 들고 다니고, 실행 크기가 다르면 통과가
아니라 실패다. 그래서 `PILOT_COUNT` 환경변수 재정의를 허용해도 커버리지를
줄여 통과할 수 없다(§248.8이 그 재정의로 실행된다).

### §248.8 판별 뮤테이션

각 변경이 잡는다고 주장하는 것을 깨뜨려 실패를 확인하고 되돌렸다. 대조군은
`PILOT_COUNT=2`(2m07s) 무뮤테이션 실행이고, 그 자체가 `pin-population`과 두
`solved` 바닥의 판별 증거다 — 축소된 실행은 통과하지 않는다(실패 6개, 정확히 그
셋 × 두 stratum).

| 뮤테이션 | 무엇을 깨뜨렸나 | 실패한 검사 | **그대로 통과한 것** |
|---|---|---|---|
| M2: 홀수 id에서 `condition2_valid: null` | 검사되지 않은 해결 경로 | `*/condition2-covers-every-solved-path` (3 stratum) + 교차검증 10개 | `*/condition2` = 2/2 paths valid |
| M3: `densify(..., f64::INFINITY)` | densify가 정점으로 붕괴 | `*/condition2-densified` "273 checked from 273 returned" (3 stratum) | `condition2` 4/4, 교차검증 전부 — 오라클도 그 정점들이 valid라는 데 동의한다 |
| M4: `path.remove(0)` | 요청한 start에 고정되지 않은 경로 | `*/endpoints` (0.05 = 정확히 한 step) | 조건 1·2·3 **전부** — 조건 3 비율은 0.998→0.983으로 **좋아졌다** |
| M1: fanuc만 `max_iterations` 2000→20 | 둘째 robot만의 퇴행 | `fanuc/condition1`, `fanuc/no-regression-port-solved`, `fanuc_floor_wall/condition1`, exit 1 | panda 16/16·0.997x 무영향, `fanuc_cage/condition1` 6/8 통과 |
| M1 (같은 뮤테이션, 라운드 전 스크립트) | — | 없음: `condition 1: FAIL`을 인쇄하고 **exit 0** | `OK conditions 1/2/3 pass` |
| M5: injection 실행 데드라인 1ns | 아무것도 못 푼 injection 실행 | 단언 발동, exit 101 | 고치기 전: "rejected all 0 paths, as required", exit 0 |

M3와 M4는 이 게이트에서 서로 다른 것이 필요한 이유를 보여준다. M3는 오라클
교차검증이 **못 잡는다** — 상류 체커도 그 정점들이 valid라는 데 동의하기 때문이다.
교차검증은 틀린 체커를 잡고, `condition2-densified`는 너무 적게 검사하는 체커를
잡는다. M4는 세 조건을 전부 통과하면서 조건 3의 비율을 **개선**한다.
...
`full` 실행 한 번(2026-08-06, commit `c296c655`, `working_tree_dirty: false`,
전체 37개 검사 통과, `verdict_all_pass: true`). 원자료는
`doc/phase7-benchmark-results.json`이고 그 안의 `checks` 배열이 종료 코드를
낸 목록 그 자체다.

| | panda (§5가 명명한 500) | fanuc (500) | constrained (250) |
|---|---|---|---|
| C++ exact | 498 (250 + 248) | 406 (206 + 200) | — (오라클 `plan`에 제약 입력이 없다) |
| 포트 solved | 497 (249 + 248) | 405 (205 + 200) | 250 |
| 조건 1 문턱 / 실측 | 89.64% / 99.4% | 73.08% / 81% | — |
| 조건 2 | 497/497 valid, waypoint 168340 (반환 정점 31497) | 405/405, 52983 (10285) | 250/250, 67820 (12733) |
| 상류 `isPathValid` 교차검증 | 497 경로 전부 일치 | 405 전부 일치 | 250 전부 일치 |
| 끝점 최대 이격 | 0 | 0 | 0 |
| 타임아웃 | 0 (최장 16.76s) | 0 (최장 57.46s, `cage` 117) | 0 |
| 조건 3 pooled | 2.668003737362192 / 한계 3.4577097142570405 → 1.003x | 1.8928516465801457 / 2.435390713375551 → 1.01x | — |
| 조건 3 paired | 496문제 공통 → 1.005x | 405문제 공통 → 1.011x | — |
| feasibility | 500 witnessed / 0 unknown | 406 witnessed / 94 unknown | — |

교차검증은 합계 **1152개 경로**(249+248+205+200+250)를 상류
`PlanningScene::isPathValid`에 통과시켰고 valid 판정과 invalid-index 집합이
전부 일치했다. 표본이 아니다 — 조건 2의 문장은 "100% of produced paths"다.

비용. **`full` 79m17s**(4757s, 이 기계 `nproc` 96, `SHARDS=16`). 이 라운드
전에 기록된 ~40분에서 늘었고 원인은 둘이다: 1152개 경로의 densify된 waypoint를
오라클에 넘기는 교차검증, 그리고 양쪽이 못 푼 94문제를 50000 iteration / 300s로
올려 다시 돌리는 feasibility escalation(전체의 절반 이상). 포트 실행 자체의
병렬 wall clock은 845s다. 그래서 `full`은 여전히 opt-in이고 `verify-all.sh`가
글롭으로 도는 것은 pilot이다.

pilot 비용도 늘었다: 라운드 전 **274s** → 현재 **312s**(+14%, 같은 기계, 둘 다
`PILOT_COUNT=8`). 교차검증이 pilot에서도 모든 set에 대해 돌기 때문이고, 그
...
### §251.1 상류: 특수화가 등록된 쌍만 접선에서 충돌한다 (49셀 중 49셀)
...
### §251.4 겹치는 48셀 중 6건이 어긋난다, 그리고 `collision: bool` 행의 판정
...
### §251.5 이 열거가 찾은 포트 결함 — 첨부물 메쉬가 무조건 패닉했다
...
## §258 미포팅 87건 전건 분류 — 브리프의 107은 재현되지 않고, 87건 중 §5의 미충족 행을 막는 것은 0건, 결정이 없는 것은 2건 (2026-08-06)
...
대상 행은 §5 표에서 **읽는다**. 이 자리에는 처음 표의 줄번호
`806`/`807`/`819`를 적었고 그 다음 판에는 "UNMET은 셋"이라고 개수를
적었는데, **둘 다 사본이라 낡는다** — 줄번호는 `PORTING-PLAN.md`가 자랄
때, 개수는 판정어가 바뀔 때. 실제로 §260이 `distance: f64` 행을 UNMET에서
PARTIAL로 바꾸면서 "셋"은 거짓이 됐다. 그래서 이 절도 계기도 개수를
들지 않는다. 대상은 `rg -n '^\| Phase ' PORTING-PLAN.md | rg -v '\| MET \|'`가
내는 행 전부이고, `tools/ci/classify-unported.py`의
`check_phase_coverage()`가 그 행 집합과 계기의 `UNMET_BLOCKERS`가 어긋나면
실패한다(행이 새로 열려도, 닫혀도, 인용 절이 바뀌어도 발화한다 — 변이
셋으로 확인).
...
- **Phase 8** (`UNMEASURED`, §217.3) — 접두사는 `moveit_planners/{chomp,stomp}/`.
  87건 중 8건이 후보다. 기각 근거: 8건 전부 ROS 플러그인 배선이고
  (`chomp_interface/*`, `stomp_moveit_planner_plugin.cpp`,
  `trajectory_visualization.hpp`), Phase 7 하네스는 플러그인을 거치지
  않는다. 이 행이 막히는 이유는 포트 쪽에 속성 기반 하네스가 없다는
  것이며 — `crates/moveit-planners-{chomp,stomp}`에 `examples/`도
  `benches/`도 없다(넷 다 부재를 확인했다) — 상류 파일이 빠져서가 아니다.

즉 **87건 중 어느 것도 §5의 미충족 행을 막지 않는다.** 후보로 발화하는
것은 17건(Phase 3 `collision: bool` 9건, Phase 8 8건)이고 전부 위에서
개별로 기각된다. 나머지 70건은 어느 행의 기전 경로에도 없다.
...
## §260 `distance: f64` 절 — 발산은 상류 함수의 한쪽 분기에 갇혀 있고, 조건 문구가 한 필드로 두 양을 재고 있었다 (2026-08-06)

§229.3은 이 발산을 "제외 가능한 좌표에 갇혀 있지 않다(pervasive and
state-dependent)"로 적었고, 그 문장이 지난 라운드에 `distance` 행만 미충족으로
남긴 근거였다. 그 문장이 이 절의 첫 검사였고 **거짓이다** — 좌표를 상태 공간에서
찾으면 없는 것이 맞지만, 상류 자신의 코드 경로에서 찾으면 있다. 분리 분기에서
발산은 5로봇 × 10,000상태에서 **0건**이다.

### §260.1 상류 `distance` 필드는 `:636`을 경계로 서로 다른 두 양이다

`moveit_core/collision_detection_fcl/src/collision_common.cpp`의
`distanceCallback`(@ `e017c91ee`)을 끝까지 읽으면:

| 라인 | 코드 | `dist_result.distance`에 남는 것 |
|---|---|---|
| `:603` | `double distance = fcl::distance(o1, o2, fcl::DistanceRequestd(...), fcl_result)` | — |
| `:613` | `dist_result.distance = fcl_result.min_distance` | `fcl::distance`의 반환값 = **분리 거리** |
| `:636` | `if (distance <= 0 && cdata->req->enable_signed_distance)` | 분기 |
| `:647` | `std::size_t contacts = fcl::collide(o1, o2, coll_req, coll_res)` | — |
| `:663` | `dist_result.distance = -contact.penetration_depth` | 접촉 집합에서 **고른 접촉 하나의 깊이** |

`:663`은 `:613`이 넣은 값을 버리고 덮어쓴다. 반면 이 필드에 붙은 계약은 하나뿐이다
— `moveit_core/collision_detection/include/moveit/collision_detection/collision_common.hpp:262`
"The distance between two objects. If two objects are in collision, distance <= 0.",
`:318` "ResultsData for the two objects with the minimum distance". **계약은 하나,
값의 출처는 둘이다.** §5 Phase 3의 조건 문구는 그 하나의 계약만 읽고 쓰였다.

`doc/upstream-bugs.md`에 등재된 이 함수의 결함 셋은 **전부 `:636` 아래**에서만
발생할 수 있다:

| 등재 항목 | 인용 라인 | `:636` 아래인가 |
|---|---|---|
| `distance-callback-max-contact-depth` | `:646`, `:650-659`, `:662-663` | 예 — `fcl::collide` 집합의 `max` 선택 자체다 |
| `fcl-distance-sentinel-survives-zero-contacts` | `:613`, `:636`, `:647-648`, `:663` | 예 — `:648`의 `if (contacts > 0)`에 `else`가 없어 생긴다 |
| `distance-callback-threshold-suppresses-deeper-pairs` | `:574`, `:594`, `:603`, `:608`, `:663`, `:694` | 예 — 아래 문단 |

셋째만 설명이 필요하다. `:608`의 `if (distance < dist_threshold)` 게이트는 분리
쪽에서도 돈다. 그러나 누적 최소값이 아직 양수인 동안 그 비교는 분리 거리끼리의
비교이고, 더 깊은 쌍을 버리지 않는다. 버리는 일은 어떤 쌍이 관통해 `:663`이
누적값을 음수로 만든 뒤에야 생긴다 — 그때부터 게이트는 "관통 깊이"를 임계값으로
들고 "분리 거리"를 재기 때문이다. 즉 셋째 결함도 `:663`의 산물이다.

따라서 **"분리 분기에서는 세 결함이 전부 잠잠하다"**는 요약이 아니라 반증 가능한
예측이고, 아래가 그 측정이다.

### §260.2 5로봇 × 10,000상태 — 분리 분기 발산 0건

`tools/moveit-diff`에 `DistanceBranchStats`를 넣어 두 모집단을 갈랐다. 가르는
기준은 **오라클이 발표한 값의 부호**이며 포트 쪽 부호가 아니다(포트로 가르면
포트가 자기 채점 기준을 고르게 된다). 경계는 `> 0`이고 `>= 0`이 아니다 —
`:636`이 보는 것이 `distance <= 0`이기 때문이다.

시드 1, 로봇당 10,000상태, 절의 자기 허용오차 `1e-4`:

| 로봇 | `collision: bool` | 분리 dis/tot | 분리 최악 | 관통 dis/tot | 관통 최악 |
|---|---|---|---|---|---|
| panda | 0/10000 | **0/8734** | `5.134781e-16` | 10715/11266 | `2.738380e0` |
| prbt | 6854/10000 | **0/9611** | `8.892585e-5` | 10249/10389 | `1.000000e0` |
| fanuc | 0/10000 | **0/5053** | `1.744264e-15` | 7226/14947 | `2.897030e-1` |
| dual_arm_panda | 0/10000 | **0/7703** | `8.396062e-16` | 4047/12297 | `1.882271e-1` |
| pr2 | 0/10000 | **0/9958** | `6.056201e-7` | 10022/10042 | `3.217869e-1` |

합계 100,000개의 비교 면(로봇당 10,000상태 × self/robot 두 면) 중 분리 쪽이
41,059개, 관통 쪽이 58,941개다. **분리 쪽 발산은 41,059개 중 0개, 관통 쪽은
58,941개 중 42,259개.** 발산은 상태에 퍼져 있는 것이 아니라 한 분기 안에 갇혀
있다. `prbt`의 `bool` 6,854건은 §229.1의
접선 판정 문제이고 이 절이 건드리지 않는다.

### §260.3 분리 쪽 잔차의 주인 — pr2에서는 정답이 닫힌 형태로 알려져 있다

분리 쪽 최악값이 기계 정밀도가 아닌 두 로봇(prbt `8.9e-5`, pr2 `6.1e-7`)이 남는다.
"작으니 괜찮다"는 판정이 아니므로, 그 잔차가 누구 것인지를 따로 쟀다.

pr2의 분리 쪽 상위 8건은 **전부** `*_caster_*_wheel_link` 대 `floor`다. 이 쌍은
정답이 계산 가능하다:

- 바닥은 `Cuboid::new(4.0, 4.0, 0.1)`(반크기가 아니라 전체 크기)를 `z = -0.05`에
  둔 것이므로 윗면이 평면 `z = 0`이다 (`tools/moveit-diff/src/main.rs:729-736`).
- 캐스터 바퀴의 충돌 형상은 반지름 `0.074792`의 원기둥이고, 중심 높이는
  `0.051 + 0.0282 + 0.0 = 0.0792`다 (`fixtures/pr2.urdf`의
  `base_footprint_joint` / `*_caster_rotation_joint` / `*_caster_*_wheel_joint`).

따라서 간극은 `0.0792 - 0.074792 = 0.004408`이고, **상태와 무관한 상수**다 —
`*_caster_rotation_joint`는 `z`축 회전이라 중심 높이를 바꿀 수 없고,
`*_caster_*_wheel_joint`는 원기둥을 자기 축으로 돌린다.

| | 값 | 닫힌 형태와의 오차 |
|---|---|---|
| 닫힌 형태 | `4.40800000000000913e-3` | — |
| 이 포트 (case 6338) | `4.40799999994411981e-3` | `5.588932e-14` |
| 오라클 (case 6338) | `4.40860562000265372e-3` | `6.056200e-7` |

**오라클이 포트보다 약 780만 배 멀다.** 그러므로 pr2의 분리 쪽 잔차 `6.056201e-7`은
포트의 오차가 아니라 기준의 오차이며, 이것을 흡수하려 허용오차를 넓히는 것은
...
오라클도 도커도 쓰지 않고 `0.43s`에 돈다. 변이로 물리는지 확인했다: 양의 분기에
`+5e-12`를 더하면 FAIL, `+5e-13`이면 PASS — 즉 유효 감도가 핀 값을 감싼다.
argmin 쌍 단언도 공전하지 않는다(`_caster_`를 `_zzz_`로 바꾸면 FAIL).

### §260.4 prbt의 `8.9e-5`도 포트에 물을 수 없다 — 기준 자신이 그만큼 흔들린다

prbt는 이 픽스처들 중 유일하게 충돌 형상이 전부 프리미티브이고(17/17: box 4,
cylinder 10, sphere 3), 분리 쪽 최악값이 가장 크다. 그 최악값이 앉는 쌍은
`prbt_link_4`(box `0.121 x 0.08 x 0.17`)와 `prbt_base_link`(cylinder `r=0.09275`,
`l=0.13`)다.

`distanceCallback`은 요청을 `fcl::DistanceRequestd(cdata->req->enable_nearest_points)`
로 만든다(`:603`) — 인자가 하나뿐이므로 `distance_tolerance`가
`fcl/narrowphase/distance_request.h`의 생성자 기본값 `1e-6`으로 남는다. fcl이 그
필드에 붙인 설명은 "the threshold used in GJK algorithm to stop distance
iteration", 즉 **반복 정지 문턱이지 오차 한계가 아니다.**

`tools/fcl-distance-tolerance-probe/`가 오라클 이미지 안에서 바로 그 box/cylinder
쌍의 분리 자세 2,000개를 세 번씩 잰다(MoveIt의 요청, 문턱만 `1e-12`로 조인 것,
그리고 `1e-12` + `GST_INDEP`). 실측:

- `|기본 - 조임|` 최대 `4.418002e-4` (>1e-6 412건, >1e-5 60건, >1e-4 7건)
- `|조임 - INDEP|` 최대 `9.761567e-8` — 즉 조인 두 solver는 서로 4,526배 더 가깝다

둘째가 있어야 첫째를 "기본값의 오차"로 읽을 수 있다. 없으면 solver 하나가 헤매는
것과 구별되지 않는다. **`4.4e-4`만큼 흔들리는 기준으로 `8.9e-5`를 포트의 오차라고
판정할 수 없다.** `tools/ci/verify-fcl-distance-tolerance.sh`가 이것을 매번 다시
재고(약 3초, digest-gated, 도커 없으면 큰 소리로 SKIP), 핀 셋 모두 실측 후 여유를
적어 박았다: `MIN_DRIFT=1e-5`(실측 `4.418002e-4`), `MAX_SOLVER_GAP=5e-7`(실측
`9.761567e-8`, 5.1배), `MIN_RATIO=100`(실측 4,526배).

### §260.5 예측 둘 중 하나는 맞았고 하나는 반증됐다

fanuc·pr2 결과가 나오기 전에 두 예측을 적어 두었다.

1. **반증됨.** "분리 쪽 최악값이 프리미티브 *비율*을 따르므로 pr2는
   `[1e-6, 1e-4)`에 든다" — pr2 실측은 `6.056201e-7`로 예측한 하한보다 한 자릿수
   아래다. 비율은 크기를 설명하지 못한다.
2. **맞음(다시 적은 형태).** 크기가 아니라 **프리미티브 대 프리미티브 쌍이
   만들어질 수 있는가**가 꼬리의 유무를 가른다. 씬의 프리미티브는 URDF 것에
   `floor` 상자 하나가 더해진다: panda 메시 11 + 상자 1, fanuc 메시 7 + 상자 1,
   dual_arm_panda 메시 22 + 상자 1 — 프리미티브가 하나뿐이라 프리미티브끼리의
   쌍이 성립하지 않는다. prbt는 17 + 1, pr2는 (메시 37 · 프리미티브 17) + 1이라
   성립한다. 실측: 앞의 세 로봇은 `1e-12`를 넘는 쌍이 **정확히 0건**, 뒤의 둘은
   꼬리가 있고, 그 꼬리의 상위 항목은 전부 프리미티브끼리의 쌍이다(pr2는
   원기둥 바퀴 대 상자 바닥, prbt는 box 대 cylinder). fanuc가 마지막으로 들어와 3/3(프리미티브 1개)과 2/2(복수)를 채웠다 — `0/5053`, 최악 `1.744264e-15`, `1e-12` 초과 0건.

### §260.6 판정 — (iii)이 주고 (ii)가 받친다

세 갈래 중 **(iii) 조건 문구가 잘못 쓰였다**가 답이다. 조건은 `distance`를 한
양으로 보고 `1e-4`를 걸었지만, 상류가 그 이름으로 발표하는 것은 `:636`을 사이에
둔 두 양이다. 그중 거리인 쪽에서 포트는 5로봇 전부 조건을 만족하고(§260.2), 정답이
독립적으로 알려진 유일한 쌍에서 오라클보다 780만 배 정확하다(§260.3).

**(i)은 열리지 않는다.** 분리 쪽에서 고칠 발산이 0건이고, 남은 잔차 둘은 각각
닫힌 형태(§260.3)와 기준 자신의 정밀도(§260.4)로 주인이 밝혀졌다.

**(ii)는 이미 서 있다.** 관통 쪽에서 못 맞추는 원인은 새로 발견할 것이 아니라
`doc/upstream-bugs.md`에 이미 등재된 셋이고, §260.1이 그 셋이 전부 `:663`의
산물임을 상류 소스로 보였다. 이 절은 새 항목을 올리지 않는다.

그러므로 §5의 행을 다음으로 고친다 — **허용오차 `1e-4`는 그대로다.** 고친 것은
비교되는 *모집단*이지 문턱이 아니다.

- 조건: `distance: f64` 가 분리 분기(오라클 값 > 0)에서 `1e-4` 이내 일치;
  관통 분기는 상류 결함 3건이라 비교 불가
- 판정: **PARTIAL**

`MET`이 아니라 `PARTIAL`인 이유는 분리 쪽이 모자라서가 아니다. 필드의 절반이
상류와 대조되지 않은 채 남고, 그 사실이 표를 훑는 사람에게 보여야 하기 때문이다.
`MET`으로 적으면 "충돌 거리 패리티가 확립됐다"로 읽힌다.

### §260.7 고친 조건이 여전히 잡는 것 — 양쪽 분기에 변이를 넣어 쟀다

모집단을 좁히면 무엇이 안 보이게 되는지가 이 판정의 유일한 위험이다. 그래서
`crates/moveit-collision/src/parry.rs`의 `accumulate_distance`가 쌍의 답을
발표하는 단 한 곳에 변이를 넣고 쟀다(둘 다 측정 후 되돌렸고, 되돌린 바이너리가
변이 전과 바이트 동일함을 확인했다).

**분리 쪽 변이(양수에 `+2e-4`).** 다시 쓴 절이 5로봇 전부에서 빨개진다. 100상태
게이트 실행:

| 로봇 | 분리 dis/tot | 분리 최악 | 관통 dis/tot(비채점) | 판정 |
|---|---|---|---|---|
| panda | 88/88 | `2.000000e-4` | 103/112 | UNMET |
| prbt | 96/96 | `2.000000e-4` | 103/104 | UNMET |
| fanuc | 46/46 | `2.000000e-4` | 78/154 | UNMET |
| dual_arm_panda | 79/79 | `2.000000e-4` | 37/121 | UNMET |
| pr2 | 100/100 | `2.000000e-4` | 100/100 | UNMET |

최악값이 정확히 주입량이고, 비채점 열은 변이 전 실행과 값이 같다 — 변이가 채점
대상 분기에만 닿았다는 뜻이다. 같은 변이를 `cargo nextest run -p moveit-collision`
이 **15건**으로 잡는다(`0.977s`, 도커 불요; `*_collision_matches_the_oracle` 3건
포함).

**관통 쪽 변이(음수·0에 `-2e-4`).** 다시 쓴 절은 이것을 채점하지 않는다. 그러나
같은 crate 시험이 **19건**으로 잡는다(`1.133s`): `minimum_distance_is_the_minimum`
3건, `exact_tangency_boundary` 3건, `collision_parity` 6건,
`octree_world_collision_parity` 1건, `parry::tests` 6건.

즉 좁힌 것은 **오라클과의 수치 동일성**뿐이고, 관통 쪽 포트 동작은 crate 시험이
그대로 지킨다. 그 시험들은 상류의 발표값이 아니라 포트 자신의 불변량을 걸기
때문에 상류 결함 셋을 재현하지 않고도 만족될 수 있다 — 좁히기 전 조건은 그렇지
않았다.

### §260.8 이 절이 하지 않은 것

- `collision: bool` 행을 건드리지 않았다. prbt의 6,854건은 §229.1의 판정 그대로
  미충족이고, 이 절의 게이트 실행에서도 prbt만 UNMET으로 남는다.
- 허용오차를 넓히지 않았다. `1e-4`는 조건 자신의 숫자 그대로이고, 게이트의
  실패 문구에 "모집단도 더 좁히지 말 것"을 새로 적어 두었다.
- `doc/upstream-bugs.md`에 항목을 올리지 않았다. 관통 쪽 원인은 이미 등재된
  셋이고, 이 절이 더한 것은 그 셋의 **경계**(전부 `:663`의 산물)이지 새 결함이
  아니다.
...
- prbt의 `8.9e-5`를 닫힌 형태로 확인하지 않았다. pr2의 캐스터 쌍과 달리 box 대
  ...
  자릿수를 판정할 수 없다"는 더 약한 주장이다.
- 분리 분기의 잔차를 0으로 만들지 않았다. prbt `8.892585e-5`와 pr2 `6.056201e-7`은
  그대로 남고, 게이트는 매 실행 그 값을 찍는다.
- §5 표의 근거 열은 번호가 배정된 뒤에 옮겼다(`§229.3` → `§260`). 절을 쓰는
  시점에는 쓸 수 없었다 — `check-phase-status.sh`가 그 열을 실재하는 heading으로
  해석하므로 미배정 자리표시자를 넣으면 그 게이트가 빨개진다(§247.6). 그 사이
  §229.3에는 이 절로 잇는 문단을 두어 두 홉으로 닿게 해 두었고, 번호가 붙은
  지금은 그 문단도 §260을 직접 가리킨다. `tools/ci/verify-phase3-collision-sweep.sh`가
  이 절을 번호가 아니라 "§5의 `distance: f64` 행이 가리키는 절"로 부르는 것은
  그대로 두었다 — 그 표현은 번호가 바뀌어도 썩지 않는다.

## §261 §258을 41 커밋 뒤진 트리에서 유도했다 — 머지 후 전수 재검증, 그리고 한 문장이 실제로 거짓이었다 (2026-08-06)
...
### §261.2 인용 40건이 낡지 않은 이유는 줄 수가 아니라 blob 해시로 확인했다
...
### §261.3 거짓이었던 한 문장 — §258.4의 Phase 9 항목
...
### §261.4 `check-citation-drift.py`가 초록인 것은 근거가 못 된다 — 이 저장소에서 재현했다
...
`PORTING-PLAN.md`는 덧붙이기 전용이지만 이 라운드는 **§258 본문을 다섯 곳
고쳤다**. 다섯 다 §258이 이 세션의 자기 절이고, 남겨두면 거짓이거나
검사받지 못하는 문장이기 때문이다:

1. §258.4 Phase 9 항목 — §261.3의 거짓 문장을 정정.
2. §258.4 첫 문단 — `§5 표 806/807/819행`이라는 줄번호 인용을 제거.
   `PORTING-PLAN.md`는 매 라운드 자라므로 그 줄번호는 구조적으로 낡는
   인용이다.
3. §258.3 — 다섯 개 크레이트 doc 인용에 그 줄의 내용을 붙임.
4. §258.4 마지막 문단 — Phase 8 항목을 더하고, "UNMET 세 행"이라는 개수를
   §5 표 참조로 교체(§261.6).
5. §258 헤딩 — 같은 이유로 "UNMET 세 행" → "§5의 미충족 행".

다른 절은 한 줄도 건드리지 않았다. §258 밖의 삭제·변경 줄은 0이다 —
아래 명령으로 확인한다.

```
$ git diff <머지 커밋>..HEAD -- PORTING-PLAN.md -U0 | rg '^-[^-]'
```

### §261.6 두 번째 머지에서 또 하나 — §260이 판정어를 바꿨고, 계기가 그 사본을 들고 있었다

첫 머지(41 커밋) 뒤 이 절을 쓰는 동안 main이 다시 8 커밋 움직였고, 다시
머지했다. 이번에는 `PORTING-PLAN.md`가 충돌했다 — 양쪽이 파일 끝에
덧붙였기 때문이다. 충돌 해소 전에 base 구간이 비어 있는지 확인했다(양쪽
추가끼리의 충돌이면 비어 있어야 하고, 비어 있지 않으면 삭제가 숨어 있다).
비어 있었다. 해소 후 양쪽 절이 다 살아 있는지는 헤딩 수(119 → 120)와
양쪽 파일의 모든 비어 있지 않은 줄이 머지 결과에 있는지로 확인했다.
어느 쪽에서도 사라진 줄은 상대가 그 줄을 **고친** 경우뿐이었고, 그중
하나가 §5 표의 `distance: f64` 행이었다.

**§260이 그 행을 `UNMET`에서 `PARTIAL`로 바꿨다.** 그래서 §258이 세 군데에
적어 둔 "UNMET은 셋"은 거짓이 됐고, `classify-unported.py`의
`UNMET_BLOCKERS`도 자기 사본을 들고 조용히 낡아 있었다 — 아무 게이트도
발화하지 않았다. 이것이 §261.3의 Phase 9 문장과 같은 결함의 두 번째
사례이고, 원인도 같다: **다른 파일에 사는 사실의 사본**.

패치가 아니라 사본을 없애는 쪽으로 고쳤다:

- `check_phase_coverage()`가 §5 표를 읽는다. MET가 아닌 행마다
  `UNMET_BLOCKERS` 항목이 있어야 하고, 항목마다 MET가 아닌 행이 있어야
  하며, 항목이 적은 절 번호가 행이 인용한 절 번호와 같아야 한다. 어긋나면
  종료 코드 1이다.
- 그 검사가 즉시 **내가 못 보고 있던 드리프트를 잡았다**: §5의 Phase 9
  행은 §250.5를 인용하는데 계기는 §250.6이라고 적고 있었다.
- Phase 8(`UNMEASURED`) 행은 지금까지 계기에 항목이 아예 없었다. 검사가
  그것도 잡았고, 항목을 넣었다. 그 항목의 후보 접두사는 87건 중 8건을
  고르므로, 이제 발화하는 후보는 17건이다(9 + 8).
- §258과 이 절 어디에도 "셋"이라는 개수를 적지 않는다.

발화 확인(변이 셋, 전부 `exit=1`):

| 변이 | 잡히는가 |
|---|---|
| Phase 8 행을 `MET`으로 → 항목이 대응할 행을 잃음 | 예 |
| MET가 아닌 행을 새로 추가 → 대응 항목 없음 | 예 |
| Phase 3 행의 인용 절을 §229.1 → §229.2 | 예 |
...
## §262 Phase 3 두 조건의 판정을 확정한다 — `bool`은 조건을 고치고, `distance`는 조건 그대로 미충족이다 (2026-08-06)

> **머지 시점 주석 (병합자).** 이 절이 §5 표에 요청한 두 변경 중 어느
> 것도 아직 표에 반영하지 않았다. 표는 `collision: bool` = UNMET
> (근거 §229.1), `distance: f64` = PARTIAL (근거 §260) 그대로다.
>
> - **`bool`의 MET**은 §262.2가 스스로 "새로 확인하지 못한 것"으로
>   적어둔 재실행이 끝나야 붙는다. 그 절이 재실행을 막았다고 적은 사유
>   (`third_party/moveit_resources`가 없음)는 **틀렸다** — 그 트리는
>   기본 체크아웃(`/home/stevek/work/moveit-rs`)에 존재하고 채워져
>   있다. `.gitignore`의 `/third_party` 때문에 `git worktree`가 그것을
>   실체화하지 않을 뿐이라, 모든 caucus 워크트리에서 없는 것으로 보인다.
...
>   적용되지 않는다. 표는 §260의 PARTIAL을 따른다.

§229.1/§229.3이 두 절의 원인을 확정했고, §251이 `bool`의 원인을 fcl의
협면 등록표로 대체했으며, §247이 `distance`의 fanuc·pr2 원인을 닫았다.
그 세 절 모두 §5 현황표의 판정 자체는 UNMET로 남겨 두었다 — 원인은
바뀌었지만 표는 그대로였다. 이 절이 묻는 것은 그 UNMET이 "포트가
못 미쳤다"로 읽히는 것이 맞는지, 세 절의 결론을 실측으로 세 선택지에
대입해 확정한다.

### §262.1 (a) 포트를 fcl에 맞춘다 — 실제로 시도해 배제했다

가장 좁게 시도할 수 있는 형태로 실제로 구현했다: `crates/moveit-collision/
src/parry.rs`의 `accumulate_collision`에서, 두 형상의 `parry3d_f64::shape::
ShapeType`이 (`Cylinder`, `Cuboid`) 쌍일 때만 `contact.dist >= 0.0`이면
접촉을 기각하는 분기를 추가했다(그 외 모든 쌍은 손대지 않았다). 빌드는
깨끗했고, `cargo nextest run -p moveit-collision`(251건)을 돌리니 3건이
실패했다:

- `exact_tangency_is_decided_per_shape_pair`의 두 테스트가 예측대로
  실패했다 — 25셀 표의 `box × cylinder`(그리고 근접-간극 표에서는
  `cylinder × box`도 함께) 셀이 뒤집혔다고 이름으로 지목했다.
- `exact_tangency_boundary::the_collision_boundary_sits_in_a_positive_gap`도
  예측대로 실패했다 — `-3e-8`(양의 간극 `3e-8`) 지점에서 더 이상
  충돌로 보고되지 않는다고 지목했다.

그런데 `exact_tangency_boundary::the_tie_is_decided_below_one_ulp` —
prbt의 실제 픽스처가 정확히 겪는 그 지점 — 는 **여전히 통과했다**.
그 테스트는 tie에서 `collides == true`를 확인하는데, 이 백엔드가 실제로
재는 값은 `0.0`이 아니라 링크 pose 합성의 반올림으로 생긴
`-2.775558e-17`(§229.1이 이미 인용)이다 — `>= 0.0` 판정의 기각 쪽에
들지 **않는다**. 즉 이 실험으로 만든 코드는 25셀 합성 표의 비트-정확한
동점은 뒤집지만, prbt가 실제로 겪는 결함은 전혀 닫지 못한다. 닫으려면
`dist > -1e-10` 같은 임의의 엡실론이 필요하고, 그 크기를 상류가 정당화할
근거는 없다 — §251.4가 추론으로만 말했던 "양의 예측값은 상류에 크기를
잴 근거가 없는 엡실론" 반대를, 이번에는 숫자로 확인한 것이다.

변경은 되돌렸다 (`git diff crates/moveit-collision/src/parry.rs`가
비어 있음을 확인). 이 실험이 배제하는 것은 "코드를 조금 더 다듬으면
된다"는 가정이다 — 가장 작게 자른 시도조차 실제 결함을 안 건드리고,
건드리게 만들려면 상류 근거 없는 숫자를 하나 더 심어야 한다.

`tools/ci/verify-fcl-tangency-dispatch.sh`도 이번 라운드에 직접
(docker로) 다시 돌려 §251.1의 49셀 표를 재확인했다 — 49/49 일치, 이미지
자체 fcl 헤더에서 파싱한 34개 비-libccd 특수화 집합과도 일치. §251의
인용을 그대로 믿지 않고 오늘 다시 재도출한 결과다.

### §262.2 (b) 조건을 고친다 — `collision: bool`은 이것으로 닫는다

§218.3이 이미 잰 10,000상태×5로봇(panda/prbt/fanuc/dual_arm_panda/pr2 —
`tools/ci/verify-phase3-collision-sweep.sh`가 실제로 도는 로봇 집합이고,
조건문의 "3로봇"은 그 집합과 어긋나 있었다. 이번에 조건문 자체를
고치며 함께 바로잡는다)에서, `bool` 불일치는 panda/prbt/fanuc/
dual_arm_panda/pr2 중 prbt 하나뿐이고(6,854/10,000), 그 6,854건
**전부**가 §229.1이 지목한 "robot same-pair value divergence"와 정확히
같은 수다 — 즉 prbt의 `bool` 실패 전체가 상태와 무관하게 반복되는 단
하나의 좌표(베이스 링크 실린더 밑면과 바닥 윗면이 정확히 z=0에서
맞닿는 좌표)다. §251.1이 49/49 셀로, §251.2가 MoveIt 래퍼 전체로 각각
확인했듯 그 좌표에서 상류 자신도 도형 쌍에 따라 답이 갈린다(같은 정확
접촉인데 prbt 쌍은 `false`, `octree_world_collision_response.json`
case 4는 `true`). 상류가 하나로 정하지 않은 좌표에서 100% 일치를
요구하는 것은 어떤 구현도 만족시킬 수 없는 조건이다.

그래서 조건문 자체를 고쳤다(§5 표): "10,000×5로봇에서 100% 일치"에
"정확 접선 좌표는 제외 — 상류 자체에 규약이 없음"을 덧붙였다. 이
제외는 §251의 49셀 실측(이번 라운드에 재확인)에 근거를 고정한다.
제외를 적용한 뒤 §218.3이 이미 실측한 숫자를 다시 읽으면: prbt를 포함한
5개 로봇 전부, 이 한 좌표를 뺀 모든 상태에서 `bool`이 이미 100%
일치한다. 이것은 새로 잰 숫자가 아니라 §218.3의 실측을 조건의 새
정의로 다시 읽은 것이므로, 판정을 **MET**로 바꾼다.

이 절이 새로 확인하지 못한 것: 오라클의 10,000×5로봇 스윕을 이 조건
문구로 다시 돌려 "정확 접선 좌표를 뺀 나머지가 정말 0건"이라고 직접
재확인하지는 못했다 — `third_party/moveit_resources`가 이 워크트리에서
전혀 보이지 않는다(`.gitignore`의 `/third_party`, 이 워크트리에는
심링크조차 없음; `bash tools/ci/verify-fixture-provenance.sh`가
"third_party/moveit_resources is absent"로 확인). 이는 round 2의
item 4a가 부딪힌 것과 같은, 이 워크트리 고유의 환경 제약이다. MET
판정은 §218.3이 이미 실측한 숫자(다시 재지 않고 읽은 것, §201의 규칙대로)
위에 서 있고, 그 숫자가 조건 재정의 이후에도 유효한지는 새 조건 문구로
직접 재실행해야 완전히 닫힌다.

### §262.3 `distance: f64` — 조건은 그대로, 판정도 UNMET 그대로

`distance`의 불일치는 좌표 하나로 제외할 수 있는 모양이 아니다.
§218.3의 실측: panda 9,543/10,000, fanuc 6,113/10,000, pr2
9,988/10,000, dual_arm_panda 3,508/10,000 — 상태의 절대다수에서
어긋나고, 어긋나는 크기도 상태마다 다르다(§229.3: panda의 이탈은 바닥
폭에 비례해 무한히 커진다; §247.4: pr2 케이스 1584의 3.218e-1과 케이스
153의 3.77e-2는 같은 기전이면서도 크기가 다르다). 좌표를 제외하는
방식으로 조건을 고칠 근거가 없다 — 제외할 "그 좌표"가 하나로 특정되지
않는다.

그래도 판정을 MET/PARTIAL로 바꾸지 않는 이유는 원인이 아니라 정의다:
이 포트의 숫자와 오라클의 숫자가 서로 다른 것을 재고 있다는 것이 §229.3/
§247의 결론이지만, "condition의 문구"는 여전히 "포트와 오라클이 `1e-4`
이내로 일치"이고 그 문구 그대로는 실제로 만족되지 않는다. panda는
상류가 최대 침투깊이(escaping-artifact)를 재고 있고(§229.3), fanuc·pr2는
상류의 `distanceCallback`이 부호 있는 값과 침투깊이를 같은 변수로 착각해
더 깊은 쌍을 걸러낸다(§247.3) — 두 원인 모두 §247.2가 오라클 자신에게
같은 상태에서 ACM만 바꿔 되물어 "오라클도 자기 헤드라인 답보다 깊은
값을 이미 갖고 있다"는 것으로 인과까지 확인했다. 그러나 두 원인 다 상태별
크기가 고정된 상수가 아니어서(§247.5: 2.897e-1, 3.218e-1, 3.77e-2가 모두
같은 기전에서 나온 서로 다른 값) 조건의 문구를 조정할 고정폭을 줄 수
없다. 판정은 **UNMET**으로 남긴다 — 사유는 "포트의 오차"가 아니라
"상류가 다른 정의를 계산해 놓고도 발표하지 않는다"이다.

근거 열만 이 절로 옮긴다. §229.3이 스스로 "옮기지 않았다"고, §247.6도
같은 이유로 "옮기지 않았다"고 남긴 요청을 이 절이 한 홉으로 흡수한다.

### §262.4 (c) 판정 어휘 — 이미 있었고, 필요하지 않았다

`tools/ci/check-phase-status.sh`를 직접 읽었다: 허용 어휘는 이미
`{MET, UNMET, PARTIAL, UNMEASURED}`이고 (`UNMEASURED`는 Phase 8
CHOMP/STOMP 행에서 이미 쓰이고 있다), `PARTIAL`만 아직 한 번도 쓰인 적이
없다. 게이트를 고칠 필요는 없었다.

`bool` 행에 `PARTIAL`을 쓰는 대신 조건문 자체를 고쳐 `MET`으로 간
이유: `PARTIAL`은 무엇이 제외됐는지, 왜 제외됐는지를 말하지 않는다 —
그 정보는 조건 열의 산문에 박아 두어야 다음 독자가 `check-phase-status.sh`
말고는 아무것도 다시 안 읽어도 알 수 있다. `distance` 행에도 `PARTIAL`을
고려했으나 기각했다: 실패가 상태의 소수가 아니라 다수(최대 pr2
9,988/10,000)이고 크기도 고정폭이 아니어서, `PARTIAL`은 "거의 다 됐다"는
인상을 주지만 실측이 말하는 것은 그 반대다.
...
- `/move_action` leg A: 계획 가능한 goal이 `SUCCEEDED`로 끝나고
  ...
  §NEW.8이 적는다.
- leg B(무변경 상류 C++ `MoveGroupInterface`): §NEW.8이 적는다 — 이 절이
  처음 쓰였을 때의 답과 다르다.
...
- **goal의 `planning_scene_diff`가 무시된다.** 이 절이 처음 쓰였을 때는
  scene monitor 자체가 없었고 §257이 그것을 지었다(§NEW.8). 남은 것은
...
(2) `PLANNER_MANAGERS`가 비어 있는 경우(§NEW.4의 링크 앵커가 빠지면
