...
| Phase 3 | `distance: f64` 가 분리 분기(오라클 값 > 0)에서 `1e-4` 이내 일치; 관통 분기는 상류 결함 3건이라 비교 불가 | PARTIAL | §229.3 | 2026-08-06 |
...
`main.rs:2928-2938`의 `oracle_only`/`rust_only` 독코멘트가 바로 그
구분을 위해 존재한다고 적고 있다.
...
`main.rs:4020`·`main.rs:4138`에 있고, 둘 다
...
부른다(`main.rs:3173`, `main.rs:3184`). `Verdict` 부분은 맞고,
...
(`main.rs:3453`). 두 필드 쪽은 여전히 참이다: `stats.rust_degenerate`나
...
고정했다(`tools/moveit-diff/src/main.rs:3490`, 내가 확인했다).
...
남은 둘은 `tools/moveit-diff/src/main.rs:4020,4138` —
...
근거 열이 아직 §229.3을 가리키는 것은 §247.6이 적어둔 번호-배정 이유 그대로다.
...
- §5 표의 근거 열을 이 절로 옮기지 **않았다**. `check-phase-status.sh`가 그 열을
  실재하는 heading으로 해석하므로 번호가 배정되기 전에는 쓸 수 없다 — §247.6이
  적어둔 그대로다. 병합 시 번호가 붙으면 그 열을 `§229.3`에서 이 절로 옮겨야
  한다. `tools/ci/verify-phase3-collision-sweep.sh`가 이 절을 번호가 아니라
  "§5의 `distance: f64` 행이 가리키는 절"로 부르는 것도 같은 이유다 —
  `check-porting-plan-sections.sh`는 `.md`/`.rs`만 훑으므로 쉘 스크립트에 적힌
  미배정 자리표시자는 병합 때 치환되지 않고 그대로 썩는다.
...
### §NEW.6 두 번째 머지에서 또 하나 — §260이 판정어를 바꿨고, 계기가 그 사본을 들고 있었다
...
발화하지 않았다. 이것이 §NEW.3의 Phase 9 문장과 같은 결함의 두 번째
