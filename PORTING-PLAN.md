...
## §NEW planning scene 토픽 구독을 지었다 — 그리고 그 구독이 먹일 변환 셋도 없었다 (2026-08-06)

### §NEW.1 먼저 "왜 안 지었는가" — D1 표시가 근거로 든 것과, 그것이 지금도 유효한가
...
### §NEW.2 브리프와 §226.3의 전제를 트리에 대고 확인했다 — 둘이 틀렸다

이 라운드의 브리프는 전제 셋을 실었다. 전부 직접 확인했다.

| 전제 | 확인 | 결과 |
|---|---|---|
| 세 변환은 이미 `crates/moveit-scene/src/scene.rs`에 순수 `TryFrom`으로 포팅돼 있다 | 위 §NEW.1의 두 표시를 읽음 | **틀림.** 트리에 없었다. 같은 트리의 모듈 문서가 "포팅하지 않는다"고 반대로 적어 두고 있었다 |
| `rg -n 'create_subscription' ros/moveit-ros/src/ -t rust`가 0건 | 실행 | 0건 맞음 — 다만 이 앵커는 **아무것도 재지 못한다**(§NEW.7) |
| §226.4의 네 조각 중 셋은 존재한다 | 앵커 재실행(§NEW.7) | 맞음 |
...
### §NEW.3 지은 것 (1) — `is_diff`를 불린이 아니라 타입으로
...
### §NEW.4 지은 것 (2) — 구독은 기존 노드에 올린다, 그리고 관측 가능하게 만든다
...
### §NEW.5 게이트 다리 C — 발행이 아니라 **답이 갈리는 것**을 단언한다
...
### §NEW.6 물린 자국 넷 — 각 단언이 무엇을 잡는지 이름으로 확인
...
### §NEW.7 §226.3의 네 조각을 지금 트리에서 다시 센다 — 그리고 앵커 하나가 눈이 멀어 있었다
...
### §NEW.8 Phase 9 판정 — 바꿀 근거가 없다, 그 이유
...
- **§226.3 표의 근거 칸 둘이 틀린 채로 남아 있다** — 구독 행의 "이미
  순수 `TryFrom`으로 포팅" 서술(§NEW.2)과 `create_subscription` 앵커
  (§NEW.7). 역사 기록이라 본문을 고쳐 쓰지 않고 여기에 정정을 적는다.
  §250.1의 같은 앵커도 같다.
