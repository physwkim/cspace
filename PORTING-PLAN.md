...
- **다른 쌍** (pair-flip) — 이긴 쌍이 서로 다르다. pr2의 self가
  ...
  승자 교체"라고 적었던 이 줄의 이전 판은 값에 대한 가설이었고, §NEW가
...
**(§NEW에서 이어짐)** 그 pair-flip을 "근접 동점"이라 읽은 것 역시
측정되지 않은 가설이었고, §NEW가 두 후보를 한 metric에 놓고 재어
반증했다. fanuc·pr2의 원인은
...
- fanuc의 2,897배를 이 기전으로 설명하지 않는다 (§218.4 자신이 "이탈
  ...
  **둘 다 §NEW가 닫았다** — 원인은 임계값 게이트의 억압이고, pr2는
  이 절의 기전과 그것이 겹친 경우다.
...
## §NEW `distance` 절의 fanuc·pr2 원인 — 근접 동점이 아니라 상류가 최소값을 발표하지 않는다 (2026-08-06)
...
### §NEW.1 "pair-flip = 근접 동점"은 값에 대한 주장이었고, 값은 측정된 적이 없다
...
### §NEW.2 상류 자신의 질의가 자기 발표값을 반증한다 (기하는 그대로, ACM만 바뀐다)
...
### §NEW.4 pr2도 같은 기전이다 — 세 번째 원인이 아니다

pr2 10,000상태(seed 1), pair-flip 전수를 §NEW.1의 두 양으로 분류:
...
### §NEW.5 판정 — 두 절 다 이 픽스처 위에서 닫히지 않고, 재측정이 그것을 바꾸지 않는다
...
| fanuc | `distance-callback-threshold-suppresses-deeper-pairs` (robot 2,302건 단독, self 1,586건 중 1,542건은 겹침) | §NEW.1~3 |
| pr2 | 같음 (self 9,820건 중 9,656건, 그중 9,141건은 겹침) | §NEW.4 |
...
1. **발산의 크기가 상수가 아니다.** 그것은 억눌린 쌍의 깊이이고 상태마다
   ...
   적어둔 이유가 그대로 적용된다.
2. **오라클 쪽 반증이 시드·표본에 걸려 있지 않다.** §NEW.2의 실험이 바꾼
...
3. **포트 쪽은 불변량이지 수치가 아니다.**
   ...
   `masking_the_tangent_pair_does_not_move_the_answer`가 §NEW.2 표의 첫 두
...
- 허용오차를 넓히지 않았다. 넓힐 수 없다는 것이 §NEW.5의 1이다.
