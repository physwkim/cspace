...
## §NEW 무변경 C++ `MoveGroupInterface`의 공개 선언 126개를 전수로 세고, 각 선언이 무엇을 부르는지 확정했다 (2026-08-06)
...
### §NEW.1 세는 대상은 `.hpp`다 — `.h`는 선언을 하나도 담지 않는다
...
### §NEW.3 클라이언트가 묶는 것은 아홉이다 — §252.3의 여덟에 `/joint_states`가 없었다
...
### §NEW.4 `/plan_kinematic_path`는 이 클래스의 어느 선언도 부르지 않는다
...
### §NEW.5 126개 선언의 분류 — 와이어 38, 클라이언트-로컬 88
...
### §NEW.6 판정 — 아홉 중 (b) 하나, (c) 일곱, 요구 밖 하나
