...
## §NEW Phase 4(a) 재확인 — 재시작 로또인지 독립 재측정과 상류 소스로 확인한다
...
### §NEW.3 재시작 예산을 20 → 100으로 올리면 부호가 뒤집힌다
...
### §NEW.4 스트림을 3개에서 12개로 넓히면 "항상 실패"는 공집합이 된다
...
### §NEW.5 평결과 제안 — §5는 고치지 않는다

**소스 수정으로 닫히지 않는다.** §NEW.2가 상류 소스로 직접 확인한 대로,
포트의 재시작 표본 분포·bound 의미론·수렴 판정은 상류와 이미 일치한다.
버그가 없다 — 고칠 코드가 없다.

**이것은 재시작 로또이고, "한계 성공률의 대소"는 5,000건 표본에서
정합한(well-posed) 검정이 아니다.** §NEW.3의 예산 실험이 결정적 증거다:
같은 두 "항상 실패" 케이스가 예산을 5배 늘리는 것만으로 풀리고 부호까지
뒤집힌다. §NEW.4는 표본을 넓히기만 해도 "항상 실패" 집합이 공집합으로
