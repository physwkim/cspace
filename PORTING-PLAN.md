...
| Phase 3 | `collision: bool` 이 10,000×5로봇에서 100% 일치 (정확 접선 좌표는 제외 — 상류 자체에 규약이 없음) | MET | §NEW | 2026-08-06 |
| Phase 3 | `distance: f64` 가 `1e-4` 이내 일치 | UNMET | §NEW | 2026-08-06 |
...
### §NEW.1 (a) 포트를 fcl에 맞춘다 — 실제로 시도해 배제했다
...
### §NEW.2 (b) 조건을 고친다 — `collision: bool`은 이것으로 닫는다
...
### §NEW.3 `distance: f64` — 조건은 그대로, 판정도 UNMET 그대로
...
- `crates/moveit-collision/src/parry.rs`를 고치지 않았다. §NEW.1의
...
- 오라클로 조건 재정의 이후의 10,000×5로봇 `bool` 스윕을 직접 다시
  ...
  없다(§NEW.2). §218.3의 기존 실측을 새 조건 문구로 다시 읽은 것이지,
  새로 실행한 스윕이 아니다.
