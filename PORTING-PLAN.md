...
**(§NEW에서 이어짐) 이 절의 "제외 가능한 좌표에 갇혀 있지 않다"는 거짓이다.**
...
## §NEW `distance: f64` 절 — 발산은 상류 함수의 한쪽 분기에 갇혀 있고, 조건 문구가 한 필드로 두 양을 재고 있었다 (2026-08-06)
...
### §NEW.1 상류 `distance` 필드는 `:636`을 경계로 서로 다른 두 양이다
...
### §NEW.2 5로봇 × 10,000상태 — 분리 분기 발산 0건
...
### §NEW.3 분리 쪽 잔차의 주인 — pr2에서는 정답이 닫힌 형태로 알려져 있다
...
### §NEW.4 prbt의 `8.9e-5`도 포트에 물을 수 없다 — 기준 자신이 그만큼 흔들린다
...
### §NEW.6 판정 — (iii)이 주고 (ii)가 받친다

세 갈래 중 **(iii) 조건 문구가 잘못 쓰였다**가 답이다. 조건은 `distance`를 한
양으로 보고 `1e-4`를 걸었지만, 상류가 그 이름으로 발표하는 것은 `:636`을 사이에
둔 두 양이다. 그중 거리인 쪽에서 포트는 5로봇 전부 조건을 만족하고(§NEW.2), 정답이
독립적으로 알려진 유일한 쌍에서 오라클보다 780만 배 정확하다(§NEW.3).

**(i)은 열리지 않는다.** 분리 쪽에서 고칠 발산이 0건이고, 남은 잔차 둘은 각각
닫힌 형태(§NEW.3)와 기준 자신의 정밀도(§NEW.4)로 주인이 밝혀졌다.

**(ii)는 이미 서 있다.** 관통 쪽에서 못 맞추는 원인은 새로 발견할 것이 아니라
`doc/upstream-bugs.md`에 이미 등재된 셋이고, §NEW.1이 그 셋이 전부 `:663`의
...
### §NEW.7 고친 조건이 여전히 잡는 것 — 양쪽 분기에 변이를 넣어 쟀다
...
- prbt의 `8.9e-5`를 닫힌 형태로 확인하지 않았다. pr2의 캐스터 쌍과 달리 box 대
  cylinder의 분리 거리는 자세에 따라 변하므로 상수가 없다. 그쪽 근거는 §NEW.4의
