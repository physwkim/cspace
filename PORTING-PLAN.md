...
### §229.3 `distance: f64` 절 (3로봇 전부, 2,897배~27,384배) — 배율이 아니라 다른 양이다
...
## §NEW `planning_response.cpp`의 남은 28줄 — `MotionPlanDetailedResponse::getMessage`를 포팅하지 않기로 판정했다
...
### §NEW.1 무엇을 포팅하지 않는지부터 적는다 — 이름이 아니라 규칙
...
### §NEW.2 D6은 이 함수를 `moveit-ros`로 보내지만, 변환할 코어 쪽 원본이 없다
...
§NEW.1의 **네 경계가 전부 표현 불가능해진다.** 벡터가 없으니 빈 벡터가
...
### §NEW.3 상류에서 이 함수의 호출자는 0이고, 와이어 타입은 어떤 srv/action/msg에도 실리지 않는다
...
1. 이 워크스페이스의 어떤 크레이트가 궤적 **벡터**를 담은 응답 타입을
   ...
   내놓는다). 그 순간 §NEW.1의 네 경계가 표현 가능해지고, D6에 따라
   변환은 `moveit-ros`의 `TryFrom`이어야 한다.
2. `moveit-ros`가 `MotionPlanDetailedResponse`를 담은 서비스나 액션을
   내보내야 한다 — 즉 §NEW.3의 두 번째 실측이 뒤집힌다.
3. 상류가 이 함수에 호출자를 추가한다. 그러면 §NEW.3의 첫 번째 실측이
