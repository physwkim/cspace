...
| Phase 4 | (a) 성공률이 C++ KDL 플러그인 이상 | MET | §NEW | 2026-08-06 |
...
## §NEW Phase 4 (a)를 판정 규칙째로 고정한다 — 재시작 스트림은 판정의 일부가 아니고, 시드가 겹치면 오라클이 자기 답안을 돌려받는다 (2026-08-06)
...
### §NEW.1 계측기 결함: `--seed`와 오라클 재시작 시드가 겹치면 오라클이 정답 구성을 그대로 다시 뽑는다
...
### §NEW.2 재시작이 켜지면 두 성공 수는 확률변수다 — 4906 대 4921은 포트의 5/8분위와 오라클의 **최댓값**을 비교한 것이다
...
### §NEW.3 재시작을 끈 동작점은 이 레포 자신의 게이트가 "판정 불가"라고 답하는 지점이다
...
### §NEW.4 판정 규칙을 고정한다 — 동작점은 게이트가 검정력을 갖는 최소 재시작 수, 검정은 게이트 자신
...
2. **검정**: 성공 수 두 개의 대소가 아니라 **짝지은 McNemar 게이트**
   (`|z| <= PAIRED_DIVERGENCE_Z_THRESHOLD` = 3)가 판정이다. §NEW.2가
...
0.4%이며, §NEW.2가 잰 오라클 자신의 표본 폭(4891..4921, 30건)보다 작다.
"어느 쪽이 앞서는가"가 스트림 추첨이라는 §NEW.2의 결론과 같은 그림이다.
