...
**후속 (§222에서 닫힘).** 하네스 두 개를 만들어 세 조건을 다 쟀다: CHOMP
...
**§222 시점 요약 정정.** Phase 8의 CHOMP/STOMP 항목이 측정되어 UNMET으로
확정됐으므로 이 트리에서는 **UNMET 5개(Phase 1, 3, 4, 8의 CHOMP/STOMP 항목,
9), 부분 UNMET 1개(Phase 2의 셋째 항목), 미측정 0개**다.

## §222 Phase 8의 CHOMP/STOMP 항목을 측정했다 — 벽시계가 성공률을 정하고 있었다
...
### §222.1 어떤 기준선인가 — 이 읽기는 고른 것이 아니라 강제된 것이다
...
### §222.3 상류 기본값으로 재면 숫자가 이 기계를 재고 있다
...
하나로 재생된다(§222.5). 실패 사유도 열거해 둔다. CHOMP의 미해결 120건은
...
- **CHOMP/STOMP를 각자의 상류 C++ 구현과 비교하는 읽기.** §222.1의 이유로
...
- **상류 기본 벽시계 구성의 재현 가능한 수치.** §222.3의 표는 기록이지
