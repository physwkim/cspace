...
> **상태: 미충족 (2026-08-05, §218).** 조건이 명시한 10,000 상태 × 3로봇을
> 실제로 돌려 항목별로 측정했다 (seed 1, `tools/ci/verify-phase3-collision-sweep.sh`).
>
> | 로봇 | `collision: bool` | `distance: f64` (tol 1e-4) | 측정 max\|Δ\| |
> |---|---|---|---|
> | panda | **충족** 0/10,000 불일치 | 미충족 9,543/10,000 | 2.738380e0 (1e-4의 27,384배) |
> | prbt | **미충족** 6,854/10,000 (68.54%) | 미충족 10,000/10,000 | 1.000000e0 (배율 아님 — 센티널 간격, §218.3) |
> | fanuc | **충족** 0/10,000 불일치 | 미충족 6,113/10,000 | 2.897030e-1 (2,897배) |
>
> 즉 `bool` 절은 panda·fanuc에서 충족이고 prbt에서만 깨지며, `distance`
> 절은 3종 전부 미충족이다. **허용오차를 넓혀 닫지 않는다** — §11.10이
> 미리 적어둔 대로 1e-4를 넘는 발산은 백엔드에 대한 발견이지 오차를 늘릴
> 사유가 아니다. 두 절의 원인은 서로 다르고 §218.3/§218.4에 나눠 적었다.
> §13.4가 미충족 원인으로 지목한 `<mesh>` 미로딩은 그 사이 해소됐다 —
> 같은 panda가 그때 10,000/10,000 불일치였고 지금 `bool` 0/10,000이다.
>
> **`bool` 절 원인 확정 (2026-08-06, §226.1).** 정확 접촉에서 맞출 상류
> 규약이 **존재하지 않는다**. `fcl::collide`가 도형 쌍마다 다르게 분기해,
> 정확히 닿는 두 쌍이 `false`(prbt 실린더/상자, 접촉 0개 → `-1.0` 센티널)와
> `true`(octree 리프/상자, `-0.0`)로 갈린다. 상태는 여전히 **미충족**이다.
>
> **`distance` 절 원인 확정 (2026-08-06, §226.3).** 배율이 아니라 **다른
> 양**이다. 상류는 접촉 200개 중 `penetration_depth`가 최대인 것을
> 고르는데(`collision_common.cpp:650-659`), 그 최대값은 상대 물체의 폭에
> 비례해 무한히 커진다 — 겹침을 `0.05 m`로 고정한 채 바닥 폭만 `0.4 m`
> →`20 m`로 늘리면 오라클 값이 `-0.225255`→`-9.999483`으로 44배 변한다.
> 같은 접촉 집합의 median은 두 폭 모두 `~0.05`로 옳고, 이 포트는 네 폭
> 전부에서 산포 `0.000000e0`으로 그 median과 `3.3e-5` 이내다.
> `doc/upstream-bugs.md`의 `distance-callback-max-contact-depth`. 상태는
> 여전히 **미충족**이다.
...
`d4ca334`가 §17.5의 "반대 극성" 세 자리에 만료 조건을 달면서 **세 그룹이
서로 다른 조건을 갖는다**는 것을 밝힌 것이 이번 라운드의 제일 좋은 부분이다.
...
**병합 시점 정정 — Phase 1과 Phase 3 (§218이 대체한다).** 이 절의 Phase 1·
Phase 3 판정은 `prbt` 픽스처가 없던 트리에서 나온 것이라 둘 다 뒤집힌다.
`p10-phase13`이 `prbt`를 벤더링해 커밋했으므로(§218.1) 조건이 이름 부르는
세 로봇이 처음으로 다 갖춰졌고, §218이 둘을 다시 쟀다.

- **Phase 1 — UNMET → MET.** 위의 "prbt 픽스처가 없다"는 더 이상 참이
  아니다. §218.2가 다섯 절(링크 수 / 조인트 수 / 그룹 구성 / 조인트 한계값 /
  mimic 관계)을 절별로 판정해 5종 25절 전부 일치를 보인다. 특히 prbt와
  fanuc의 mimic 관계는 **빈 집합**이며, §218.2는 빈 집합을 빈 집합으로
  읽었다는 것 자체를 비교 대상 크기와 함께 적는다 — 이 절이 못 하던 구별이다.
- **Phase 3 첫 항목 — MET → UNMET.** 위 표의 셋째 로봇은 조건이 이름 부르는
  `prbt`가 아니라 `dual_arm_panda`다. 조건대로 prbt를 넣으면 `collision: bool`
  이 **6,854/10,000** 어긋난다. 원인은 편차가 아니라 정확한 접점이다:
  하네스 바닥면의 윗면이 z=0.000000이고 `prbt_base_link`의 충돌 실린더
  아랫면도 z=0.000000이라, FCL이 접촉을 0개 찾아 `-1` 센티널을 그대로
  돌려준다(§218.3). panda·fanuc는 여전히 0/10,000이다.
- **여전히 유효하므로 남긴다:** panda `robot_same_pair_and_value_diverges`
  6,364건이 전부 `floor/panda_link0` 한 쌍이라는 위 괄호. §218.3이
  `robot_same_pair_histogram`의 키가 정확히 하나임을 재도출해 확인했다.
- **Phase 3 둘째 항목은 UNMET 그대로**지만 배수는 정정된다. prbt의 `1.0`은
  거리가 아니라 센티널이므로 배수 목록에서 빠진다 — 남는 것은 panda
  27,384배와 fanuc 2,897배다(§218.4).

따라서 이 트리의 요약은 **UNMET 3개(Phase 3, 4, 9), 부분 UNMET 1개(Phase 2의
셋째 항목), 미측정 1개(Phase 8의 CHOMP/STOMP 항목)**이다.
...
|---|---|---|---|
| `1e-5` | 4906/5000 | 728,695 | 5.413 s |
| `1e-6` | 4897/5000 | 668,855 | 5.017 s |
| `1e-7` | 4915/5000 | 920,881 | 5.492 s |
| `1e-8` | 4904/5000 | 798,879 | 5.123 s |
...
## §226 Phase 3 미충족 두 절의 원인을 확정했다 — `bool`은 상류에 규약이 없고, `distance`는 다른 양이다 (2026-08-06)
...
### §226.1 `collision: bool` 절 (prbt 6,854/10,000) — 상류에 정합할 규약이 없다
...
### §226.2 이 백엔드의 충돌 경계는 0이 아니라 양의 간극 안에 있다 (측정, 미수정)

§226.1을 조사하다 별도로 나온 **포트 쪽** 발견이다. `parry.rs`의 모듈
...
오라클은 `false`(§226.1 표의 `-3e-8` 행)이므로 이것은 그 자체로
발산이다.
...
### §226.3 `distance: f64` 절 (3로봇 전부, 2,897배~27,384배) — 배율이 아니라 다른 양이다
...
**판정: 이 절의 미충족은 포트의 오차가 아니라 상류가 다른 양을 보고하는
것이다.** `doc/upstream-bugs.md`에 `distance-callback-max-contact-depth`로
등재했고(not-reproduced), 재현자는
...
- prbt `bool` 절을 초록으로 만들지 않았다. §226.1의 판정은 "이 픽스처
  위에서 닫히지 않는다"이며, 그 상태는 여전히 **미충족**이다.
- §226.2의 `5e-8 m` 여유를 없애지 않았다. **UNFIXED**이며 사유는 측정으로
  적었다.
- `distance` 절을 초록으로 만들지 않았다. 상류 수치와의 일치는 상류의
  결함을 재현해야만 얻어지고, 그러면 §226.3의 두 불변량 시험이 함께
  깨진다.
