...
> **상태: 충족 (2026-08-05, §216).** 3종 전부에 대해 다섯 항목을 항목별로
...
> 항목별 판정은 `compare_model_info_clauses`다. 수치와 변별력 근거는 §216.
...
> **상태: 미충족 (2026-08-05, §216).** 조건이 명시한 10,000 상태 × 3로봇을
...
> | prbt | **미충족** 6,854/10,000 (68.54%) | 미충족 10,000/10,000 | 1.000000e0 (10,000배) |
...
> 사유가 아니다. 두 절의 원인은 서로 다르고 §216.3/§216.4에 나눠 적었다.
...
## §216 Phase 1 완료 조건을 닫고, Phase 3을 조건이 말한 크기로 처음 실제로 측정했다 (2026-08-05)
...
### §216.2 Phase 1 — 다섯 항목을 항목별로 판정한다, 그리고 충족
...
### §216.3 Phase 3 `collision: bool` 절 — panda·fanuc 충족, prbt만 깨진다
...
### §216.4 Phase 3 `distance: f64` 절 — 5종 전부 미충족, 그리고 원인은 하나가 아니다

`1e-4` 대비 초과 배율은 panda 27,384배, prbt 10,000배, fanuc 2,897배다.
`DistancePairStats`가 두 기전을 따로 세고 있고, 로봇마다 지배적인 쪽이
다르다:
...
- prbt `collision[2]` — 같은 쌍 `floor/prbt_base_link`, 오라클
  −1.0(센티널) vs Rust −2.77555756156288149e-17. §216.3의 접선.
...
로봇별로 어느 기전이 지배적인지가 다르다는 것이 중요하다. 하나의
`failed:` 총계로는 이 구분이 나오지 않아서, `compare_collision`이
