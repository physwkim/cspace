...
## §NEW Phase 7 게이트가 못 보던 것 — 조건별로 세고, 닫을 것은 게이트 안에서 닫았다 (2026-08-06)
...
것을 실제로 깨뜨려 실패를 보이고 되돌린다(§NEW.8). 판별하지 못하는 검사는
검사가 아니다.

### §NEW.1 세 조건이 전부 참인데 포트가 틀릴 수 있는 경로

| # | 조건 | 조건이 참인데 포트가 틀릴 수 있는 방식 | 판정 |
|---|---|---|---|
| 1 | 1 | fanuc이 붕괴해도 게이트는 panda만 보고 통과 — fanuc 판정은 **인쇄만** 되고 `failed`에 들어가지 않았다 | 닫음 §NEW.6 |
| 2 | 1 | `floor_wall`이 무너지고 `cage`가 버티면 stratum pooled가 상쇄한다 | 닫음 §NEW.6 (set별 게이트) |
| 3 | 1 | C++ 기준선이 퇴행하면 문턱 자체(`0.9 × cpp_rate`)가 내려간다 | 닫음 §NEW.7 (`cpp_solved_floor`) |
| 4 | 1 | RNG 스트림 하나에 대한 비교다 (§221은 스트림 차이로 IK 성공이 30건 움직였다) | 측정 후 닫지 않음 §NEW.2 |
| 5 | 2 | 포트 자신의 체커가 판정한다 — 플래너와 같은 방향으로 틀린 체커는 100%를 보고한다 | 닫음 §NEW.3 |
| 6 | 2 | 500개 중 1개만 검사해도 `condition2_checked > 0`은 만족된다 | 닫음 §NEW.6 (`condition2-covers-every-solved-path`) |
| 7 | 2 | densify가 플래너 정점으로 되돌아가면 탐색이 이미 승인한 상태만 다시 읽는다 | 닫음 §NEW.6 (`condition2-densified`) |
| 8 | 2 | 아무 문제도 못 푼 injection 실행이 체커를 보증한다 | 닫음 §NEW.4 |
| 9 | 3 | 각 진영이 **자기** 해결 집합에 대한 median을 쓴다 — 어려운 문제를 놓치면 통과가 쉬워진다 | 닫음 §NEW.6 (paired median) |
| 10 | 3 | pooled median이 robot별 퇴행을 숨긴다 | 닫음 §NEW.5 |
| 11 | 3 | 조건 3은 단측이다 — median이 붕괴하면(경로가 우회를 그만두면) 더 잘 통과한다 | 닫음 §NEW.7 (`ratio_floor`) |
| 12 | 1·2·3 | `solved`는 경로가 돌아왔다는 말이지 **요청한 두 상태 사이의** 경로라는 말이 아니다 | 닫음 §NEW.6 (`endpoints`) |
| 13 | 1·2·3 | 타임아웃이 하나라도 있으면 그 실행은 재현 불가인데 조건 1의 여유가 그것을 흡수한다 | 닫음 §NEW.6 (`no-timeouts`) |
| 14 | 1·3 | §5의 문턱은 큰 퇴행을 통과시킨다 — 48문제 손실, 27% 경로 연장 | 닫음 §NEW.7 (핀) |
| 15 | 2 | 오라클과 포트가 **같은** `objects` 배열에서 장면을 만든다 | 닫지 않음 §NEW.9 |
| 16 | 1·3 | C++ 경로의 끝점은 검사되지 않는다 (포트 쪽만 검사한다) | 닫지 않음 §NEW.9 |

### §NEW.2 조건 1은 스트림 하나에 대한 비교다 — §221의 30건이 여기서도 나오는가
...
마진의 1/50이다. **대신 같은 여유를 핀으로 닫았다**(§NEW.7): 관측 최소값보다
...
### §NEW.4 체커를 한 번도 부르지 않은 injection 실행이 체커를 보증했다
...
같은 자리에 함께 단언해 닫았다. 같은 뮤테이션에서 이제 exit 101이다(§NEW.8 M5).

### §NEW.5 pooled median은 robot별 퇴행을 숨긴다 — fanuc을 따로 둔 판단과 그 규칙

**따로 둔 것은 옳았다. 다만 절반만 옳았다.** fanuc은 panda 숫자에 섞이지 않았지만
(옳음) `failed`에도 들어가지 않았다(틀림, §NEW.6에서 닫음).
...
측정(§NEW.8 M1, fanuc만 퇴행시킨 뮤테이션, 라운드 전 스크립트):
...
같은 목록에 §NEW.1의 나머지를 검사로 넣었다. 조건 2가 참인데 포트가 틀릴 수
있는 방식들:
...
그래서 실측값 아래에 핀을 깔았다. 값은 고르지 않고 §NEW.2의 스트림 간 폭에서
...
줄여 통과할 수 없다(§NEW.8이 그 재정의로 실행된다).
...
### §NEW.10 실측 — 게이트가 지금 무엇을 재고 얼마가 드는가
...
38초가 §NEW.3–§NEW.6의 검사를 라운드마다 실제로 실행되게 만드는 값이다.
`PILOT_COUNT=2`로는 2m07s이며, 그 실행은 `pin-population` 때문에 **통과하지
않는다**(§NEW.7) — 축소된 실행이 커버리지로 읽히는 일이 구조적으로 막혀 있다.

## §NEW2 Phase 8의 "Phase 7과 같은 속성 기반 검증"을 항목별로 셌다 — 옮겨지는 것만 계측기에 넣었다 (2026-08-06)
...
§NEW2.6과 §NEW2.7이 그 답의 근거다.

### §NEW2.1 기준선: Phase 7 게이트가 실제로 세는 20종 / 39개
...
|---|---|---|---|
| 1 | `condition1` (성공률 ≥ 0.9 × C++ OMPL) | **없다** | §NEW2.3 — `no-regression-solved`(이 트리에서 실측한 하한)로 대체 |
...
| 4 | `condition2-densified` | 있다, **Phase 7보다 더** | `validity-densified` — 그대로 구현(§NEW2.5) |
| 5 | `endpoints` (요청한 start/goal에서 정확히 0) | CHOMP은 그대로, **STOMP은 아니다** | CHOMP `endpoints` 상한 0.0, STOMP은 실측 상한 핀(§NEW2.4) |
| 6 | `cpp-endpoints` | **없다** | C++ 기준선이 없다(§NEW2.3) |
| 7 | `no-timeouts` | 있다, 그대로 | `no-timeouts` — 그대로 구현. STOMP은 이 모집단에서 **실패한다**(§NEW2.7) |
| 8 | `condition3-pooled` (중앙 경로길이 ≤ 1.3 × C++) | **없다** | §NEW2.3 |
| 9 | `condition3-paired` | **없다**, 그러나 *짝지음*이라는 성질은 옮겨진다 | CHOMP `length-ratio-band`, STOMP `cost-fn-sees-seed-collisions`. STOMP의 첫 설계였던 `paired-cost-not-worse`는 반증 불가능해서 지웠다(§NEW2.10) |
...
| 16 | injection 변별 게이트 | collision은 그대로, constraint는 **모집단을 바꿔야 성립한다** | 구현(§NEW2.6) |
| 17 | feasibility accounting(양쪽 다 못 푼 문제 재시도) | 있다, 형태를 바꿔서 | `nontrivial-population` — 씨앗이 이미 유효했던 문제는 최적화기를 재지 않는다(§NEW2.7) |
| 18 | (Phase 7에 없음) CHOMP `mesh_to_mesh_collision_free` | — | `mesh-check-exercised`(§NEW2.8) |
| 19 | (Phase 7에 없음) 핀이 측정되지 않은 모드 | — | `pins-unmeasured` — 핀 없는 모드는 조용히 체크를 줄이지 않고 실패한다 |

### §NEW2.3 C++ 기준선을 요구하는 네 항목 — 왜 없고, 없앨 수 있나
...
그 말을 그대로 한다 — 아래 §NEW2.13의 실행에서 CHOMP은 panda 16문제 중 9,
...
이번 라운드에서는 만들지 않았다 — §NEW2.12에 남긴다.
...
| constrained | 606 → 2065 (3.4배) | 0 → 0 (해결 0건, §NEW2.6) |
...
### §NEW2.6 실측: 경로 제약이 걸린 문제를 STOMP은 어떤 시간을 줘도 풀지 못한다
...
8/8에서 0/8로 뒤집힌다. 그리고 이 표는 §NEW2.6 앞머리의 주장도 같이 증명한다:
...
### §NEW2.7 실측: 이 모집단에서 풀리는 문제는 최적화기가 필요 없던 문제다
...
실패하거나 시한을 넘긴다). §NEW2.13 실행의 실측이 그 갈림을 그대로 보여준다:
...
### §NEW2.8 CHOMP의 `mesh_to_mesh_collision_free`를 처음으로 실제로 공급한다
...
### §NEW2.9 계측기 구조 — 그리고 왜 `verify-`가 아니라 `measure-`인가
...
없다(§NEW2.7). 실패는 보고할 값이지 남의 라운드를 빨갛게 만들 값이 아니다.

핀은 `PINS_ALL` 한 곳에만 있고, 전부 §NEW2.13의 실행에서 측정한 값이다.
...
### §NEW2.10 계측기 자신의 결함 두 개 — 모집단을 8문제로 키우자 드러났다
...
### §NEW2.11 변별 변이 — 각 체크가 무엇을 보고 있는지 깨서 확인했다

체크가 통과한다는 것은 그 체크가 무언가를 보고 있다는 뜻이 아니다(§NEW2.10이
...
- **씨앗이 주어진 문제 모집단.** §NEW2.7이 이 라운드의 가장 큰 미결이다. Phase
...
- **C++ CHOMP 기준선.** 오라클은 이미 `chomp_motion_planner`를 링크하므로
  `chomp_plan` op는 만들 수 있다(§NEW2.3). 만들면 `condition1`의
  ...
  않았다.
- **`full` 모드의 핀.** 한 번도 돌리지 않았다(§NEW2.9). 250문제 × 2로봇 ×
...
- **STOMP의 끝점 밀림 자체.** §NEW2.4는 상한을 핀으로 걸었을 뿐이고, 밀림을
  없애지 않았다. upstream 동작이므로 포트가 임의로 다시 고정하면 안 된다.
- **제약 비용이 위반량이 아니라 거리라는 것.** §NEW2.6의 원인도 upstream
...
| 실패 | 근거 절 |
|---|---|
| `chomp/fanuc/nontrivial-population` (0 of 11) | §NEW2.7 — 모집단 |
| `stomp/fanuc/nontrivial-population` (0 of 11) | §NEW2.7 — 모집단 |
| `stomp/{panda,fanuc}/no-timeouts` + per-set 4개 | §NEW2.7 — 모집단 |
| `stomp/constrained/{validity, …-covers…, …-densified, no-timeouts}` | §NEW2.6 — 제약 비용이 거리 |
...
(§NEW2.8), 후자는 120초 경계에 걸친 문제 하나다. 그래서 이 두 양의 핀은 등식이
아니라 하한이고(`mesh_calls_floor`), 해결 수 하한은 실측보다 2 낮다.

**변이 비용.** §NEW2.11의 변이 7개는 `PLANNERS=chomp`(6개, 각 ~140초)와
