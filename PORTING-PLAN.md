...
## §NEW `attached_body.cpp`의 마지막 갭 둘 — `setScale`/`setPadding`을 옮겼고, `Arc::make_mut` 등가는 **강한** 공유에서만 성립한다 (2026-08-06)
...
### §NEW.1 상류가 하는 일 (`attached_body.cpp:86-103`, `:120-137`)
...
### §NEW.2 `Arc::make_mut`는 `use_count() == 1`이 아니다 — 약한 참조에서 갈린다
...
### §NEW.3 `void`가 아니라 `Result<()>`이고, 적용은 원자적이지 않다
...
- `PlanningScene`에 `attached_body_mut` 류의 접근자를 만들지 않았다.
  과제 범위가 `attached_body.rs`였고, §NEW.1대로 상류에도 호출자가 없다.
- `attached_body.rs`의 헤더를 `Ported from`으로 바꾸지 않았다. 이 파일의
  ...
  세고, 행은 표에 남는다.
- `geometric_shapes`의 예외 거동을 확인하지 않았다. §NEW.3의 오류 규약은
  이 포트 쪽 사실만으로 적혀 있다.
