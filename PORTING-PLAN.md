...
## §NEW `/move_action`을 게이트에 올린다 — 상류 C++ `MoveGroupInterface`를 매 실행마다 다시 돌린다 (2026-08-06)
...
### §NEW.1 발견 — 액션 서버를 들여온 그 트리에서 게이트는 초록이었다
...
### §NEW.4 오라클 이미지가 없을 때 — 조용히 통과하지 않는다
