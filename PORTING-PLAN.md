...
| `pad_environment_collisions` | **포팅 안 함 (§NEW)** | **포팅 안 함 (§NEW)** | 필드가 없다 (`b5cced7`) | 백엔드에는 없다. `planning_scene.cpp:442`가 유일한 독자이고, 필드를 백엔드에 넘기는 대신 패딩된/안 된 두 `CollisionEnv` 인스턴스 중 하나를 고른다 |
| `pad_self_collisions` | **포팅 안 함 (§NEW)** | **포팅 안 함 (§NEW)** | 필드가 없다 (`b5cced7`) | 같은 방식, `planning_scene.cpp:453`·`:558`. 상류 트리 전체에 **대입하는 곳이 0**이므로 실효값은 기본 `false` 하나다 |
...
**§NEW가 이 문단을 대체한다.** "환경 둘을 받는 API 변경"이라는 전제가
...
연다. `pad_*` 두 행의 만료 조건은 §NEW.4가 다시 쓴다.

## §NEW `pad_environment_collisions`/`pad_self_collisions` — 상류의 두 환경 차이는 패딩뿐이고, D4의 `E` 하나가 그것을 낸다 (2026-08-06)
...
### §NEW.1 앵커 넷을 전수하면 코퍼스 안 호출자는 0이다
...
### §NEW.2 `move_group` 경로에서 닿는 것은 `isRemainingPathValid` 하나다
...
### §NEW.3 두 환경은 패딩 말고 다른 것이 없다 — 그래서 대안은 "필드"가 아니라 "clone"이다
...
(b)를 택했다(`b5cced7`). 근거는 취향이 아니라 §NEW.1~.3이다: 이 둘은
...
고정했다. 상류 실효 규칙은 §NEW.1이 잰 대로 "자기 충돌은 언제나 패딩
...
때다. 그때의 수정은 그 호출자가 §NEW.3의 미패딩 `clone`을 자기 손으로
