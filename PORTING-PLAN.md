...
## §NEW `tools/ci` 게이트가 못 보는 경계를 뒤진다 — `verify-all.sh` 전체 실행과 32개 게이트 커버리지 표
...
§NEW.1/§NEW.3에 전부 적었다 — collision/distance-field는 이 패널의 펜스
...
`doc/claim-audit/tools-ci-gates.md` §NEW.2 표에 적었다. 가장 값비싼 갭으로
고른 것을 게이트로 만들어 커밋했다 (§NEW.4): `crates/*/tests/*.rs`의
...
드러나 소스에서 고쳤다, 뮤테이션 판별 통과) — 전체 경위는 §NEW.4.

찾았지만 이번 라운드에 게이트로 만들지 않은 것도 하나 있다 (§NEW.5):
