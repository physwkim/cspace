...
| 조각 | §226.3 | 지금 | 지은 절 |
|---|---|---|---|
| 노드 바이너리(`fn main`/`r2r::Node`/`spin`) | 부재 | **존재** — `ros/moveit-ros/src/bin/move_group.rs:212,269,359` (§NEW.2 이전 이름은 `plan_kinematic_path_server.rs`, 줄 번호는 지금 트리에서 다시 유도했다) | §241 |
...
고친 것은 `ros/moveit-ros/src/bin/move_group.rs`(§NEW.2가 이름을 바꾸기
...
### §NEW.2 `plan_kinematic_path_server` → `move_group` — 상류가 이 노드에 붙인 이름
...
다시 유도했다)과 §NEW.1 자신. `[[bin]]` 테이블은 없다 — Cargo가
