...
//! (`crates/cspace-scene/src/scene.rs:613`,`:671`), not on `RobotState`/
...
/// (`crates/cspace-scene`'s `AttachedBody` module doc), so the equivalent
/// lookup lives there instead — `PlanningScene::frame_transform`'s private
/// `attached_frame` helper (`crates/cspace-scene/src/scene.rs:545`). Taking
