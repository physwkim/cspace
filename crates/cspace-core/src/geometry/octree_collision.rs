...
//! ([`crate::octomap::Leaf::is_occupied`]), the same predicate FCL's own
...
/// [`crate::geometry::shapes::OcTree::octree`] already uses for "no tree at all" —
/// rather than a panic.
pub fn compound_from_octree(tree: &OcTree) -> Option<Compound> {
    let leaf_shapes: Vec<(Isometry3, SharedShape)> = tree
        .leaves()
        .filter(crate::octomap::Leaf::is_occupied)
        .map(|leaf| {
