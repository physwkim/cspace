...
use crate::model::joint::{JointKind, JointType};
use crate::state::Posed;
...
/// [`crate::state::Posed::jacobian`]'s columns come back in, since both this
/// type and that method read it off the same
/// [`crate::model::JointModelGroup::variable_names`]. Call this the *full*
...
/// mimic joint's value is derived. This port's [`crate::state::RobotState`]
/// already derives a mimic joint's position from its master whenever the
/// master is set ([`crate::state::RobotState::set_variable_position`]), so
...
/// ([`crate::kinematics::cart_to_jnt`]) ever sees full-space values, exactly as
...
pub(crate) struct ChainInfo {
    ...
    /// `crate::state::RobotState`'s own `parent_link_of_joint`.
    pub(crate) root_link_index: Option<usize>,
    /// `KinematicsBase::base_frame_`: [`ChainInfo::root_link_index`]'s link
    /// name, or [`crate::model::RobotModel::root_link_name`] when that index
    /// is `None`. The frame every [`crate::kinematics::KinematicsSolver::solve`] target
    ...
    /// [`crate::state::Posed::jacobian`] directly.
    pub(crate) link_index: Vec<usize>,
    /// This full-space entry's index into
    /// [`crate::model::RobotModel::variable_names`] — where
    /// [`crate::kinematics::cart_to_jnt`] writes it back into a whole-model positions
    /// buffer for [`crate::state::RobotState::set_variable_positions`].
    ...
    /// [`crate::kinematics::cart_to_jnt::search_position_ik`] attempts — a mimic
    ...
    /// ([`crate::model::joint::RevoluteJoint::is_continuous`]). Copied here
...
impl ChainInfo {
    ...
    /// ([`crate::model::JointModelGroup::is_chain`]), if it contains a
    ...
    pub(crate) fn build(model: &RobotModel, group_name: &str) -> Result<Self> {
        ...
        // `crate::state::RobotState::parent_link_of_joint` does with its
    ...
    /// [`crate::kinematics::KinematicsSolver::solve`] target pose is given in. Identity
    ...
    /// to weight `1.0`, then [`crate::kinematics::SolverParams::joint_weights`]
    /// overrides by joint name.
    ///
    /// # Errors
    ///
    /// [`Error::Other`] if [`crate::kinematics::SolverParams::joint_weights`] gives one
    ...
    /// of a small positive one: [`crate::kinematics::velocity::solve_velocity`]'s weighted
    ...
    ///
    /// [`SolverParams::joint_weights`]: crate::kinematics::params::SolverParams::joint_weights
    pub(crate) fn resolve_joint_weights(
        &self,
        params: &crate::kinematics::params::SolverParams,
    ) -> Result<Vec<f64>> {
    ...
    /// [`crate::kinematics::KinematicsSolver::joint_names`]'s return value: the public,
    ...
    /// [`crate::state::Posed::jacobian`] here would silently drop every
    ...
    /// [`crate::state::Posed::global_link_transform_at`] (a public,
    /// already-fresh transform read) rather than
    /// [`crate::state::Posed::jacobian`]'s active-only loop.
...
"#;
    ...
    /// `crate::state::Posed::jacobian`'s own boundary.
    #[test]
...
"#;
    ...
    /// [`crate::kinematics::SolverParams::joint_weights`] overrides by name and only by
...
"#;
        ...
        let mut params = crate::kinematics::params::SolverParams::default();
