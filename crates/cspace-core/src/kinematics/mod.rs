...
//! KDL dependency (D1/D2): both come from [`crate::state::Posed`] —
//! [`crate::state::Posed::jacobian`] and
//! [`crate::state::Posed::global_link_transform`] — which already
//! encapsulate the chain-validity checks
//! (`crate::model::JointModelGroup::is_chain`) this crate would otherwise
//! have to re-derive.
//!
...
//! are [`crate::geometry::Isometry3`]; errors that mean "this solver cannot
//! be built for this group" go through [`crate::error::Error`]; failure to
...
//! - `getPositionFK` — **not in this crate**. `crate::state::Posed`
...
//!   symbol exists anywhere under `crates/cspace-core/src/kinematics/`.
...
//!   `crate::state::Posed`'s forward-kinematics calls take a link name
...
//!     here (position/orientation go through [`crate::geometry::Isometry3`]
//!     directly)
...
//!   [`crate::kinematics::registry::KinematicsSolver`]'s own documented deviation 1
