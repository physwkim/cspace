...
///    [`crate::kinematics::resolve_ik_queries`]; it defaults to the single
...
pub trait KinematicsSolver {
    ...
    /// in the order [`crate::kinematics::resolve_ik_queries`] fills them.
    ///
    ...
    /// [`fn@crate::kinematics::set_from_ik`] calls it once per request, not per attempt.
    ...
    /// `seed` can fail in the sense [`crate::error::Error`] means.
    ///
    /// # Panics
    ///
...
pub struct SolverRegistration {
    ...
    pub name: &'static str,
    /// Build one instance for `(model, group)`, or an [`crate::error::Error`]
...
/// `KDL::ChainIkSolverVelMimicSVD` — [`crate::kinematics::NewtonRaphsonSolver`]'s own
...
/// [`crate::kinematics::CachedIkSolver`]'s doc comment for why a caching solver is never
