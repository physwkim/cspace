...
//!
//! 9 audit bullets below (verify: `rg -c '^//! - \`' crates/cspace-core/src/metrics/lib.rs`
...
//!   `&str` group names, matching [`crate::state::Posed::jacobian`]'s own
...
//! a chain" alike) becomes [`crate::error::Result`] with the two causes
//! distinguished ([`crate::error::Error::UnknownName`] vs.
//! [`crate::error::Error::Other`]), matching
//! [`crate::state::Posed::jacobian`]'s own error shape (see its doc
...
//!   variables, so [`crate::model::joint::JointModel::distance`] returns
...
//!   confirmed via [`crate::model::JointModelGroup::joint_indices`], so
...
impl<'m> KinematicsMetrics<'m> {
    ...
    /// [`crate::state::Posed::jacobian`], which this delegates to).
...
mod tests {
    ...
    /// Falsification testing (see `crates/cspace-core`'s round-12 commit
    ...
    /// non-chain cases and returns the same [`Error`](crate::error::Error) variants (see
    /// `crate::state::Posed::jacobian`). Measured, not assumed: replacing
