...
//! that split:
//!
//! - **`X[i]` (`Segment::pose(q)`) is exactly [`crate::model::LinkModel::joint_origin_transform`]
//!   composed with [`crate::model::joint::JointModel::compute_transform`].** Both describe the
...
//!   `X[i]` is already the formula [`crate::state::robot_state`]'s forward kinematics
...
//!   [`crate::state::robot_state`]'s Jacobian relies on.
//!
...
//! [`crate::model::LinkModel::mass`]/[`crate::model::LinkModel::center_of_mass`]/[`crate::model::LinkModel::inertia`]
...
//! port's own [`crate::model::joint::VariableBounds`] deliberately doesn't
...
fn rigid_body_inertia_from_link(link: &crate::model::LinkModel) -> RigidBodyInertia {
    let m = link.mass();
...
impl<'m> DynamicsSolver<'m> {
        ...
        let active_max_torques = group
            ...
            .filter_map(|(&joint_index, &max_torque)| {
                let is_active = model.joint_model_at(joint_index).joint_type()
                    != crate::model::joint::JointType::Fixed;
    ...
    fn sweep(
        ...
        for k in 0..ns {
            ...
            let is_active = joint.joint_type() != crate::model::joint::JointType::Fixed;
        ...
        for k in (0..ns).rev() {
            let joint = self.model.joint_model_at(joint_indices[k]);
            if joint.joint_type() != crate::model::joint::JointType::Fixed {
                j -= 1;
