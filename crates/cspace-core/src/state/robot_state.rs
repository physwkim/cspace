...
use crate::geometry::{Isometry3, Vector3};
use crate::model::joint::{JointKind, JointModel, PlanarJoint};
use crate::model::{JointModelGroup, RobotModel};
use rand::{Rng, RngExt};
use rand_distr::StandardNormal;

use crate::state::numeric::{cxx_max, cxx_min};

/// An index into [`RobotModel::joint_models`](crate::model::RobotModel::joint_models).
///
...
///   lockstep, and this port's [`crate::model::LinkModel`] carries no
...
///    [`crate::model::RobotModel`] deliberately does not build that table
...
impl<'m> RobotState<'m> {
    ...
    /// [`RobotModel::variable_count`](crate::model::RobotModel::variable_count).
    ...
    /// [`JointModel::interpolate`](crate::model::joint::JointModel::interpolate)
...
impl<'s, 'm> Posed<'s, 'm> {
    ...
    /// link in [`RobotModel::link_models`](crate::model::RobotModel::link_models)
    /// order).
    ...
    ///    [`crate::model::JointModelGroup::is_chain`]'s own doc comment), so
...
/// port's [`crate::model::RobotModel`] deliberately excludes it (Phase 1
...
///   infinite `min`/`max` — see [`crate::model::joint::FloatingJoint`]'s
...
///   matching `crates/cspace-core/src/kinematics/cart_to_jnt.rs`'s
...
fn sample_random_positions_near_by(
    ...
    match joint.kind() {
        ...
        JointKind::Planar(p) => {
            ...
            // assignment never runs) — see `crate::state::numeric`.
...
// matching how `crate::model::joint::{RevoluteJoint, PlanarJoint,
