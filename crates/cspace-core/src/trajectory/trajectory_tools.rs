...
//! beyond a call each into [`crate::trajectory::time_optimal_trajectory_generation::compute_time_stamps`]/
//! [`crate::trajectory::ruckig_smoothing::apply_smoothing`] — see each function's doc
...
//! [`crate::trajectory::time_optimal_trajectory_generation`]'s now-closed "Known gap"
//! section describes.
//!
//! # Out of scope
//!
...
//! [`crate::trajectory::time_optimal_trajectory_generation`]'s "Out of scope" section
...
use crate::trajectory::ruckig_smoothing::{self, SmoothingOptions};
use crate::trajectory::time_optimal_trajectory_generation::{self, TotgOptions};
...
/// # Errors
///
/// [`crate::error::Error`] if `resample_dt` is not finite or not positive (see
...
mod tests {
    ...
    /// same read ([`crate::model::joint::JointModel::variable_bounds_msg`])
    /// / mutate / write
    /// ([`crate::model::joint::JointModel::set_variable_bounds_from_limits`])
    ...
    fn apply_totg_time_parameterization_with_upstream_defaults_forwards_to_compute_time_stamps() {
        ...
        apply_totg_time_parameterization(
            ...
            1.0,
            1.0,
            crate::trajectory::DEFAULT_PATH_TOLERANCE,
            0.1,
            0.001,
        )
