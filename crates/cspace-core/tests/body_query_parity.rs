...
//! Parity test for [`cspace_core::geometry::bodies::Body`]'s posed algorithms
    ...
    for (request, response) in requests.iter().zip(&responses) {
        ...
        let shape = match &request.shape {
            ShapeSpec::Sphere { radius } => {
                Shape::Sphere(cspace_core::geometry::shapes::Sphere { radius: *radius })
            }
            ShapeSpec::Cylinder { radius, length } => {
                Shape::Cylinder(cspace_core::geometry::shapes::Cylinder {
                    radius: *radius,
                    length: *length,
                })
            }
            ShapeSpec::Box { size } => {
                Shape::Cuboid(cspace_core::geometry::shapes::Cuboid { size: *size })
            }
        };
