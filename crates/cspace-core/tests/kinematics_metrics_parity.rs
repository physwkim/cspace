...
//! joint values through this port's [`cspace_core::state::RobotState`].
//!
//! Regenerate the response fixture with:
//! ```text
//! R=/home/stevek/work/moveit-rs/.caucus/worktrees/5REEQZSC40-p1-fixtures-920dace3-1
//! python3 -c "import json; print(json.dumps(json.load(open('$R/crates/cspace-core/tests/fixtures/metrics/metrics/panda_kinematics_metrics_request.json'))))" \
...
//! `crates/cspace-core/tests/fixtures/metrics/metrics/oracle-models.json`'s
...
//! `crates/cspace-core/tests/fixtures/metrics/metrics/panda.srdf` (see that file's
...
//! ```text
//! R=/home/stevek/work/moveit-rs/.caucus/worktrees/5REEQZSC40-p1-fixtures-920dace3-1
//! python3 -c "import json; print(json.dumps(json.load(open('$R/crates/cspace-core/tests/fixtures/metrics/metrics/panda_arm_5dof_kinematics_metrics_request.json'))))" \
//!   | sg docker -c "$R/tools/moveit-oracle/run-oracle.sh --urdf $R/fixtures/panda.urdf --srdf $R/crates/cspace-core/tests/fixtures/metrics/metrics/panda.srdf" \
//!   | python3 -c "import json,sys; print(json.dumps(json.load(sys.stdin), indent=2, sort_keys=True))"
//! ```
...
use cspace_core::metrics::KinematicsMetrics;
...
fn fixture_path(file_name: &str) -> String {
    format!(
        concat!(env!("CARGO_MANIFEST_DIR"), "/tests/fixtures/metrics/{}"),
        file_name
    )
}
...
fn build_model_with_5dof_group() -> RobotModel {
    ...
    let srdf_path = concat!(
        ...
        "/tests/fixtures/metrics/panda.srdf"
    );
    let urdf_xml =
        fs::read_to_string(urdf_path).unwrap_or_else(|e| panic!("read {urdf_path}: {e}"));
    let urdf = urdf_rs::read_file(urdf_path).expect("fixture URDF must parse");
