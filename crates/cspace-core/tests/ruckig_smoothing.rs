...
// (`robot_state/src/robot_state.cpp:222-231`) don't zero storage either — they clear the
// `has_velocity_`/`has_acceleration_` presence flags, which a freshly
// constructed `RobotState` already starts with `false`
// (`robot_state/src/robot_state.cpp:68-69`; see `cspace_core::state`'s `RobotState::new`, which
