...
//!    [`crate::Transforms::new`]) instead of leaving a `Vec` index that can
...
//!    [`Error::Construct`].
//! 5. **The `OcTree` payload is `Arc<cspace_octomap::OcTree>`, not
...
//!    `Arc<cspace_octomap::OcTree>` reproduces both facts at once: cloning a
...
//!   pruned, variable-depth [`cspace_octomap::OcTree`] leaf directly, for the
...
//!   all. The interface it needs is exactly what [`cspace_octomap::OcTree`]
//!   already exposes for this purpose: [`cspace_octomap::OcTree::leaves`]
//!   filtered by [`cspace_octomap::Leaf::is_occupied`], reading
//!   [`cspace_octomap::Leaf::coordinate`] (and, if a decomposition wants to
//!   record cell size, [`cspace_octomap::Leaf::size`]) per occupied leaf.
...
//!   `crates/cspace-geometry/tests/octree_in_world_parity.rs`, using only
//!   this crate's own [`OcTree`]/[`crate::Isometry3`] and
//!   [`cspace_octomap::OcTree`]'s existing point-query API — nothing further
...
//!   this.
//! - **`cspace_octomap::OcTree`'s raw leaf payload → `cspace-distance-field`,
...
//!   constructor over [`cspace_octomap::OcTree::leaves`] filtered by
//!   [`cspace_octomap::Leaf::is_occupied`] (see "Who consumes
...
//!   duplicate.
//! - **[`crate::bodies::Body::intersects_ray`] → stays in `cspace-geometry`,
...
/// # Deviations from upstream
///
/// - **`octree` is `Option<Arc<cspace_octomap::OcTree>>`, not a raw
...
pub struct OcTree {
    ...
    pub octree: Option<Arc<cspace_octomap::OcTree>>,
}
...
impl OcTree {
    ...
    pub const fn from_tree(tree: Arc<cspace_octomap::OcTree>) -> Self {
