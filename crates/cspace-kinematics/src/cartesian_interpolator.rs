...
//! `RobotModel` loads `kinematics.yaml` and `cspace_model::JointModelGroup`
...
//! `cspace_model::RobotModel::rigidly_connected_parent_link`); an attached
...
//! timeout parameter at all ([`crate::SolverParams::max_restarts`] replaced it
...
//!   [`cspace_geometry::Isometry3`] is `nalgebra::Isometry3`, a translation
...
use cspace_geometry::{Isometry3, Vector3, quaternion};
use cspace_model::joint::{JointModel, JointType};
...
impl<'a> CartesianInterpolator<'a> {
    ...
    /// # Errors
    ///
    /// [`cspace_error::Error::UnknownName`] if
...
fn interpolate_pose(from: &Isometry3, to: &Isometry3, t: f64) -> Isometry3 {
    // `cspace_geometry::quaternion::slerp` rather than nalgebra's, which is a
...
fn enforce_continuous_joint_bounds(state: &mut RobotState<'_>, group: &JointModelGroup) {
    ...
    let continuous: Vec<&JointModel> = group
        ...
        .filter(|joint| {
            joint
                .as_revolute()
                .is_some_and(cspace_model::joint::RevoluteJoint::is_continuous)
        })
        .collect();
...
mod tests {
    use super::*;
    use cspace_geometry::UnitQuaternion;
