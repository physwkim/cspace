...
use cspace_model::joint::{JointKind, JointType};
...
/// [`cspace_state::Posed::jacobian`]'s columns come back in, since both this
/// type and that method read it off the same
/// [`cspace_model::JointModelGroup::variable_names`]. Call this the *full*
...
/// mimic joint's value is derived. This port's [`cspace_state::RobotState`]
/// already derives a mimic joint's position from its master whenever the
/// master is set ([`cspace_state::RobotState::set_variable_position`]), so
...
pub(crate) struct ChainInfo {
    ...
    /// `cspace_state::RobotState`'s own `parent_link_of_joint`.
    pub(crate) root_link_index: Option<usize>,
    /// `KinematicsBase::base_frame_`: [`ChainInfo::root_link_index`]'s link
    /// name, or [`cspace_model::RobotModel::root_link_name`] when that index
    ...
    /// [`cspace_state::Posed::jacobian`] directly.
    pub(crate) link_index: Vec<usize>,
    /// This full-space entry's index into
    /// [`cspace_model::RobotModel::variable_names`] — where
    /// [`crate::cart_to_jnt`] writes it back into a whole-model positions
    /// buffer for [`cspace_state::RobotState::set_variable_positions`].
    ...
    /// ([`cspace_model::joint::RevoluteJoint::is_continuous`]). Copied here
...
impl ChainInfo {
    ...
    /// ([`cspace_model::JointModelGroup::is_chain`]), if it contains a
    ...
    pub(crate) fn build(model: &RobotModel, group_name: &str) -> Result<Self> {
        ...
        // `cspace_state::RobotState::parent_link_of_joint` does with its
    ...
    /// [`cspace_state::Posed::jacobian`] here would silently drop every
    ...
    /// [`cspace_state::Posed::global_link_transform_at`] (a public,
    /// already-fresh transform read) rather than
    /// [`cspace_state::Posed::jacobian`]'s active-only loop.
...
"#;
    ...
    /// `cspace_state::Posed::jacobian`'s own boundary.
    #[test]
