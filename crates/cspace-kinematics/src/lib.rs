...
//! KDL dependency (D1/D2): both come from [`cspace_state::Posed`] —
//! [`cspace_state::Posed::jacobian`] and
//! [`cspace_state::Posed::global_link_transform`] — which already
//! encapsulate the chain-validity checks
//! (`cspace_model::JointModelGroup::is_chain`) this crate would otherwise
//! have to re-derive.
//!
...
//! are [`cspace_geometry::Isometry3`]; errors that mean "this solver cannot
//! be built for this group" go through [`cspace_error::Error`]; failure to
...
//! - `getPositionFK` — **not in this crate**. `cspace_state::Posed`
...
//!   symbol exists anywhere under `crates/cspace-kinematics/src/`.
...
//!   `cspace_state::Posed`'s forward-kinematics calls take a link name
...
//!     here (position/orientation go through [`cspace_geometry::Isometry3`]
//!     directly)
