...
/// ([`crate::NewtonRaphsonSolver`] ports it directly). Upstream has no
...
impl LevenbergMarquardtSolver {
    /// See [`crate::NewtonRaphsonSolver::new`], including its `# Deviation`
    ...
    /// See [`crate::NewtonRaphsonSolver::new_with_seed`].
    ///
    /// # Errors
    ///
    /// `SolverParams::validate`, or see `ChainInfo::build`'s `# Errors`.
