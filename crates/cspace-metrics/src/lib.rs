...
//!
//! 9 audit bullets below (verify: `rg -c '^//! - \`' crates/cspace-metrics/src/lib.rs`
...
//!   `&str` group names, matching [`cspace_state::Posed::jacobian`]'s own
...
//! a chain" alike) becomes [`cspace_error::Result`] with the two causes
//! distinguished ([`cspace_error::Error::UnknownName`] vs.
//! [`cspace_error::Error::Other`]), matching
//! [`cspace_state::Posed::jacobian`]'s own error shape (see its doc
...
//!   variables, so [`cspace_model::joint::JointModel::distance`] returns
...
//!   confirmed via [`cspace_model::JointModelGroup::joint_indices`], so
...
impl<'m> KinematicsMetrics<'m> {
    ...
    /// [`cspace_state::Posed::jacobian`], which this delegates to).
...
mod tests {
    ...
    fn range_at_the_min_positive_boundary_still_skips() {
        ...
        joint
            .set_variable_bounds(
                "panda_joint7",
                cspace_model::joint::VariableBounds {
                    min_position: 0.0,
    ...
    /// Falsification testing (see `crates/cspace-metrics`'s round-12 commit
    ...
    /// non-chain cases and returns the same [`Error`](cspace_error::Error) variants (see
    /// `cspace_state::Posed::jacobian`). Measured, not assumed: replacing
