...
//! `PlanningRequest`/`PlanningResponse` — into `cspace-planning`, so the
//! symbol-by-symbol accounting for that header lives in
//! `cspace_planning::planner`'s module doc, next to the traits it is about.
...
//!   [`cspace_planning::PlannerManager`], whose upstream counterpart is
...
//!   `cspace_planning::planner`'s own module doc, not duplicated here:
...
//!   [`cspace_planning::PlannerConfigurationMap`], which this crate takes as
...
//!   per query through [`cspace_planning::configuration_for`], and today it
