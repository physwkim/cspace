...
//! This needs no port: round 24's [`cspace_stomp_core::CancelHandle::new`]/
...
//! `cspace-planning/src/request.rs`'s `MotionPlanRequest` field audit
...
//! at which a caller could obtain a [`cspace_stomp_core::CancelHandle`] --
...
//! not a special-cased seam: [`cspace_stomp_core::CancelHandle::new`] (new
//! this round) lets a caller build a handle *before* any `Stomp` exists,
//! and [`cspace_stomp_core::Stomp::with_cancel_handle`] (new this round,
...
///
/// # Deviation from upstream: no `cspace-planning` dependency
///
...
/// (`cspace-planning/src/request.rs:235`, already exactly this type)
/// directly.
///
...
/// `MotionPlanRequest`-level concern this function has no `cspace-planning`
...
mod tests {
    ...
    /// (`cspace-stomp-core::stomp`) `num_iterations_after_valid` mask.
    ...
    /// `cspace-stomp-core::stomp::tests::cancelling_before_solve_stops_before_num_iterations_completes`,
