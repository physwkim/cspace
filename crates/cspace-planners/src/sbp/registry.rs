...
// `MotionPlanRequest`, `MotionPlanResponse`) lives in `cspace-planning`,
...
//! only *names* with `cspace-planning`'s, which meant the workspace's
//! planning pipeline could not call the workspace's only planner at all:
//! `cspace_planning::pipeline::generate_plan` wanted
//! `cspace_planning::PlanningRequest` and this crate accepted something
//! else with the same spelling. All four names now resolve to
//! `cspace-planning`'s single set, and this crate depends on that crate.
//!
...
//! sampler. `cspace_planning::PlanningRequest::goal_constraints` is that,
...
//! `detail/goal_union.cpp:83-95`). [`cspace_planning::PlanningContext::solve`] reproduces
...
/// [`cspace_planning::PlanningContext`] from this crate. Boxed into
/// [`cspace_planning::PlanError`] at the trait boundary, which is
/// deliberately opaque so `cspace-planning` need not depend on any concrete
/// planner crate.
...
pub enum PlanError {
    ...
    #[error(transparent)]
    Sbp(#[from] SbpError),
    /// [`cspace_planning::PlanningContext::solve`] ran but did not find a path.
...
///
/// [`cspace_planning::PlanningRequest`] is a port of upstream's
...
pub struct RrtConnectManager {
    ...
    ///
    /// [`cspace_planning::PlannerManager::get_planning_context`] takes
    ...
    /// [`cspace_planning::configuration_for`] — not once at construction —
...
impl RrtConnectManager {
    ...
    fn params_for(&self, request: &PlanningRequest) -> RrtConnectParams {
        ...
        let settings = cspace_planning::configuration_for(
            &self.configurations,
            &request.group_name,
            &request.planner_id,
        );
...
/// The one [`cspace_planning::PlannerConfigurationSettings::config`] key this manager reads:
...
impl PlannerManager for RrtConnectManager {
    ...
    ) -> Result<Box<dyn PlanningContext<'m> + 'a>, cspace_planning::PlanError> {
        let space = JointModelGroupSpace::new(scene.robot_model(), &request.group_name)
            .map_err(|err| Box::new(PlanError::Sbp(err)) as cspace_planning::PlanError)?;
...
impl<'m> PlanningContext<'m> for RrtConnectContext<'_, 'm> {
    fn solve(&mut self) -> Result<PlanningResponse<'m>, cspace_planning::PlanError> {
        self.solve_inner()
            .map_err(|err| Box::new(err) as cspace_planning::PlanError)
    }
}
...
impl<'m> RrtConnectContext<'_, 'm> {
    ...
    /// [`cspace_planning::PlanError`] the trait declares. Split out so this
    ...
    fn solve_inner(&mut self) -> Result<PlanningResponse<'m>, PlanError> {
        ...
        // does — timing is what `cspace_planning`'s
...
mod tests {
    ...
    /// `cspace_planning::PlanningRequest`'s own defaults, which this
    ...
    fn range_configuration(range: &str) -> PlannerConfigurationMap {
        let name = cspace_planning::configuration_name("panda_arm", "RRTConnect");
        let settings = cspace_planning::PlannerConfigurationSettings {
    ...
    /// `cspace_planning::configuration_for` must not find it, so the plan is
    ...
    fn a_configuration_for_another_group_leaves_the_plan_alone() {
        ...
        settings.name = cspace_planning::configuration_name("hand", "RRTConnect");
    ...
    fn unknown_group_is_rejected_before_any_search_runs() {
        ...
        // `cspace_planning::PlanError` is `Box<dyn Error>` by design, so the
...
"#;
    ...
    /// as reusable code (`doc/claim-audit/cspace-planners-sbp.md`'s round-24
    ...
    fn path_constraints_four_scenario_wired_vs_unwired_sweep() {
        ...
        // `doc/claim-audit/cspace-planners-sbp.md` is a claim, not merely an
        ...
        // `doc/claim-audit/cspace-planners-sbp.md`), and is exactly as
