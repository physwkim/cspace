...
//! `rg -c '^//! - `' crates/cspace-planning/src/constraints/lib.rs` is **61** — the
...
//! crates/cspace-planning/src/constraints/lib.rs` is **66**, one bullet per
...
//! (`sed -n '176,555p' crates/cspace-planning/tests/utils_parity.rs | rg
...
//! `cspace-ros::TryFrom<moveit_msgs::...>` will have to report as lossy.
//!
...
//!   `crate::constraints::utils`'s `update_*` functions, which call `constraints_mut()`).
//!
...
//!   which this core crate has no access to at all (D1/D2) — a `cspace-ros`
...
//!
//! Reproduction: `rg -c '^//! - CS:' crates/cspace-planning/src/constraints/lib.rs`
...
//! with `rg -c '^//! - CS:.*-> TAG' crates/cspace-planning/src/constraints/lib.rs`
//! for the given `TAG`:
//!
...
//!
//! `grep -rn 'assert_relative_eq!(' crates/cspace-planning/ --include=*.rs
