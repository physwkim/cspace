...
//!   access to at all (D1/D2) — a `cspace-ros` concern.
//!
...
//! (`crates/cspace-planning/src/scene/scene.rs:613`,`:671`), not on `RobotState`/
...
//! runs *before* constructing a [`crate::constraints::PositionConstraint`]/
//! [`crate::constraints::OrientationConstraint`] rather than mutating an already-built
...
//! added to [`crate::constraints::JointConstraint`]/[`crate::constraints::PositionConstraint`]/
//! [`crate::constraints::OrientationConstraint`] for exactly this purpose — not a raw
//! field overwrite.
//!
//! # Collapsed overloads
//!
...
//! function and only one, `scene.rs:951`, is the function's own `pub fn`
...
use crate::constraints::orientation::OrientationTolerance;
...
/// `JointConstraint::merged` (private: `crate::constraints::joint`'s per-joint merge
...
/// [`crate::constraints::JointConstraintSampler`]. That draw is
...
/// was accepted by [`crate::constraints::JointConstraint::decide`].
///
...
///   [`crate::constraints::IkConstraintSampler`], takes its bounds from a position
...
/// `updateJointConstraints`: for every [`crate::constraints::JointConstraint`] already in
...
/// [`crate::constraints::OrientationConstraint`] in `constraints` whose
...
/// [`crate::constraints::PositionConstraint`] in `constraints` whose
/// [`PositionConstraint::link_name`] is `link_name` with
/// `PositionConstraint::with_updated_position` (private: `crate::constraints::position`'s
...
/// [`crate::constraints::PositionConstraint::new`].
///
...
/// exactly mirroring [`crate::constraints::PositionConstraint::new`]'s own
...
/// into one ([`crate::constraints::PositionConstraint::new`] validates immediately — see
/// this crate's introducing doc comment on why "update reconstructs, not
/// mutates"). A [`crate::constraints::PositionConstraint`]/[`crate::constraints::OrientationConstraint`]
...
/// [`crate::constraints::PositionConstraint::new`]/[`crate::constraints::OrientationConstraint::new`] —
...
/// (matching [`crate::constraints::PositionConstraint::decide`]'s own point-not-vector
...
/// [`crate::constraints::OrientationConstraint::new`]) rather than on a built
...
/// to hand to [`crate::constraints::OrientationConstraint::new`] together with the
/// resolved link.
///
/// `Ok(None)` if `link_name` resolves via none of `resolve_frame_to_link`'s
