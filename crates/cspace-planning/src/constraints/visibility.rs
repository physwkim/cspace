...
/// `cspace_collision::ParryCollisionEnv`, mirroring upstream's local,
...
impl VisibilityConstraint {
    ...
    /// getter" gap [`crate::constraints::JointConstraint::weight`]/
    /// [`crate::constraints::PositionConstraint::weight`]/
    /// [`crate::constraints::OrientationConstraint::weight`] already close for `weight_`
    ...
    /// [`crate::constraints::JointConstraint::weight`]/[`crate::constraints::PositionConstraint::weight`]/
    /// [`crate::constraints::OrientationConstraint::weight`]): exposed for the same
    ...
    /// exactly that: a fresh `cspace_collision::World` holding only the
    ...
    /// --urdf crates/cspace-planning/tests/fixtures/constraints/constraints/pr2.urdf --srdf
    /// crates/cspace-planning/tests/fixtures/constraints/constraints/pr2.srdf --group right_arm
    ...
    /// contact *stored* (`cspace-collision`'s `accumulate_collision`,
    ...
    /// just empirically. That majority's cause is `cspace-collision`'s
    ...
    /// approximation lives in `cspace-collision`, owned by p3-acm — not
    ...
    fn cone_collision_result(
        ...
        state: &Posed,
        max_contacts: usize,
    ) -> cspace_collision::CollisionResult {
        let world_to_sensor = self.sensor.resolve(state);
        let world_to_target = self.target.resolve(state);
...
mod tests {
    ...
    fn pr2_model() -> RobotModel {
        let urdf_path = concat!(
            ...
            "/tests/fixtures/constraints/pr2.urdf"
        );
        let srdf_path = concat!(
            env!("CARGO_MANIFEST_DIR"),
            "/tests/fixtures/constraints/pr2.srdf"
        );
