...
//! let [`crate::scene::PlanningScene`] shadow a second, parallel notion of
//! "attached" next to a `RobotState` that has none, this crate is the sole
//! owner of attached-body data for now:
//! [`crate::scene::PlanningScene::attached_bodies`] is the one place this state
...
//! [`crate::scene::PlanningScene::detach`] only ever needs one transform (the
...
//! `pose_`/`global_pose_` do. [`crate::scene::PlanningScene::frame_transform`]
...
impl AttachedBody {
    ...
    /// [`crate::scene::PlanningScene::frame_transform`]) rather than this method
    /// re-parsing it, the same split [`cspace_collision::World`] already
    ...
    /// Borrows this body's fields as a [`cspace_collision::AttachedBodyGeometry`]
    /// — the view a [`cspace_collision::CollisionEnv`] backend needs, without
    /// `cspace-collision` depending back on this crate. See
    /// [`crate::scene::PlanningScene`]'s "Collision checking" doc for how this is used.
