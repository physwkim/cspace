...
//! [`Layered`]: the single mechanism [`crate::scene::PlanningScene`] uses to answer
//! "is this mine, or my parent's?" — see the crate-level doc for why this
//! exists instead of upstream's `std::optional<T>` fields.

/// A value a [`crate::scene::PlanningScene`] either owns locally, or defers to its
/// parent for.
///
...
/// upstream's own doc comment on [`crate::scene::PlanningScene::diff`] that really
...
/// reasoned about exactly once, and every [`crate::scene::PlanningScene`] accessor
