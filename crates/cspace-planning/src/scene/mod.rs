...
//! # Scope
//!
//! [`WorldDiff`] (a change record over a [`cspace_collision::World`]) and
...
//! state/path validity built on `crate::constraints`'s
//! [`crate::constraints::KinematicConstraintSet`] — see
...
//!
//! ```text
//! rg -c '^/// - `' crates/cspace-planning/src/scene/scene.rs
//! ```
//!
...
//! own module doc explains — `cspace_collision::World` has no observer
//! mechanism at all; every mutator returns the
//! [`cspace_collision::Notification`] it produced instead of pushing it
...
//! needs already exists in `cspace-collision`
//! ([`cspace_collision::CostSource`],
//! [`cspace_collision::CollisionResult::cost_sources`]), but
//! `cspace_collision::ParryCollisionEnv`'s collision callback hardcodes
//! `cost_sources: None`. [`PlanningScene`]'s own doc, "Cost sources and
//! diagnostics", traces exactly what would have to land in
//! `cspace-collision` to close it — split into a non-mesh fill-in (data
...
//!
//! ```text
//! rg -n '^fn .*matches_the_oracle' crates/cspace-planning/tests/*.rs
//! ```
//!
//! finds exactly **3**:
...
//! would duplicate `cspace-collision`'s own oracle parity suite rather than
...
pub use planning_scene::{ObjectColor, ObjectType, PathValidity, PlanningScene};
