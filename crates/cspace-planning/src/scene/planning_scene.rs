...
use crate::scene::attached_body::AttachedBody;
use crate::scene::layered::Layered;
use crate::scene::numeric::cxx_min;
use crate::scene::world_diff::WorldDiff;
...
///   call them is ROS-coupled, and `cspace-ros` (the one crate that could
...
///   `044a788f`, replaced `cspace_collision::ParryCollisionEnv`'s hardcoded
...
/// practice [`cspace_collision::ParryCollisionEnv`]) rather than owning one —
...
/// [`cspace_collision::CollisionRequest`]). D4's compile-time-registry
...
/// [`cspace_collision::AttachedBodyGeometry`] borrows. This follows upstream
...
/// [`cspace_collision::CollisionEnv::check_collision`]'s existing default
...
impl<'m> PlanningScene<'m> {
    ...
    /// `cspace-ros`'s identical local copy (`ros/cspace-ros/src/scene/collision_object.rs`).
    ...
    /// constraint against this scene (`crate::constraints::PositionConstraint::new`
    ...
    /// [`crate::constraints::KinematicConstraintSet::decide`] itself
    ...
    /// - [`cspace_collision::remove_cost_sources`] runs first, against
    ...
    /// - [`cspace_collision::remove_overlapping`] runs second (cpp:2490),
    ...
    /// [`WorldDiff::record`](crate::scene::WorldDiff::record)'s
...
/// `cspace_collision::World` uses internally for its own `isApprox` calls,
...
/// `cspace-collision`'s hardcoded dummy-precision default, and that
...
mod tests {
    ...
    use crate::constraints::{Constraint, JointConstraint};
    ...
    // transform. Mirrors `cspace_collision::parry`'s own test fixture.
    ...
    /// unrelated-group setup `cspace_collision::parry`'s own
