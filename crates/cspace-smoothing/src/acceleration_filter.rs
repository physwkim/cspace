...
// into `crates/cspace-smoothing/tests/fixtures/acceleration_filter_{request,response}.json`
...
//   [`cspace_model::JointModelGroup::active_joint_names`] entry from that
...
//   [`cspace_model::joint::JointModel::new_multi_variable`] names every
...
/// [`cspace_model::joint::VariableBounds`]'s public fields with no
...
mod tests {
    ...
    fn multi_dof_active_joint_is_a_typed_error_not_a_silent_last_variable_wins() {
        let urdf_path = format!(
            concat!(
                ...
                "/../cspace-trajectory/tests/fixtures/{}"
            ),
            "totg_synthetic.urdf"
        );
    ...
    // `_response.json` (`crates/cspace-smoothing/tests/fixtures/`), captured
