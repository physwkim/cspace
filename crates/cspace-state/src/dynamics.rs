...
//! that split:
//!
//! - **`X[i]` (`Segment::pose(q)`) is exactly [`cspace_model::LinkModel::joint_origin_transform`]
//!   composed with [`cspace_model::joint::JointModel::compute_transform`].** Both describe the
...
//! [`cspace_model::LinkModel::mass`]/[`cspace_model::LinkModel::center_of_mass`]/[`cspace_model::LinkModel::inertia`]
...
//! port's own [`cspace_model::joint::VariableBounds`] deliberately doesn't
...
use cspace_model::joint::JointKind;
...
fn rigid_body_inertia_from_link(link: &cspace_model::LinkModel) -> RigidBodyInertia {
    let m = link.mass();
...
impl<'m> DynamicsSolver<'m> {
        ...
        let active_max_torques = group
            ...
            .filter_map(|(&joint_index, &max_torque)| {
                let is_active = model.joint_model_at(joint_index).joint_type()
                    != cspace_model::joint::JointType::Fixed;
    ...
    fn sweep(
        ...
        for k in 0..ns {
            ...
            let is_active = joint.joint_type() != cspace_model::joint::JointType::Fixed;
        ...
        for k in (0..ns).rev() {
            let joint = self.model.joint_model_at(joint_indices[k]);
            if joint.joint_type() != cspace_model::joint::JointType::Fixed {
                j -= 1;
