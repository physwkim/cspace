...
//!    `crates/cspace-model/src/joint/urdf.rs`'s `joint_bounds_from_urdf`
...
//! -> Result<&mut JointModel>` (`crates/cspace-model/src/robot_model.rs`),
...
//! front of.
//!
//! **It is already ported**, as `cspace_state::dynamics::DynamicsSolver`
//! (`crates/cspace-state/src/dynamics.rs`): `torques`, `max_torques`,
...
//! `crates/cspace-state/tests/dynamics_parity.rs` reads
...
//!   for the same reason: [`cspace_model::JointModelGroup`] has no
//!   index-list-returning method in this workspace, and this crate's
//!   [`cspace_state::RobotState`] accessors are name-based.
...
) -> Vec<(String, String, cspace_model::joint::VariableBounds)> {
...
pub fn has_mixed_joint_types(trajectory: &RobotTrajectory<'_>, group: &JointModelGroup) -> bool {
    ...
    for &joint_index in group.active_joint_indices() {
        ...
        match joint.joint_type() {
            cspace_model::joint::JointType::Prismatic => have_prismatic = true,
            cspace_model::joint::JointType::Revolute => have_revolute = true,
            _ => {}
        }
    }
