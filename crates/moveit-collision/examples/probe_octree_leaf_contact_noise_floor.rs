...
// `crates/moveit-collision/src/parry.rs` -- this calls
// `moveit_geometry::compound_from_octree` and `parry3d_f64::query::contact`
...
//! # Method
//!
//! Builds a real [`moveit_octomap::OcTree`] at each swept `resolution`,
//! occupies one leaf via [`moveit_octomap::OcTree::update_node`] (so the
//! leaf's `coordinate()`/`size()` come from the tree's own
//! `key_to_coord_axis`/`node_size`, not a hand-approximation), converts it
//! through the real [`moveit_geometry::compound_from_octree`], and places a
...
//!
//! Run: `cargo run -p moveit-collision --example probe_octree_leaf_contact_noise_floor --release`
...
fn touching_dist_at_leaf_offset(
    ...
    let leaf = tree
        .leaves()
        .find(moveit_octomap::Leaf::is_occupied)
...
fn main() {
    ...
    // (`OcTree::TREE_MAX_VAL = 32768`, `moveit-octomap/src/tree.rs:646`) --
