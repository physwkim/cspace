...
use moveit_geometry::{Isometry3, Shape, UnitQuaternion, Vector3};
...
// could depend on, and `moveit-collision` cannot depend on
// `moveit-constraints` either, since the dependency edge runs the other
...
/// it (that file's own module doc): `moveit-constraints` depends on this
