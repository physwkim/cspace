    ...
    for ((body_1, body_2), contact_vector) in &mut contacts.by_pair {
        ...
        for contact in contact_vector {
            ...
            let node_centers: Vec<Vector3> = match octree.leaves_in_bbx(bbx_min, bbx_max) {
                Some(leaves) => leaves
                    .filter(moveit_octomap::Leaf::is_occupied)
...
mod tests {
    ...
    use moveit_geometry::{Isometry3, OcTree as OcTreeShape, Shape, Sphere};
    use nalgebra::Point3;
    ...
    fn octree_with_cells(cells: &[Point3<f64>]) -> moveit_octomap::OcTree {
        let mut tree = moveit_octomap::OcTree::new(0.5);
        for cell in cells {
    ...
    fn object_with_octree(tree: Option<moveit_octomap::OcTree>) -> Arc<crate::world::Object> {
        let mut world = World::new();
        let shape = Shape::OcTree(OcTreeShape::from_tree(Arc::new(
            tree.unwrap_or_else(|| moveit_octomap::OcTree::new(0.5)),
        )));
        world
