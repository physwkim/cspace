...
//!    `moveit_geometry::Transforms` — already made for the same reason.
//! 6. **`getTransform`'s two overloads become [`World::get_transform`] /
//!    [`World::try_get_transform`].** Same split as
//!    `moveit_geometry::Transforms::transform`/`try_transform`: the
//!    `Result`-returning form uses [`moveit_error::Error::UnknownName`] where
...
mod tests {
    ...
    fn sphere(radius: f64) -> Arc<Shape> {
        Arc::new(Shape::Sphere(moveit_geometry::Sphere::new(radius).unwrap()))
    }
