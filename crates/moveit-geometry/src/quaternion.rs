...
//!
//! The first version of this lived privately in `moveit-model`'s
//! `floating.rs`. Sweeping the anchor `rg -n 'slerp' crates/` found two more
//! sites porting the same Eigen call — `moveit-kinematics`'
//! `cartesian_interpolator.rs` and `moveit-planners-pilz`'
//! `trajectory_blender_transition_window.rs` — and one that is **not** the
//! same defect: `moveit-planners-sbp`'s `se3.rs`, which hand-writes OMPL's
