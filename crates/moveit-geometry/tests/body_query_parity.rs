...
//! Parity test for [`moveit_geometry::bodies::Body`]'s posed algorithms
    ...
    for (request, response) in requests.iter().zip(&responses) {
        ...
        let shape = match &request.shape {
            ShapeSpec::Sphere { radius } => {
                Shape::Sphere(moveit_geometry::shapes::Sphere { radius: *radius })
            }
            ShapeSpec::Cylinder { radius, length } => {
                Shape::Cylinder(moveit_geometry::shapes::Cylinder {
                    radius: *radius,
                    length: *length,
                })
            }
            ShapeSpec::Box { size } => {
                Shape::Cuboid(moveit_geometry::shapes::Cuboid { size: *size })
            }
        };
