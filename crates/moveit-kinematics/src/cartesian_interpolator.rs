...
//! `moveit-error`, `moveit-geometry`, `moveit-model` and `moveit-state` are
...
//! `RobotModel` loads `kinematics.yaml` and `moveit_model::JointModelGroup`
...
//!   [`moveit_geometry::Isometry3`] is `nalgebra::Isometry3`, a translation
...
fn enforce_continuous_joint_bounds(state: &mut RobotState<'_>, group: &JointModelGroup) {
    ...
    let continuous: Vec<&JointModel> = group
        ...
        .filter(|joint| {
            joint
                .as_revolute()
                .is_some_and(moveit_model::joint::RevoluteJoint::is_continuous)
        })
        .collect();
