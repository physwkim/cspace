...
//! [`KinematicsSolver`], and `moveit-state` -> `moveit-kinematics` is a Cargo
//! cycle (`moveit-kinematics` already depends on `moveit-state`, see this
//! crate's `Cargo.toml`). So it is a free function in `moveit-kinematics`
//! taking `&mut RobotState`, over the edge that already exists — the same
//! placement, and the same reason, as [`crate::CartesianInterpolator`]. No
//! new crate edge is added by this module.
//!
//! Attached bodies are the one thing that placement cannot reach directly:
//! this workspace keeps them on `moveit-scene`'s `PlanningScene`, not on
//! `RobotState` (see that crate's `attached_body` module doc), and
//! `moveit-scene` depends on `moveit-kinematics` transitively through
//! `moveit-constraints`, so a `moveit-kinematics` -> `moveit-scene` edge
...
//!    [`SolverParams::max_restarts`](crate::SolverParams::max_restarts)
...
/// The part of `RobotState`'s frame resolution that lives on `moveit-scene`
...
/// (see `moveit-scene`'s `attached_body` module doc), and a
/// `moveit-kinematics` -> `moveit-scene` dependency would close a cycle
/// through `moveit-constraints`. Implement this on whatever type does hold
...
/// [`crate::CartesianInterpolator`] does exactly that, once per waypoint and
...
/// and links from `moveit-state`, attached bodies and their subframes from
...
/// internally — [`SolverParams::max_restarts`](crate::SolverParams::max_restarts)
