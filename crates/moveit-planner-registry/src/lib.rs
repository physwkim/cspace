...
//!
//! Everything here could have been a module of `moveit-planning`, next to
...
//! of `moveit-planning`'s planning logic (PORTING-PLAN.md §140.1; see this
//! crate's `Cargo.toml` for the full argument).
//!
//! The layering that falls out of that is deliberate and worth stating:
//! `moveit-planning` defines the vocabulary (`PlanningRequest`,
...
//! `moveit_planning::pipeline::generate_plan` does not — it takes already
...
/// request. Mirrors `moveit_kinematics::registry::SolverRegistration`
...
pub struct PlannerRegistration {
    ...
    /// builds — `"rrt_connect"` for `moveit_planners_sbp::RrtConnectManager`.
...
/// `moveit-planners-sbp` already sets it for exactly this reason.
///
...
/// Measured, not reasoned about: `moveit-planning`'s crate-level doctest and
/// this crate's `tests/registered_planners.rs` both failed exactly that way
/// until each grew a `use moveit_planners_sbp as _;`. Any binary that
/// expects to find a planner here needs that line (see
/// `ros/moveit-ros/src/lib.rs` for the production one).
...
/// `moveit_kinematics::registry::resolve_solver`, whose registrations are
