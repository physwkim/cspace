...
//! The half of `moveit-planner-registry`'s coverage that needs a real
...
//! `moveit-planner-registry`, so a *unit*-test build of this crate links two
...
// nothing below names a `moveit_planners_sbp` item, so without this line the
...
/// through that name (`moveit_planning::pipeline::PipelineError::Planner`),
