...
//!   (`moveit-planners-chomp`'s ported twin is `ChompTrajectory::
...
//! moveit-planners-sbp` with a `plan`-op request JSON on stdin, optionally
...
fn parse_obstacles(objects: &[serde_json::Value]) -> Vec<(String, Arc<Shape>, Isometry3)> {
    objects
        ...
        .map(|object| {
            ...
            (id, shape, translation_from_row_major_4x4(&pose_flat))
        })
        .collect()
}
