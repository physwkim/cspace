...
//! carry [`moveit_constraints::KinematicConstraintSet`], not a raw message —
//! and [`moveit_constraints::PositionConstraint::new`]/
//! [`moveit_constraints::OrientationConstraint::new`] both require
//! `link_name` to already name a real robot link
//! (`RobotModel::link_model(link_name)`, `moveit-constraints/src/position.rs`/
...
//! [`moveit_scene::PlanningScene`]/`AttachedBody`, a type `RobotModel` does
//! not reference) — so a subframe-named `link_name` can never exist in an
//! already-constructed [`moveit_constraints::PositionConstraint`]/
//! [`moveit_constraints::OrientationConstraint`] in this workspace at all.
...
//! Verified, not assumed: `rg -n 'pub fn new' crates/moveit-constraints/src/position.rs
//! crates/moveit-constraints/src/orientation.rs` shows both taking
