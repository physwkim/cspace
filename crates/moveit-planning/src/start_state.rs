...
//!   [`moveit_scene::PlanningScene`], not on a state — so the msg->core
//!   conversion in `moveit-ros` rejects `attached_collision_objects` on its
...
pub enum StartState {
    /// Plan from [`moveit_scene::PlanningScene::current_state`] exactly, with
...
impl StartState {
    ...
    /// # Errors
    ///
    /// [`moveit_error::Error::UnknownName`] if the overlay names a variable
...
impl StartStateOverride {
    ...
    /// # Errors
    ///
    /// [`moveit_error::Error::Construct`] if `names` is empty, if `positions`
