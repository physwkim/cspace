...
//! [`MultivariateGaussian`]'s module doc) differ. `moveit-planners-stomp`
//! and (in a future round) `moveit-planners-chomp` both need this class.
...
//! planner crates depend on `moveit-sampling`.
//!
...
//! counting convention (`moveit-geometry`/`moveit-octomap`), every
