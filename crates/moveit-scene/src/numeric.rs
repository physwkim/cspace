...
// than reused from `moveit-collision`.

//! `std::min`-compatible comparison.
//!
//! Same contract as `moveit_trajectory::numeric`'s [`cxx_min`] — see that
...
//! not by inspection — see `moveit_trajectory::numeric`'s own doc comment
