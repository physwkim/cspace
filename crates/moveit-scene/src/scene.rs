...
impl<'m> PlanningScene<'m> {
    ...
    /// See [`PlanningScene::attached_bodies_diff`]'s doc for why this is
    ...
    /// Upstream `pushDiffs`.
    ///
    /// If [`PlanningScene::attached_bodies_diff`] is
    /// [`AttachedBodiesDiff::Touched`], `target`'s entire attached-body set
    /// is overwritten with this scene's — matching upstream's granularity
    /// exactly: attached bodies live inside `RobotState` there, so pushing
    /// `robot_state_` (`planning_scene.cpp:348`, only if locally
    /// materialized) already replaces the whole thing at once, not a
    /// per-id merge. If [`AttachedBodiesDiff::Untouched`] (or this is a
    /// root scene, `None`), `target`'s attached bodies are left alone —
    /// see [`PlanningScene::attached_bodies_diff`]'s doc for why that
    /// distinction is recorded at mutation time rather than inferred here
