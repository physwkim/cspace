...
//! (the oracle's real answer) — see `world_parity.rs` in `moveit-collision`
...
//! [`moveit_error::Error::UnknownName`] for a name resolving in no tier, the
//! idiomatic `Result` shape every other lookup in this port already uses
//! (see e.g. [`moveit_collision::World::get_transform`], upstream's own
...
/// [`moveit_collision::World`]'s own lower-level `add_to_object` directly
