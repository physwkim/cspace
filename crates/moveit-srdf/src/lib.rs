...
//!
//! ```
//! use moveit_srdf::{SrdfModel, VirtualJointType};
//!
...
//! puts `moveit-srdf` below `moveit-model`, so no URDF is available here and
//! those checks belong to `moveit-model`, which holds both descriptions. The
