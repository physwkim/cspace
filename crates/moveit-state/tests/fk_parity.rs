...
//! for [`moveit_model::joint::PlanarJoint`]'s `normalize_angle` path.
...
fn build_model(urdf_file: &str, srdf_file: &str) -> RobotModel {
    ...
    // groups moveit-srdf reports as `UnknownGroup` diagnostics — a real
    // quirk of that fixture's SRDF, not a parse failure, and `<end_effector>`
    // is out of `moveit-model`/`moveit-state`'s scope either way (see
...
/// bounds comparison in the sibling `moveit-model` crate.
