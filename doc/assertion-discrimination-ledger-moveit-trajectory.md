...
| `tests/robot_trajectory.rs:560` | `set_way_point_duration_from_previous`'s single `index == 0 && value != 0.0` guard (src `:234`) | `set_way_point_duration_from_previous_at_zero_rejects_a_nonzero_value` | single-branch | structural: exactly one `if`/one `return Err` in the function body |
| `tests/robot_trajectory.rs:578` | `append`'s single `self.waypoints.is_empty() && dt != 0.0` guard (src `:338`) | `append_onto_an_empty_trajectory_rejects_a_nonzero_dt` | single-branch | structural: `append`'s only other early return (`start_index >= end_index`) is `Ok`, not `Err` — one `Err` site total |
| `tests/robot_trajectory.rs:596` | `way_point`'s single `ok_or_else(index_error)` (src `:171`) | `empty_trajectory_accessors_return_typed_errors_not_panics` | single-branch | structural: one `ok_or_else` in the function body |
| `tests/robot_trajectory.rs:597` | `first_way_point`'s single `ok_or_else(empty_error)` (src `:186`) | same fn | single-branch | same |
| `tests/robot_trajectory.rs:598` | `last_way_point`'s single `ok_or_else(empty_error)` (src `:196`) | same fn | single-branch | same |
| `tests/robot_trajectory.rs:618` | `is_empty()` on an emptied trajectory | `reverse_on_an_empty_trajectory_is_a_no_op` | not-this-family (clause 1) | — |
| `tests/robot_trajectory.rs:696` | `way_point(len)`'s single `ok_or_else(index_error)` | `out_of_range_index_access_is_a_typed_error` | single-branch | same structural argument as `:568` |
| `tests/robot_trajectory.rs:697` | `way_point_mut(len)`'s single `ok_or_else(index_error)` (src `:178`) | same fn | single-branch | structural: one `ok_or_else` in the function body |
| `tests/robot_trajectory.rs:698` | `remove_way_point(len)`'s single `index >= len` guard (src `:314`) | same fn | single-branch | structural: exactly one `if`/one `return Err` |
| `tests/robot_trajectory.rs:704` | `insert_way_point`'s `index > self.waypoints.len()` guard (src, `index_error`) | `out_of_range_index_access_is_a_typed_error` | discriminating | bite (`&& !true`; message-unique against `first_duration_error`) |
| `tests/robot_trajectory.rs:736` | `Debug`-formatted `"dirty: None"` substring on a waypoint after settling | `add_suffix_way_point_settles_the_stored_waypoint` | not-this-family (clause 1 — `Debug` text on a successfully-settled value) | — |
| `tests/robot_trajectory.rs:751` | same | `add_prefix_way_point_settles_the_stored_waypoint` | not-this-family (clause 1) | — |
| `tests/robot_trajectory.rs:766` | same | `insert_way_point_settles_the_stored_waypoint` | not-this-family (clause 1) | — |
| `tests/robot_trajectory.rs:777` | `for_group_name`'s single fallible call (`joint_model_group(group)?`, src `:126`) | `unknown_group_name_is_a_typed_error_not_a_silent_whole_robot_fallback` | single-branch | structural: exactly one `?`-propagated call in the function body; the external callee (`moveit-model`) is out of this crate's fence |
