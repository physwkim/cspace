...
- `tools/moveit-diff/src/main.rs:2534`'s `near_placement_never_touches_more_than_one_link_at_once` is `#[ignore]`d and needs `third_party/moveit_resources`; that directory does not exist in this worktree (`find / -iname 'moveit_resources*'` finds nothing on this machine), so this test could not be run to confirm or refute anything about it this round. Noted, not silently skipped.
