...
| file:line | member? | verdict | evidence |
|---|---|---|---|
| `crates/moveit-model/src/joint/model.rs:946` (`local_variable_names().is_empty()` in `single_variable_joints_use_the_bare_joint_name`, single-variable joint) | yes | discriminating | cross-test sibling `multi_variable_joints_prefix_local_names_with_the_joint_name` shows 7 non-empty names for a floating joint |
