...
| `crates/moveit-state/tests/csv_conversions.rs:188` | `assert!(line.contains(&FIRST_POSITION.to_string()), ...)` — the writer emitted all 16 significant digits, not upstream's six (`doc/upstream-bugs.md`, `robot-state-to-stream-default-ostream-precision`) | `a_round_trip_returns_every_position_bit_for_bit` | single-branch | Exactly one expression formats a value: `.map(f64::to_string)` at `conversions.rs:105`. Bite D3 replaces it with `format!("{v:e}")` — chosen because it is *still* an exact round trip, so the preceding `assert_eq!(read.positions(), written.positions())` at `:187` passes and execution reaches this site. 1 test fails, panicking at `csv_conversions.rs:188` (`a_round_trip_returns_every_position_bit_for_bit`). The `{:.6}` bite (C5 in the port's own bite set) is not the evidence here: it fails at `:187` first, and this site never runs. |
