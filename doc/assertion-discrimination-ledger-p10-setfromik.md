...
The eleven sites the sweep's scanner emits for the `setFromIK` port. All
eleven are test assertions in
`crates/moveit-kinematics/tests/set_from_ik.rs`, and all eleven are of the
...
`kind` strings, one of which three separate rows now share — so every row
below is a claim about *which* site, or which condition reaching it, the
test can name; not merely that some error came back.
...
passing, `moveit-model` 144 tests / 144 passing. Bites B9c, B24, B25 and
B26 were run later, against `moveit-kinematics` 101 tests / 101 passing;
their rows say "the other 100 stay green" for that reason, and the earlier
rows' "75 stay green" is left as it was measured rather than restated
against a baseline those bites were never run on.
...
|---|---|---|---|---|
| `crates/moveit-kinematics/tests/set_from_ik.rs:388` | `matches!(error, Error::Other(ref m) if m.contains("panda_link6"))` — the `let Some(tip_id) = matched else` arm of `resolve_ik_queries` (`crates/moveit-kinematics/src/set_from_ik.rs:366-372`) | `a_target_naming_a_frame_across_a_moving_joint_is_not_a_tip_match` | discriminating | Bite B3b: `... == rigid_parent_link(model, attached, tip)? \|\| true`, so every frame matches the first unclaimed tip. That one test FAILS (it gets `Ok` where it asserted `Err`); the other 75 stay green, including `:485`, the only other test that reaches this same `else`. The substring is the *cited frame name*, which the message interpolates from the target, so a different target reaching the same site prints a different name. |
| `crates/moveit-kinematics/tests/set_from_ik.rs:485` | `matches!(error, Error::Other(ref m) if m.contains("IK target 1"))` — the same `else`, reached because `claimed[tip_id]` skipped the only tip | `a_tip_a_target_already_claimed_cannot_be_matched_twice` | discriminating | Bite B4: delete `if claimed[tip_id] { continue; }` from the *matching* loop, leaving the fill loop's copy in place. That one test FAILS; 75 stay green, `:388` among them. The two rows are each other's isolating evidence: B3b fails `:388` and not `:485`, B4 fails `:485` and not `:388`, so neither is reading the other's branch. The index in `IK target 1` is the loop counter, so it also pins *which* target was refused. |
| `crates/moveit-kinematics/tests/set_from_ik.rs:737` | `matches!(error, Error::Other(ref m) if m.contains("set_from_ik_subgroups"))` — `set_from_ik`'s `let [target] = queries.as_slice() else` (`crates/moveit-kinematics/src/set_from_ik.rs:518-525`) | `a_solver_reporting_two_tips_is_refused_rather_than_solved_for_the_first` | discriminating | Bite B10: `let [target, ..] = queries.as_slice() else`, which accepts any non-empty query list and silently solves the first tip. That one test FAILS; 75 stay green — including `a_tip_no_target_named_is_filled_with_the_pose_it_currently_holds`, which drives the same two-tip solver through `resolve_ik_queries` directly and so must not move when this refusal does. |
| `crates/moveit-kinematics/tests/set_from_ik.rs:1366` | `matches!(error, Error::Other(ref m) if m.contains("at least one"))` — `set_from_ik_subgroups`' `solvers.is_empty()` guard | `no_subgroup_solvers_is_an_error_not_a_vacuous_success` | discriminating | Bite B15: `if false {`. That one test FAILS; 75 stay green, `:1392` among them. Necessary as its own guard rather than a consequence of the length check: with zero solvers *and* zero targets the length check passes, and the sweep loop then runs zero subgroups and returns `Ok(true)` — a success that solved nothing. B15's failure is exactly that `Ok(true)`. |
| `crates/moveit-kinematics/tests/set_from_ik.rs:1392` | `matches!(error, Error::Other(ref m) if m.contains("2 subgroup solvers for 1 targets"))` — the `solvers.len() != targets.len()` guard | `one_target_per_subgroup_solver_or_it_is_an_error` | discriminating | Bite B14: `if false {`. That one test FAILS; 75 stay green, `:1366` among them. The substring carries both counts, so it separates this site from `:1366`'s (which names neither) and would not survive the counts being transposed. |
| `crates/moveit-kinematics/tests/set_from_ik.rs:1421` | `matches!(error, Error::Other(ref m) if m.contains("not a subgroup"))` — the `group.is_subgroup(solver.group_name())` guard | `a_solver_for_a_group_that_is_not_a_subgroup_is_refused` | discriminating | Bite B16: `if false {`. That one test FAILS; 75 stay green. The fixture solver is `left_arm`'s, relabelled to claim the group `base`, so the solve itself would succeed if the guard let it through — the test is measuring the guard and not the solver. |
...
which is what lets the first two rows claim discrimination rather than
collapsing into "some name was unknown".

The last three rows are a harder case and are argued differently. They
share one construction site — `solver_solution_variables`' single
`Error::unknown_name("group variable", name)` — so `kind` cannot tell them
apart, and neither can the variant. What separates them is *which of that
function's three conditions* let the name through to it, and each row's
evidence is a mutation that removes one of those conditions and no other.
A row here therefore claims "this test names this condition", not "this
test saw a `group variable` error".

| file:line | anchor | test fn | verdict | evidence |
|---|---|---|---|---|
| `crates/moveit-kinematics/tests/set_from_ik.rs:411` | `matches!(error, Error::UnknownName { kind: "IK frame", ref name } if name == "no_such_frame")` — `rigid_parent_link`'s attached-frame fallback (`crates/moveit-kinematics/src/set_from_ik.rs:269-271`) | `a_target_naming_nothing_in_the_model_is_unknown_name_not_no_match` | discriminating | Bite B22: change that one site's kind from `"IK frame"` to `"link"`, leaving the variant, the name and every other site untouched. That one test FAILS; 75 stay green, `:620` among them — so the assertion is reading the kind, not merely the variant, and the two `UnknownName` sites in this file are told apart by it. |
| `crates/moveit-kinematics/tests/set_from_ik.rs:620` | `matches!(error, Error::UnknownName { kind: "link", ref name } if name == "no_such_base")` — `to_solver_frame`'s `posed.global_link_transform(ik_frame)?` on a base frame that is neither the model frame nor a link | `a_solver_based_on_a_frame_the_model_does_not_have_is_unknown_name` | discriminating | Bite B23: `.unwrap_or_else(\|_\| Isometry3::identity())` on that transform lookup, so an unknown base silently becomes the model frame. That one test FAILS; 75 stay green, `:411` among them. |
| `crates/moveit-kinematics/tests/set_from_ik.rs:764` | `matches!(error, Error::UnknownName { kind: "group variable", ref name } if name == "panda_joint1")` — `solver_solution_variables`' membership test, `group.variable_names().iter().any(...)` | `a_solver_whose_joints_are_not_the_groups_variables_is_unknown_name` | discriminating | Bite B9c: check membership against `model.variable_names()` instead of `group.variable_names()` — the exact defect the guard exists to prevent, since upstream's own message is "group '%s' does not contain such a joint". `panda_joint1` is a model variable, so it is accepted and this one test FAILS; the other 100 stay green, `:859` and `:894` among them (`panda_hand_joint` and `world_joint` are not model variables either, so both still reach the same `Err` by the same route as before). Supersedes bite B9b, which was run against `check_solver_joints_are_group_variables` before the fixed-joint skip existed and no longer discriminates: with `&& false` on the membership test every name now falls through to the skip, and `hand` has no joint model named `panda_joint1`, so the test still passes. The name asserted is the *first* offending joint, so the row also pins that the loop reports at the first failure rather than after it. |
| `crates/moveit-kinematics/tests/set_from_ik.rs:859` | `matches!(error, Error::UnknownName { kind: "group variable", ref name } if name == "panda_hand_joint")` — the `group.has_joint_model(name)` conjunct of the fixed-joint skip (`crates/moveit-kinematics/src/set_from_ik.rs:430-433`) | `a_fixed_joint_outside_the_group_is_still_unknown_name` | discriminating | Bite B24: drop that conjunct, leaving `model.joint_model(name).is_ok_and(\|j\| j.joint_type() == JointType::Fixed)`, so any fixed joint of the *model* is forgiven. `panda_hand_joint` is fixed, so it is then skipped and the solve succeeds; this one test FAILS and the other 100 stay green, `:764` and `:894` among them. Neither sibling can ride on this conjunct: `panda_joint1` is not fixed at all, and `world_joint` is in its group already. |
| `crates/moveit-kinematics/tests/set_from_ik.rs:894` | `matches!(error, Error::UnknownName { kind: "group variable", ref name } if name == "world_joint")` — the `joint_type() == JointType::Fixed` conjunct of the same skip | `a_multi_variable_joint_named_by_the_joint_is_unknown_name` | discriminating | Bite B25: drop that conjunct, so any joint of the group that is not one of its variables is skipped. PR2's `base` group is the single planar joint `world_joint`, whose variables are `world_joint/x`, `/y` and `/theta`, so the bare name is in the group and not among its variables — under B25 it is skipped, the correspondence comes out empty, and the call no longer errors. This one test FAILS; the other 100 stay green, `:764` and `:859` among them. This is the conjunct that keeps a multi-variable joint out of a single-slot correspondence, so it is load-bearing beyond mirroring upstream's `getType() == FIXED`. |
...
| B9c | check solver-joint membership against `model.variable_names()` instead of `group.variable_names()` | `a_solver_whose_joints_are_not_the_groups_variables_is_unknown_name` | Exclusive. Replaces B9b (`&& false` on the same test), which stopped discriminating once the fixed-joint skip was added: with membership forced false every name reaches the skip, and `hand` has no joint model named `panda_joint1`, so the test passed under the mutation. |
| B26 | neutralise the fixed-joint skip entirely — keep the `if`, empty its body, so a fixed joint of the group falls straight through to the `Err` | `a_fixed_joint_of_the_group_takes_no_seed_or_solution_slot` | Exclusive. That test compares two solves of the same target — one by a solver reporting `panda_arm`'s seven joints, one by a solver reporting the same seven with the group's fixed `panda_joint8` inserted *between* the first and the second — and asserts the two end states are equal position for position. Inserting it in the middle rather than appending it is what makes the assertion measure slot alignment: a correspondence that kept `panda_joint8` would not merely be one entry long, it would put every later solution value on the wrong variable. Under B26 the second solve returns `Err` instead, so the failure is the skip's absence and not a numerical drift. |
