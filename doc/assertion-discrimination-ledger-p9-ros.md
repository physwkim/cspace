...
|---|---|---|---|---|
| `constraints/joint.rs:95` | matches! | unknown_joint_name_is_rejected | discriminating | `9c05e4f` |
| `constraints/orientation.rs:178` | matches! | converts_with_xyz_euler_parameterization | not-this-family | §9 clause 1 — `c.tolerance()` is read after `.unwrap()` on a successful build; `OrientationTolerance::XyzEuler{..}` is a computed success-path enum tag, not a failure/absence signal |
| `constraints/orientation.rs:264` | matches! | unknown_frame_is_rejected | discriminating | `3303a7e` |
| `constraints/position.rs:294` | matches! | prism_is_rejected | single-branch | bite run just now — `Error::Other` has exactly one producing arm (`PRISM`) in `TryFrom<SolidPrimitiveMsg> for Shape` |
| `constraints/set.rs:165` | matches! | aggregates_joint_constraints | not-this-family | §9 clause 1 — `set.constraints()[0]` read after `.unwrap()` on a successful build; `Constraint::Joint(_)` is a computed success-path enum tag |
| `constraints/set.rs:185` | matches! | one_bad_element_fails_the_whole_conversion | discriminating | `a7d8682` |
| `constraints/set.rs:423` | matches! | (visibility round-trip) | not-this-family | §9 clause 1 — same shape as `:163`, `Constraint::Visibility(_)` on a successful build |
| `constraints/visibility.rs:220` | matches! | (sensor_view_direction rejects unknown) | single-branch | bite run just now — single `other =>` arm in `TryFrom<SensorViewDirectionMsg>` |
...
| `scene/planning_scene.rs:1447` | matches! | unresolvable_non_empty_header_frame_id_is_still_rejected | single-branch | bite run just now — `Error::UnknownName` reachable only via `header_frame_transform` (see `scene/mod.rs:94`); the file's other two error sites are `Error::Other`/`Error::Construct`, different variants |
...
| `ros/moveit-ros/src/trajectory.rs:385` | matches! | add_suffix_way_point_rejects_a_nonzero_first_dt | single-branch | bite run just now — `moveit_trajectory::RobotTrajectory::add_suffix_way_point`'s single `first_duration_error()` site |
| `ros/moveit-ros/src/trajectory.rs:484` | matches! | seconds_to_duration_rejects_just_above_i32_max_seconds | single-branch | bite run just now — `seconds_to_duration`'s one combined guard |
| `ros/moveit-ros/src/trajectory.rs:490` | matches! | seconds_to_duration_rejects_negative | single-branch | bite run just now — same guard |
| `ros/moveit-ros/src/trajectory.rs:496` | matches! | seconds_to_duration_rejects_nan | single-branch | bite run just now — same guard |
| `ros/moveit-ros/src/trajectory.rs:502` | matches! | seconds_to_duration_rejects_infinity | single-branch | bite run just now — same guard |
| `ros/moveit-ros/src/trajectory.rs:530` | matches! | negative_cumulative_duration_from_an_unvalidated_trajectory_is_rejected | single-branch | bite run just now — `TryFrom<RobotTrajectory> for JointTrajectoryMsgOut`'s sole `?` site is `seconds_to_duration` |
...
`scene/collision_object.rs:1029` (`append_without_subframe_data_clears_existing_subframes`). Reclassified by isolating mutation (see the
...
defined separately per file (`ros/moveit-ros/src/state.rs:194` (`assert_err_mentions`), `ros/moveit-ros/src/trajectory.rs:250` (`assert_err_mentions`),
`planning.rs:510` (`assert_err_mentions`), `scene/attached.rs:311` (`assert_err_mentions`), `scene/collision_object.rs:536` (`assert_err_mentions`)
...
|---|---|---|
| `constraints/orientation.rs:196,219,246` | in-family (3 sites) | CLEAN (3 sites) — `invalid_parameterization_is_rejected`, `degenerate_orientation_is_rejected`, `orientation_norm_2_is_rejected_end_to_end_unlike_a_scene_pose`. **Reformatted (§15's round):** the cell used to hold three separately backticked citations, of which `FIRST_COL_RE` reads only the first, so `:217`/`:244` reported as orphans under the committed instrument while this row covered them all along. **Evidence upgraded from "confirmed by reading" to measured:** rewording the `OrientationConstraintQuaternion` message in `geometry.rs` ("more than 1e-3 from 1.0" -> "outside the unit band") fails `degenerate_orientation_is_rejected` and `orientation_norm_2_is_rejected_end_to_end_unlike_a_scene_pose` at their own assert lines and nothing else, `invalid_parameterization_is_rejected` staying green; aliasing the wrapper away so the field takes the generic `Quaternion` rule instead fails `degenerate_orientation_is_rejected` at its assert (norm 0 is still rejected, but by the generic "too close to zero" wording) and `orientation_norm_2_is_rejected_end_to_end_unlike_a_scene_pose` at its `.unwrap_err()` (norm 2 normalizes under the generic rule instead of failing) — which is exactly the §211 claim that test's own doc comment makes |
| `constraints/position.rs:310,394,413,434,458,474` | in-family | CLEAN (6 sites) — `dim()`'s `{field}`-interpolated messages (e.g. `:308`'s own `.contains("BOX_Y")`), `PositionConstraint::new`'s three distinct `Error::construct` texts, and the meshes-guard's shared-but-same-branch needle all verified against `crates/moveit-constraints/src/position.rs` |
| `constraints/set.rs:150` | **not-this-family** (clause 2) | n/a — `empty_constraints_is_empty_set` is the census's own vacuous-accumulator shape verbatim: all four input vecs empty, none of the four `for` loops in `KinematicConstraintSet::try_from` iterate, `set` is never touched |
| `constraints/visibility.rs:386,387` (`negative_target_radius_activates_but_negative_angles_stay_inactive`) | in-family | CLEAN (2 sites) — `normalize_angle_criterion`'s `>EPS` filter is the sole physical producer of `None` for each field; wire wrapper always constructs `Some(msg.field)` first, never `None` directly |
| `conversion_coverage.rs:271,276,430,458,481,498` | in-family (all 6) | CLEAN — re-derived after `4f2c9df` added 34 lines to `ONE_DIRECTIONAL` ahead of every one of this row's old `:227,232,386,414,437,454` citations, a pure uniform shift confirmed line-by-line against the current body (no semantic change). `:261/:266` are my own round-8 `parse_conversion` tests (already bite-checked); `:420` self-identifies via interpolated `t.from`/`t.to`/`t.covered_by`/`t.reason` in its assert text; `:448/471/488` populate disjoint vecs from disjoint scan logic, each assert interpolates the full offending list |
| `scene/shapes.rs:161,179` | in-family (2 sites) | CLEAN (2 sites) — `mesh_triangle_with_wrong_vertex_count_is_rejected`, `mesh_out_of_range_vertex_index_is_rejected`. **Reformatted (§15's round)** for the same `FIRST_COL_RE` truncation as the orientation row above, which had left `:179` an orphan. `"expected exactly 3"` is `shapes.rs`'s sole `MeshTriangle`-length format string; the "only N vertices exist" wording traces to the single physical site `moveit-geometry/src/shapes.rs:1133` (source: `"mesh triangle references vertex {idx}, but only {n} vertices exist"`), confirmed by grep not reproduced elsewhere reachable from `TryFrom<MeshMsg>` — and measured this round rather than left at the grep: rewording that one message to "out of range for {n} verts" fails `mesh_out_of_range_vertex_index_is_rejected` at `:179` and nothing else, `mesh_triangle_with_wrong_vertex_count_is_rejected` staying green, so the needle does separate `Mesh::new`'s index check from this file's own length check |
| `scene/planning_scene.rs:1162,1179,1215` | in-family (3 sites) | CLEAN (3 sites) — `empty_collision_objects_and_empty_octomap_is_a_no_op`, `non_octree_octomap_type_is_rejected`, `truncated_octree_payload_is_rejected`. **Reformatted (§15's round)** for the same `FIRST_COL_RE` truncation as the two rows above, which had left `:1123`/`:1159` orphans. `:1106`'s only reachable emptying branch is `apply_octomap`'s `remove_all`-shaped path for this fixture. The other two were "sibling-documented and grep-confirmed unique" and are now measured: forcing the type guard `if map.octomap.id != "OcTree"` never to fire fails `non_octree_octomap_type_is_rejected` at `:1123` alone, and so does rewording that guard's own message ("type 'OcTree' is expected" -> "type OcTree is required"); dropping the `decode_result.map_err(...)` conversion fails `truncated_octree_payload_is_rejected` at its `.unwrap_err()`, and rewording "octomap payload decode failed" fails it at `:1159`. Each of the four leaves the other test green, which is what separates `apply_octomap`'s two `Error::Other` siblings |
| `scene/attached.rs:430,440,501,520,541,542,582,600,637,659` | in-family except `:532` | `:532` **not-this-family** (clause 2/3) — ADD-path `merged_touch_links` is a straight `.collect()` with the merge branch gated `if !is_add`, i.e. never entered; the real "replace not merge" claim rides on the adjacent `shapes().len()==1` assertion instead. Everything else CLEAN (8 direct + 2 hidden `assert_err_mentions` at `:513,594`), including `:553/:554` (`BTreeSet::contains` is exact-element not substring match, and `moveit-scene`'s `attach_new`/`AttachedBody` store touch_links verbatim with no auto-inclusion of `link_name`, ruling out the fixture's own "tip" link name as a spurious source) |
| `scene/collision_object.rs:638,713,774,883,896,903,951,1019,1053,1111,1146` | in-family (all 11) | CLEAN (9 direct + 2 hidden `assert_err_mentions` at `:900,948`), including the pair this round corrected: `:1108`/`:1143` (`move_object_pose_with_malformed_pose_is_rejected`/`move_shape_repose_with_malformed_pose_is_rejected`) replace the old single `:1089` citation, which this table had flagged as "latent risk, not a live collision" — `4c56148` ("test(ros): reach apply_move's object-pose parse, close the :1089 gap") already fixed exactly that gap by adding the first test, and its own doc comment (`:1064-1087` in the live source) states the bite-check: neutralizing `apply_move`'s object-pose parse (`:478`) alone fails only `move_object_pose_with_malformed_pose_is_rejected`, neutralizing the shape-repose parse (`:515`) alone fails only `move_shape_repose_with_malformed_pose_is_rejected`. §13 (this ledger's own round-12 write-up) checked `4c56148`'s ancestry and wrongly treated the pre-existing `:1089` row as proof this was already ledgered — it was ledgered as *unfixed*, and the fix had already landed; §13 read "a row exists" as "nothing changed" without checking whether the row's own verdict still held. Corrected here, not re-bitten (the source comment's own trail is the bite-check) |
| `ros/moveit-ros/src/state.rs:324,348,375,398,420,435,454,473,501,519,537` | in-family (all 11) | CLEAN (11 hidden `assert_err_mentions`) — re-derived after main's merge of ~10 branches shifted this file uniformly +39 lines (previously `:283,307,334,357,379,394,413,432,460,478,496`; `verify-orphan-enumeration.sh` flagged 7 of the 11 as unresolved after the shift — the other 4 happened to land within the gate's 5-line window of a real, unrelated `assert_err_mentions` call and passed silently, which is exactly the caution against trusting a green drift gate). Re-verified all 11 against the live file, in test-declaration order: `mismatched_position_length_is_rejected` (`:322`), `mismatched_velocity_length_is_rejected` (`:346`), `unknown_joint_name_is_rejected` (`:373`), `unknown_joint_name_in_velocity_is_rejected` (`:396`), `unknown_joint_name_in_effort_is_rejected` (`:418`), `is_diff_is_rejected_not_silently_dropped` (`:433`), `attached_collision_objects_is_rejected_not_silently_dropped` (`:452`), `multi_dof_joint_state_is_rejected_not_silently_dropped` (`:471`), `multi_dof_joint_state_transforms_is_rejected_not_silently_dropped` (`:499`), `multi_dof_joint_state_twist_is_rejected_not_silently_dropped` (`:517`), `multi_dof_joint_state_wrench_is_rejected_not_silently_dropped` (`:535`). `set_parallel_array`'s `{field}`-prefixed length/unknown-name messages keep position/velocity/effort textually distinct; the four `multi_dof_joint_state` sites intentionally share one message, discriminated by guard-clause mutation already bite-checked last round, not by text |
| `ros/moveit-ros/src/trajectory.rs:299,316,339,362,400,423` | in-family (all 6) | CLEAN (6 hidden `assert_err_mentions`) — length-mismatch messages are `{field}`-interpolated; `:297`/`:421` share one needle by design (same branch, redundant coverage, not a collision) |
| `planning.rs:788,799,832,1061` | in-family (4 sites, was 3) | CLEAN (3 hidden `assert_err_mentions` at `:782,793,826` + `:1055`) — **re-derived after p11-startstate (`10f571f`), which invalidated this row's old `:481,494,1046` citation, not just shifted it.** The old `:481` site, `nondefault_start_state_is_rejected_not_silently_dropped`, tested a blanket "any non-default `start_state` field is rejected" branch that p11-startstate **deleted outright**: `start_state` is now representable via a `StartState` sum type (`CurrentState`/`Overriding`), so position/velocity/name are accepted, not rejected. The name survives only inside a comment at `planning.rs:827` (`crates/moveit-planning::start_state`'s module doc names the same split); the function is gone. The old `:494` site survives, renumbered: its `assert_err_mentions` call is now `planning.rs:832-835` (`nonempty_reference_trajectories_is_rejected_not_silently_dropped`). The row's other claim — "`:475/:488` are `TryFrom<PlanningRequestMsg>`'s only two `Error::Other` sites" — was already wrong *before* this merge (branch-point `:475/:488` landed on `let mut msg = valid_request(&model);` and `fn nonempty_reference_trajectories...`, not on any `Error::Other` construction); re-derived directly against the current body: `TryFrom<PlanningRequestMsg>::try_from` (`planning.rs:277`) now has exactly **one** own `Error::other` site, `planning.rs:289-292` (`reference_trajectories`) — the former second site did not move with a line shift, it moved to a *different* impl, `TryFrom<StartStateMsg>` (`planning.rs:125-179`), which has its own two `Error::other` sites: `planning.rs:157-161` (`attached_collision_objects`) and `planning.rs:169-173` (`multi_dof_joint_state`), backed respectively by `a_start_state_with_attached_collision_objects_is_rejected_not_silently_dropped` (`planning.rs:799-802`) and `a_start_state_with_multi_dof_joints_is_rejected_not_silently_dropped` (`planning.rs:788-791`). Those two tests' needles ("`start_state.attached_collision_objects is not representable`" / "`start_state.multi_dof_joint_state has no core`") are textually distinct from each other and from every other needle in this file — checked by reading, not re-run through the coarse-assertions scanner. This round's `verify-orphan-enumeration.sh` sweep surfaced both as live scanner orphans (`planning.rs:788`, `:793`), exactly the future sweep this row's prose predicted; they are now folded into this row's citation and count rather than left named-for-visibility. `:1055` (`multi_dof_joint_trajectory_points_is_rejected_not_silently_dropped`) is unaffected by any of the above (unrelated impl, `TryFrom<RobotTrajectoryMsg>`) and p11-startstate did not shift it (its +9 here is D8's later file-wide move); it remains a folded-operand sibling of `:1042`'s already-covered `multi_dof_joint_trajectory_is_rejected_not_silently_dropped`: the guard is `!mdjt.joint_names.is_empty() \|\| !mdjt.points.is_empty()` (one guard, two operands), and the test's own comment (`:1045-1048` in the live source, "round 8, folded-operand audit") already states `joint_names` had a test but `points` did not before this test existed — the accepted `doc/folded-operand-guards.md` shape, matched here rather than re-derived, this is not a new finding |
| `planning.rs:760,777` | in-family (both) | CLEAN (2 hidden `assert_err_mentions`) — new code this ledger has never censused, surfaced as live scanner orphans this round. `a_start_state_naming_more_joints_than_it_gives_positions_is_rejected` (`:754`, needle `"has 2 name(s) but 1 position(s)"`) and `a_start_state_giving_positions_it_does_not_name_is_rejected` (`:771`, needle `"names no variable but carries 1 position(s)"`) both call `PlanningRequest::try_from`, which for a non-empty `start_state.joint_state` delegates unchanged (`planning.rs:125-179`'s `TryFrom<StartStateMsg>` has no guard of its own over these two fixtures — only `attached_collision_objects`/`multi_dof_joint_state`, both empty here) to `crates/moveit-planning::start_state::{StartState::new, StartStateOverride::new}`. `:754`'s fixture (2 names, 1 position) reaches `StartStateOverride::new`'s position-count guard (`crates/moveit-planning/src/start_state.rs:234-241`); `:771`'s fixture (0 names, 1 position) reaches `StartState::new`'s names-empty-with-payload guard (`crates/moveit-planning/src/start_state.rs:143-150`) one level higher, before `StartStateOverride::new` is even called. Two different guards, two structurally distinct message templates (`"has {n} name(s) but {m} position(s)"` vs `"names no variable but carries {n} position(s) and {m} velocity(ies)"`) — no swap between them is possible, and both guards' own crate (`moveit-planning`) already carries independent, distinct-message coverage for every one of their sibling branches (`crates/moveit-planning/src/start_state.rs`'s own `tests` module, read not re-bitten here since it is another crate's ledger's territory) |
| `planning.rs:615,735,736,737,738,822` | in-family (4 of 6) except `:732,816` | in-family sites CLEAN, bite-confirmed; `:732`/`:816` **not-this-family** (clause 2 — structural invariant, no decision to get wrong). New code this ledger has never censused, surfaced as live scanner orphans this round. `:609` (`over.velocities().is_empty()`, `a_named_start_state_is_carried_onto_the_core_request`) is msg->core: the fixture's `velocity` array is empty on the wire, and `StartStateOverride::new` (`crates/moveit-planning/src/start_state.rs:227-258`) stores whatever it is given verbatim once the length guards pass — bite run just now (mutate `StartStateOverride::new` to pad an empty `velocities` to one zero per name): `a_named_start_state_is_carried_onto_the_core_request` alone fails (`src/planning.rs:609:17`, `assertion failed: over.velocities().is_empty()`); full `planning::` suite run under the same mutation shows every other test, including `:729-732`'s own test, staying green — reverted, tree clean. `:729`/`:730`/`:731` (`current_state_round_trips_as_the_empty_diff_upstream_itself_writes`) are core->msg: `TryFrom<StartState> for StartStateMsgOut`'s `CurrentState => (Vec::new(), Vec::new(), Vec::new())` arm (`planning.rs:195`) is a literal, mutable decision — an engineer could have miswired it to some other value, and only `Overriding`'s data (unreachable from this arm) would make that observable. Bite run just now: replacing the tuple with `(vec!["bite".into()], vec![9.9], Vec::new())` fails only `current_state_round_trips_as_the_empty_diff_upstream_itself_writes`, at `:729` (`assertion failed: back.start_state.joint_state.name.is_empty()`); commenting out `:729`'s own assert isolates `:730` (`position`) as independently failing at the same mutation (`assertion failed: back.start_state.joint_state.position.is_empty()`) — `:731` (`velocity`) is the same tuple-literal mechanism, not independently bitten. `:732` (`attached_collision_objects`) and `:816` (`effort`, `a_start_state_effort_is_dropped_the_way_upstream_drops_it`) fail clause 2 differently from the `:729-731` trio: `StartStateOverride` (`crates/moveit-planning/src/start_state.rs:209-213`) has exactly three fields — `names`, `positions`, `velocities` — no `attached_collision_objects` or `effort` field exists anywhere between `StartState`/`StartStateOverride` and `StartStateMsgOut`'s unconditional `attached_collision_objects: Vec::new()`/`effort: Vec::new()` (both hardcoded regardless of variant, `planning.rs:211`/`:208`, and `effort` is in fact dropped one conversion earlier — `TryFrom<StartStateMsg> for StartState` at `planning.rs:125-179` never reads `msg.joint_state.effort` at all). There is no mutation expressible at either site that could make these non-empty without first adding a field to a type in another crate — a structural invariant by construction, not a runtime decision this crate's own code could get wrong, the same distinction census §9's clause 2 draws for `shortest_solution_is_none_on_empty_input` |

**Totals: 73 sites examined (67 pre-existing + `planning.rs:615,735,736,737,738,822`, new code this ledger had never censused, surfaced as orphans and closed this round — `:609,729,730,731` in-family, `:732,816` not-this-family), 69 in-family, 4 not-this-family (`constraints/set.rs:150` (`empty_constraints_is_empty_set`),
`scene/attached.rs:520` (`add_replaces_existing_attached_body_instead_of_merging`), `planning.rs:738` (`current_state_round_trips_as_the_empty_diff_upstream_itself_writes`), `planning.rs:822` (`a_start_state_effort_is_dropped_the_way_upstream_drops_it`)), 0 collisions, 0 latent risks flagged but not live
(`scene/collision_object.rs:1092` (`move_object_pose_with_malformed_pose_is_rejected`)'s risk was already closed by `4c56148`
...
| `tests/collision_env_distance_field_parity.rs:417,570,772` | `is_empty` | **not-this-family** (clause 3), all 3 | `"fixture must carry at least one case"` — fixture-loading precondition. Reformatted this round for the same `{...}`-brace parser reason as the row above |
| `collision_env_distance_field_parity.rs:499,657` | `is_some` | in-family, CLEAN, both | Both sites open with `assert_eq!(`; the full call is `assert_eq!(entry.distance_field.is_some(), expected.has_field, "has_field")` — per-request oracle-driven comparison, same single-construction-site mechanism as `:3775` |
...
| `moveit-state/tests/jacobian.rs:140,159` (both open `assert!(`, one line above their own `err.to_string().contains("not a chain")`) | `"not a chain"` | CLEAN | `jacobian()`'s only guard is `crates/moveit-state/src/state.rs:1281` (reads `"the group '{group}' is not a chain; cannot compute Jacobian"`, inside `jacobian`); the crate's other `"not a chain"` message (`dynamics.rs:448` -- reads `group_name:?} is not a chain`, inside `new`) belongs to `DynamicsSolver::new`, unreachable from `jacobian()`. |
...
`planning.rs:1048` (`multi_dof_joint_trajectory_is_rejected_not_silently_dropped`), `collision_object.rs` (10 sites incl. `:1089`'s flagged
...
when its own operand's clause is neutralized"), `ros/moveit-ros/src/trajectory.rs:385` (`add_suffix_way_point_rejects_a_nonzero_first_dt`). Per
...
| file:line | in-family verdict | collision verdict |
|---|---|---|
| `planning.rs:921,959` | in-family (both) | CLEAN (2 sites) — same shape, disjoint subjects: `:915` (`allowed_planning_time_boundaries_are_not_observable_on_the_core_request`) tables 5 boundaries (`-1.0`, `0.0`, `f64::EPSILON`, `5.0`, `f64::NAN`), `:953` (`num_planning_attempts_boundaries_are_not_observable_on_the_core_request`) tables 4 (`-1`, `0`, `1`, `2`); each maps its own single `MotionPlanRequest` field through `TryFrom<PlanningRequestMsg>` and compares `format!("{req:?}")` against the table's first row. The needle cannot collide because the panic text interpolates the field name *and* `observable`, the list of boundary labels that differed — a failure names which of the 5-or-4 rows moved, so the two tests cannot be confused with each other or with a neighbouring row of their own table |
...
1. **Mechanism.** `observable.is_empty()` is a found-nothing tag, but it is
   ...
   (`planning.rs:864-876`) iterates `rows[1..]`, so the comparison runs 4
...
## §16. The two sites `4f2c9df`'s merge brought in — `execution.rs` and `monitored_scene.rs`

Fence: `ros/moveit-ros` only. Written at merge, not by the branch: both
files are new in p11-scenetopic and its own round closed before
`verify-orphan-enumeration.sh` was run against merged `main`, so the two
assertions arrived on `main` as orphans (`ros/moveit-ros/src/execution.rs:239`,
`ros/moveit-ros/src/monitored_scene.rs:299`) with 0 unresolved citations.
Both are message needles, and both were measured the way §15's method
section sets out: one mutation on the guard, one on the message, because
those answer different questions.

Test runs are `cargo test --lib` inside `moveit-rs/ros-dev:latest`
(236 tests; nextest is not in that image).

| file:line | in-family verdict | evidence: mutation -> failing test @ panic line |
|---|---|---|
| `ros/moveit-ros/src/execution.rs:239` | in-family, discriminating **by message** | `an_unknown_event_is_rejected_and_names_the_payload`. Guard side: making the `other` arm return `Ok(Self::Stop)` fails three tests — this one at `ros/moveit-ros/src/execution.rs:233`, its `.unwrap_err()`, plus `an_empty_payload_is_not_stop` (`ros/moveit-ros/src/execution.rs:249`) and `stop_is_matched_exactly_not_by_prefix` (`ros/moveit-ros/src/execution.rs:254`) — so the rejection itself is pinned three ways and `ros/moveit-ros/src/execution.rs:239` is never reached. Message side: interpolating the payload unquoted (`type: {other}` for `type: '{other}'`) fails `ros/moveit-ros/src/execution.rs:239` **alone**, 235 passed / 1 failed. What `ros/moveit-ros/src/execution.rs:239` pins is therefore not that an unknown event is refused but that the refusal quotes the payload in upstream's own form (`trajectory_execution_manager.cpp:345`'s `"Unknown event type: '" << event << '\''`), which is the only thing that lets an operator tell a typo from a version skew. Dropping the payload from the message entirely does not compile (`unused variable: other`), so the quoting is the only reachable mutation of this needle. Every line number in this row is the shipped tree's; the guard mutation inserts two lines above these tests, so the panic lines it prints are each 2 higher than what is cited here |
| `ros/moveit-ros/src/monitored_scene.rs:299` | in-family, discriminating **by message** | `a_scene_diff_detach_sees_an_attach_that_arrived_on_the_topic`. The subject is `set_current_state_msg`'s non-diff guard in `scene/planning_scene.rs`, upstream's `planning_scene.cpp:1238-1245`. Guard side: neutralizing its `is_diff` operand (`(!is_diff && false)`, so the variable is still read and it compiles) fails this test at `ros/moveit-ros/src/monitored_scene.rs:297`, its `.unwrap_err()`, and nothing else. Message side: rewording the guard's text (`is not marked is_diff` -> `lacks the is_diff flag`) fails `ros/moveit-ros/src/monitored_scene.rs:299` **alone**, 235 passed / 1 failed. So `ros/moveit-ros/src/monitored_scene.rs:299` is what separates "some error came back" from "the non-diff guard is the one that fired" — and it matters here because the same call has two other fallible steps before it (`CollisionObjectOperation::try_from` and `RobotState`'s own conversion), either of which a variant check would accept in its place |
