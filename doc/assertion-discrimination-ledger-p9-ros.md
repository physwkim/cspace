...
**Totals: 64 sites examined (62 pre-existing + `planning.rs:1046` (`multi_dof_joint_trajectory_points_is_rejected_not_silently_dropped`), added a prior
round, minus 1 for `planning.rs:481`'s `nondefault_start_state_is_rejected_not_silently_dropped`, deleted by p11-startstate `10f571f`, plus `planning.rs:773,784` (`a_start_state_with_multi_dof_joints_is_rejected_not_silently_dropped`/`a_start_state_with_attached_collision_objects_is_rejected_not_silently_dropped`), folded in this round per `verify-orphan-enumeration.sh` — see the `planning.rs:773,784,817,1046` row above), 62 in-family, 2 not-this-family (`constraints/set.rs:148` (`empty_constraints_is_empty_set`),
