...
| `planning.rs:782,793,826,1055` | in-family (4 sites, was 3) | CLEAN (3 hidden `assert_err_mentions` at `:773,784,817` + `:1046`) — **re-derived after p11-startstate (`10f571f`), which invalidated this row's old `:481,494,1046` citation, not just shifted it.** The old `:481` site, `nondefault_start_state_is_rejected_not_silently_dropped`, tested a blanket "any non-default `start_state` field is rejected" branch that p11-startstate **deleted outright**: `start_state` is now representable via a `StartState` sum type (`CurrentState`/`Overriding`), so position/velocity/name are accepted, not rejected. The name survives only inside a comment at `planning.rs:821` (`crates/moveit-planning::start_state`'s module doc names the same split); the function is gone. The old `:494` site survives, renumbered: its `assert_err_mentions` call is now `planning.rs:826-829` (`nonempty_reference_trajectories_is_rejected_not_silently_dropped`). The row's other claim — "`:475/:488` are `TryFrom<PlanningRequestMsg>`'s only two `Error::Other` sites" — was already wrong *before* this merge (branch-point `:475/:488` landed on `let mut msg = valid_request(&model);` and `fn nonempty_reference_trajectories...`, not on any `Error::Other` construction); re-derived directly against the current body: `TryFrom<PlanningRequestMsg>::try_from` (`planning.rs:273`) now has exactly **one** own `Error::other` site, `planning.rs:285-288` (`reference_trajectories`) — the former second site did not move with a line shift, it moved to a *different* impl, `TryFrom<StartStateMsg>` (`planning.rs:125-173`), which has its own two `Error::other` sites: `planning.rs:157-161` (`attached_collision_objects`) and `planning.rs:169-173` (`multi_dof_joint_state`), backed respectively by `a_start_state_with_attached_collision_objects_is_rejected_not_silently_dropped` (`planning.rs:793-796`) and `a_start_state_with_multi_dof_joints_is_rejected_not_silently_dropped` (`planning.rs:782-785`). Those two tests' needles ("`start_state.attached_collision_objects is not representable`" / "`start_state.multi_dof_joint_state has no core`") are textually distinct from each other and from every other needle in this file — checked by reading, not re-run through the coarse-assertions scanner. This round's `verify-orphan-enumeration.sh` sweep surfaced both as live scanner orphans (`planning.rs:782`, `:784`), exactly the future sweep this row's prose predicted; they are now folded into this row's citation and count rather than left named-for-visibility. `:1046` (`multi_dof_joint_trajectory_points_is_rejected_not_silently_dropped`) is unaffected by any of the above (unrelated impl, `TryFrom<RobotTrajectoryMsg>`) and did not shift; it remains a folded-operand sibling of `:1033`'s already-covered `multi_dof_joint_trajectory_is_rejected_not_silently_dropped`: the guard is `!mdjt.joint_names.is_empty() \|\| !mdjt.points.is_empty()` (one guard, two operands), and the test's own comment (`:1036-1039` in the live source, "round 8, folded-operand audit") already states `joint_names` had a test but `points` did not before this test existed — the accepted `doc/folded-operand-guards.md` shape, matched here rather than re-derived, this is not a new finding |
| `planning.rs:754,771` | in-family (both) | CLEAN (2 hidden `assert_err_mentions`) — new code this ledger has never censused, surfaced as live scanner orphans this round. `a_start_state_naming_more_joints_than_it_gives_positions_is_rejected` (`:745`, needle `"has 2 name(s) but 1 position(s)"`) and `a_start_state_giving_positions_it_does_not_name_is_rejected` (`:762`, needle `"names no variable but carries 1 position(s)"`) both call `PlanningRequest::try_from`, which for a non-empty `start_state.joint_state` delegates unchanged (`planning.rs:125-173`'s `TryFrom<StartStateMsg>` has no guard of its own over these two fixtures — only `attached_collision_objects`/`multi_dof_joint_state`, both empty here) to `crates/moveit-planning::start_state::{StartState::new, StartStateOverride::new}`. `:745`'s fixture (2 names, 1 position) reaches `StartStateOverride::new`'s position-count guard (`crates/moveit-planning/src/start_state.rs:231-238`); `:762`'s fixture (0 names, 1 position) reaches `StartState::new`'s names-empty-with-payload guard (`crates/moveit-planning/src/start_state.rs:140-147`) one level higher, before `StartStateOverride::new` is even called. Two different guards, two structurally distinct message templates (`"has {n} name(s) but {m} position(s)"` vs `"names no variable but carries {n} position(s) and {m} velocity(ies)"`) — no swap between them is possible, and both guards' own crate (`moveit-planning`) already carries independent, distinct-message coverage for every one of their sibling branches (`crates/moveit-planning/src/start_state.rs`'s own `tests` module, read not re-bitten here since it is another crate's ledger's territory) |
| `planning.rs:609,729,730,731,732,816` | in-family (4 of 6) except `:723,807` | in-family sites CLEAN, bite-confirmed; `:723`/`:807` **not-this-family** (clause 2 — structural invariant, no decision to get wrong). New code this ledger has never censused, surfaced as live scanner orphans this round. `:600` (`over.velocities().is_empty()`, `a_named_start_state_is_carried_onto_the_core_request`) is msg->core: the fixture's `velocity` array is empty on the wire, and `StartStateOverride::new` (`crates/moveit-planning/src/start_state.rs:224-255`) stores whatever it is given verbatim once the length guards pass — bite run just now (mutate `StartStateOverride::new` to pad an empty `velocities` to one zero per name): `a_named_start_state_is_carried_onto_the_core_request` alone fails (`src/planning.rs:600:17`, `assertion failed: over.velocities().is_empty()`); full `planning::` suite run under the same mutation shows every other test, including `:720-723`'s own test, staying green — reverted, tree clean. `:720`/`:721`/`:722` (`current_state_round_trips_as_the_empty_diff_upstream_itself_writes`) are core->msg: `TryFrom<StartState> for StartStateMsgOut`'s `CurrentState => (Vec::new(), Vec::new(), Vec::new())` arm (`planning.rs:195`) is a literal, mutable decision — an engineer could have miswired it to some other value, and only `Overriding`'s data (unreachable from this arm) would make that observable. Bite run just now: replacing the tuple with `(vec!["bite".into()], vec![9.9], Vec::new())` fails only `current_state_round_trips_as_the_empty_diff_upstream_itself_writes`, at `:720` (`assertion failed: back.start_state.joint_state.name.is_empty()`); commenting out `:720`'s own assert isolates `:721` (`position`) as independently failing at the same mutation (`assertion failed: back.start_state.joint_state.position.is_empty()`) — `:722` (`velocity`) is the same tuple-literal mechanism, not independently bitten. `:723` (`attached_collision_objects`) and `:807` (`effort`, `a_start_state_effort_is_dropped_the_way_upstream_drops_it`) fail clause 2 differently from the `:720-722` trio: `StartStateOverride` (`crates/moveit-planning/src/start_state.rs:206-210`) has exactly three fields — `names`, `positions`, `velocities` — no `attached_collision_objects` or `effort` field exists anywhere between `StartState`/`StartStateOverride` and `StartStateMsgOut`'s unconditional `attached_collision_objects: Vec::new()`/`effort: Vec::new()` (both hardcoded regardless of variant, `planning.rs:203`/`:199`, and `effort` is in fact dropped one conversion earlier — `TryFrom<StartStateMsg> for StartState` at `planning.rs:125-160` never reads `msg.joint_state.effort` at all). There is no mutation expressible at either site that could make these non-empty without first adding a field to a type in another crate — a structural invariant by construction, not a runtime decision this crate's own code could get wrong, the same distinction census §9's clause 2 draws for `shortest_solution_is_none_on_empty_input` |

**Totals: 73 sites examined (67 pre-existing + `planning.rs:609,729,730,731,732,816`, new code this ledger had never censused, surfaced as orphans and closed this round — `:600,720,721,722` in-family, `:723,807` not-this-family), 69 in-family, 4 not-this-family (`constraints/set.rs:148` (`empty_constraints_is_empty_set`),
