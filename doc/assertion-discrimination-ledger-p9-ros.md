...
| `moveit-trajectory/tests/ruckig_smoothing.rs:208` | `"did not set the group"` | CLEAN | `apply_smoothing` -> `validate_group` is the only reachable producer; TOTG's identically-worded message (`time_optimal_trajectory_generation.rs:697`) is a different module's function, unreachable from `apply_smoothing`. Cited `:204` before drift, now the preceding comment. |
