...
`not-this-family` means the assertion checks a value that the success path
*always* produces, regardless of which internal branch ran — a variant tag
on a type that cannot be constructed any other way (`matches!(shape,
Shape::Mesh(_))` after a build with one producing arm), or a check of the
test's own fixture setup rather than the function under test. It does
**not** mean "the assertion isn't about a `Result`/`Option` returned by a
fallible call." A post-build business-state check (`.mimic().is_none()`,
an emptied cache, a cleared flag) is in-family by default and must be
bite-checked like any other site: it is only `not-this-family` if the
isolating mutation cannot make it fail — i.e. the checked state is
provably identical on every code path, not merely unexamined.
...
| constraints/orientation.rs:176 | matches! | converts_with_xyz_euler_parameterization | not-this-family | bite run just now — success-path tolerance-variant check, not error discrimination |
...
| constraints/set.rs:163 | matches! | aggregates_joint_constraints | not-this-family | bite run just now — success-path `Constraint::Joint` variant check |
| constraints/set.rs:183 | matches! | one_bad_element_fails_the_whole_conversion | discriminating | `a7d8682` |
| constraints/set.rs:421 | matches! | (visibility round-trip) | not-this-family | bite run just now — success-path `Constraint::Visibility` variant check |
...
| scene/collision_object.rs:983 | bare | append_without_subframe_data_clears_existing_subframes | discriminating | bite run just now (docker) — wrapped `apply_add`'s unconditional `set_world_object_subframes` call in `if !subframes.is_empty()`, reverting to the historical only-call-when-there-is-data behavior the file's own doc comment names; assertion fails |
...
| joint/model.rs:1040 | bare | mimic_set_get_clear_round_trip (pre-set) | discriminating | `01KZ7P9B03SWRKQWQV1DXB4EEA-7.md` ("discriminating, non-vacuous, standing") + re-bitten just now — mutated `new_single_variable` to default-construct with a bogus `mimic: Some(...)`; assertion fails |
| joint/model.rs:1047 | bare | mimic_set_get_clear_round_trip (post-clear) | discriminating | `01KZ7P9B03SWRKQWQV1DXB4EEA-7.md` (same line) + re-bitten just now — mutated `clear_mimic` to a no-op (`self.mimic = None;` deleted); assertion fails |
| joint/urdf.rs:366 | matches! | (Fixed joint kind) | not-this-family | bite run just now — success-path `JointKind::Fixed` variant check |
| joint/urdf.rs:372 | matches! | (Floating joint kind) | not-this-family | bite run just now — success-path `JointKind::Floating` variant check |
| joint/urdf.rs:378 | matches! | (Planar joint kind) | not-this-family | bite run just now — success-path `JointKind::Planar` variant check |
...
| robot_model.rs:2025 | bare | mimic_mutual_cycle_clears_every_mimic_in_the_model (j1) | fixture-collapse-fixed | `7676185` — fixture gave j1 a mimic outside the cycle; two isolating mutations (narrow-to-cycle-members, skip-clear-entirely) both now fail here |
| robot_model.rs:2026 | bare | (j2) | discriminating | `01KZ7P9B03SWRKQWQV1DXB4EEA-7.md` ("discriminating, non-vacuous") + re-bitten just now — skip-clear-entirely fails here (proves cycle members get cleared at all); narrow-to-cycle-members does NOT fail here (j2 is itself a cycle member, cleared under any correct scoping) — non-vacuous for "clearing happens", not independent evidence for the whole-model-vs-cycle-only scope claim, which only j1/j4 carry |
| robot_model.rs:2027 | bare | (j3) | discriminating | same as :2026 — `01KZ7P9B03SWRKQWQV1DXB4EEA-7.md` + re-bitten just now, same caveat |
| robot_model.rs:2028 | bare | (j4, new in `7676185`) | fixture-collapse-fixed | `9661d4c` (this round) — was blind under every mutation tested (no `<mimic>` tag of its own, same defect as pre-fix j1); fixed by giving j4 a real out-of-cycle mimic on new leaf joint j5, re-verified failing under the narrow-scope mutation post-fix. Never bitten before this round. |
| robot_model.rs:2046 | bare | mimic_of_unknown_joint_is_dropped_with_a_diagnostic | discriminating | `01KZ7P9B03SWRKQWQV1DXB4EEA-7.md` (as old line 2010, "discriminating, non-vacuous, standing") + re-bitten just now — mutated the single-joint clear to a no-op; assertion fails |
| robot_model.rs:2078 | bare | mimic_with_mismatched_dof_is_dropped_with_a_diagnostic | discriminating | `01KZ7P9B03SWRKQWQV1DXB4EEA-7.md` (as old line 2042, "discriminating, non-vacuous, standing") + re-bitten just now — same mutation, same result |
| robot_model.rs:2413 | bare | mesh_collision_resolving_to_an_unreadable_file_is_skipped_with_a_diagnostic | not-this-family | bite run just now — assertion message says `"precondition: ..."` explicitly; verifies test fixture setup, not the function under test |
| robot_model.rs:2485 | matches! | mesh_collision_resolving_to_a_valid_stl_file_builds_a_mesh_shape | not-this-family | bite run just now — success-path `Shape::Mesh` variant check |
...
Corrected this round (see verdict-taxonomy note above): 9 sites were
originally misclassified `not-this-family` on the theory that a post-build
state check is categorically outside the family. Re-bitten individually
rather than reasoned from shape: `robot_model.rs:2026/2027` (j2/j3) and
`joint/model.rs:1040/1047` were already reported `discriminating,
non-vacuous` in `01KZ7P9B03SWRKQWQV1DXB4EEA-7.md` (round 6) and re-bite
this round agrees; `robot_model.rs:2046/2078` likewise. `robot_model.rs:2025`
(j1) was fixture-vacuous and is fixed by `7676185` (prior round).
`robot_model.rs:2028` (j4) was blind under every mutation tried and had
never been bitten before — fixed this round by `9661d4c`.

Final breakdown: 7 discriminating, 8 single-branch, 2 fixture-collapse-fixed
(`7676185` for j1, `9661d4c` for j4, this round), 5 genuinely
`not-this-family` (`joint/urdf.rs:366,372,378` success-path `JointKind`
variants, `robot_model.rs:2413` fixture precondition, `robot_model.rs:2485`
success-path `Shape::Mesh` variant). 7+8+2+5 = 22. No site here reads as a
...
**Revised after user correction:** the first pass conflated `not-this-family`
(success-path variant checks) with "not about `Result`/`Option`", and
...
verdict-taxonomy note above and the two crate tables for per-site evidence),
not by re-reasoning from shape. One was already fixed by a prior round
(`7676185`, j1); one was blind and had never been bitten by anyone before
this round, fixed now (`9661d4c`, j4); the remaining seven were confirmed
genuinely discriminating.

Final verdicts: 18 discriminating (4 ros + 7 distance-field + 7 model), 38
single-branch (19 ros + 11 distance-field + 8 model), 3 fixture-collapse-fixed
(1 ros pre-existing + 2 model: `7676185`, `9661d4c`), 8 not-this-family
(3 ros success-path/precondition checks + 5 model kind-variant checks,
one precondition, one shape-variant check). 18+38+3+8 = 67. One commit this
round: `9661d4c`.
