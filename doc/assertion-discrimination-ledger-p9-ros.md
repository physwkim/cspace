...
| `scene/planning_scene.rs:1106` (`empty_collision_objects_and_empty_octomap_is_a_no_op`), `scene/planning_scene.rs:1123-1126` (`non_octree_octomap_type_is_rejected`), `scene/planning_scene.rs:1159-1162` (`truncated_octree_payload_is_rejected`) | in-family | CLEAN (3 sites) — `:1106`'s only reachable emptying branch is `apply_octomap`'s `remove_all`-shaped path for this fixture; `:1123/:1159` already sibling-documented and grep-confirmed unique |
