...
1. **Mechanism.** `observable.is_empty()` is a found-nothing tag, but it is
   ...
   (`planning.rs:858-870`) iterates `rows[1..]`, so the comparison runs 4
...
assertions arrived on `main` as orphans (`ros/moveit-ros/src/execution.rs:234`,
...
| file:line | in-family verdict | evidence: mutation -> failing test @ panic line |
|---|---|---|
| `ros/moveit-ros/src/execution.rs:234` | in-family, discriminating **by message** | `an_unknown_event_is_rejected_and_names_the_payload`. Guard side: making the `other` arm return `Ok(Self::Stop)` fails three tests — this one at `ros/moveit-ros/src/execution.rs:228`, its `.unwrap_err()`, plus `an_empty_payload_is_not_stop` (`ros/moveit-ros/src/execution.rs:244`) and `stop_is_matched_exactly_not_by_prefix` (`ros/moveit-ros/src/execution.rs:249`) — so the rejection itself is pinned three ways and `ros/moveit-ros/src/execution.rs:234` is never reached. Message side: interpolating the payload unquoted (`type: {other}` for `type: '{other}'`) fails `ros/moveit-ros/src/execution.rs:234` **alone**, 235 passed / 1 failed. What `ros/moveit-ros/src/execution.rs:234` pins is therefore not that an unknown event is refused but that the refusal quotes the payload in upstream's own form (`trajectory_execution_manager.cpp:345`'s `"Unknown event type: '" << event << '\''`), which is the only thing that lets an operator tell a typo from a version skew. Dropping the payload from the message entirely does not compile (`unused variable: other`), so the quoting is the only reachable mutation of this needle. Every line number in this row is the shipped tree's; the guard mutation inserts two lines above these tests, so the panic lines it prints are each 2 higher than what is cited here |
