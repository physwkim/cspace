...
| `crates/moveit-planning/src/pipeline.rs` (Semantic 1: planner-chain feedforward) | `planning_pipeline.cpp:295-302` (section), `planning_pipeline.cpp:299` (`if (res.trajectory)` gate), `planning_pipeline.cpp:301` (the overwrite statement), `getTrajectoryConstraints` cited `planning_pipeline.cpp:57-73` | EXPIRED (wrong-anchor-right-claim), fixed this round | `planning_pipeline.cpp:295-302`/`planning_pipeline.cpp:299`/`planning_pipeline.cpp:301` all pinpoint-exact against the real function body. `getTrajectoryConstraints`'s true span is `planning_pipeline.cpp:57-79`, not `planning_pipeline.cpp:57-73` — the citation stopped at the per-joint field assignments, excluding the outer waypoint loop's close, the `push_back` into `trajectory_constraints`, and the final `return` statement. Recurs at 3 sites (module doc, `Feedforward` error variant doc, the private helper's own doc). | `f2f6301` |
| `crates/moveit-planning/src/pipeline.rs` (Semantic 2: response-adapter chain gate) | `planning_pipeline.cpp:332-351` (section), `planning_pipeline.cpp:333` (`if (res.error_code)`) | CONFIRMED | both pinpoint-exact — `planning_pipeline.cpp:332` is the section's leading comment, `planning_pipeline.cpp:351` its closing brace; `planning_pipeline.cpp:333` is literally `if (res.error_code)`. | |
| `crates/moveit-planning/src/pipeline.rs` (Semantic 3: `active_` flag, six manual resets) | `planning_pipeline.hpp:217-220` (`isActive`), `cpp:259` (entry set), `planning_pipeline.cpp:289`/`planning_pipeline.cpp:313`/`planning_pipeline.cpp:327`/`planning_pipeline.cpp:347`/`planning_pipeline.cpp:357`/`planning_pipeline.cpp:372` (the six resets) | CONFIRMED | opened all eight — every one pinpoint-exact (`planning_pipeline.cpp:259` is `active_ = true;`; each of the six reset lines is exactly `active_ = false;` on its named exit path: request-adapter failure, context-creation failure, planner failure, response-adapter failure, the `catch` block, and the final line). | |
...
| `crates/moveit-planning/src/pipeline.rs` (`trajectory_constraints_for`'s weight substitution) | `kinematic_constraint.cpp:263-270` | CONFIRMED | exact — `if (jc.weight <= std::numeric_limits<double>::epsilon()) { ...; constraint_weight_ = 1.0; } else constraint_weight_ = jc.weight;` spans exactly `kinematic_constraint.cpp:263-270`. | |
| `crates/moveit-planning/src/pipeline.rs` (`generate_plan` replaces `PlanningPipeline::generatePlan`, module-doc + `generate_plan`'s own doc) | `planning_pipeline.cpp:251-374` (cited twice) | CONFIRMED | exact — `generatePlan`'s definition spans precisely `planning_pipeline.cpp:251-374`, both occurrences. | |
| `crates/moveit-planning/src/response.rs` (D8 delta audit: `MotionPlanResponse`'s 6 members) | `planning_response.hpp:48-70` (struct), `ompl_interface/.../model_based_planning_context.cpp:799`, `chomp/.../chomp_planning_context.cpp:62`, `stomp/.../stomp_moveit_planning_context.cpp:277`, `pilz_industrial_motion_planner/.../trajectory_generator.cpp:267,277`, `moveit_py/.../planning_response.cpp:52,92` | CONFIRMED | opened all six — every `.planning_time =`/`.start_state` fill site is pinpoint-exact (including both pilz sites, `setSuccessResponse`/`setFailureResponse`); `moveit_py:92`'s `def_property(..., nullptr, ...)` confirms the "read-only" claim exactly. `hpp:48-70`'s true struct close is `planning_response.hpp:73`, 3 lines past the citation, but every one of the 5 data members plus the start of `operator bool` (the claim's "6th member") is inside `planning_response.hpp:48-70`. | |
| `crates/moveit-planning/src/response.rs` (table-row correction, not a code defect) | this file's prior aggregate row (line 27, now removed) listed `kinematic_constraints/src/utils.cpp:623-676` and `moveit/robot_state/robot_state.hpp:1417` as citations belonging to `response.rs` | table error, no `response.rs` fix needed | `rg -n '623\|675\|1417\|utils' crates/moveit-planning/src/response.rs` — zero hits. Neither citation exists in this file. `kinematic_constraints/utils.cpp:623-676` is actually `resolve_constraint_frames.rs`'s own citation (see its row below); `robot_state.hpp:1417` is `check_start_state_bounds.rs`'s pre-fix citation from row 1 above (already corrected to `:1419` there, commit `5347903`). The prior aggregate row conflated citations from three different files under one `response.rs` label. | |
| `crates/moveit-planning/src/request_adapters/resolve_constraint_frames.rs` | `kinematic_constraints/src/utils.cpp:623-676` (`resolveConstraintFrames`), adapter's own `cpp:41-83` (symbol audit), `:65` (`RCLCPP_DEBUG`), `:73` (`SUCCESS` return), `:66-73` (return value discarded) | CONFIRMED | opened all five — `cpp:41-83` and `:65`/`:73`/`:66-73` are pinpoint-exact against the adapter file. `kinematic_constraints/utils.cpp:623-676` is the function's whole span; the citation read `moveit_core/kinematic_constraints/src/utils.cpp:623-675`, one line short of the closing brace at `moveit_core/kinematic_constraints/src/utils.cpp:676`, and is corrected here. Both constraint loops (position, orientation) and every described rewrite (`link_name`, `target_point_offset`/`orientation` folding) are inside it. | |
