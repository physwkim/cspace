...
| `crates/moveit-scene/src/scene.rs:288-289`, `:1337` (`knowsFrameTransform`) | cited `planning_scene.cpp:2056`/`:2061` | CONFIRMED | opened both — `:2056` is the id-only overload, `:2061` is the explicit-state overload; both exact; `crates/moveit-scene/src/scene.rs:288-289`'s own text reads `PlanningScene::knows_frame_transform`. | |
