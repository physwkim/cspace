...
| `crates/moveit-scene/src/scene.rs:2585` (`decouple_parent_then_mutating_the_former_parent_is_not_observed`) | same claim, second occurrence, same citation `planning_scene.cpp:344` | EXPIRED | same as above — `planning_scene.cpp:344` is inside `pushDiffs()`, not `decoupleParent`. | `679a8f6` |
