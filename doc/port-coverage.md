...
`decided-non-port` 67 / `gap` 5 / `ported-elsewhere` 15.
...
| `moveit_core/planning_interface/src/planning_interface.cpp` | gap | `crates/moveit-planners-sbp/src/lib.rs:300-391` (`# Round 6 symbol audit`) -- 8 of the 9 definitions | "no citation and no exclusion" was wrong. The file defines **9** members of the two classes `planning_interface.hpp` declares (`PlanningContext`'s ctor/dtor/`setPlanningScene`/`setMotionPlanRequest`; `PlannerManager`'s `initialize`/`getDescription`/`getPlanningAlgorithms`/`getPlanningContext(scene, req)`/`terminate`), and the sbp audit names and decides every one of the 9 as a declaration. **8** of the 9 bodies add nothing to that decision -- an assignment, `return true`, `return ""`, `algs.clear()`, a forward to the 3-arg overload -- or the file-local `ActiveContexts` mutex+set (declared in no header) that the ctor/dtor maintain and only the two excluded `terminate()`s ever read. The **9th** is why this stays `gap`: `setMotionPlanRequest` (`:92-103`) normalizes every request before any `solve` -- `allowed_planning_time <= 0.0` becomes `1.0`, `num_planning_attempts` becomes `max(1, n)` -- and no sentence anywhere decides that rule. The audit excludes the *setter* (contexts here are single-use), which is not a decision about the rule; `moveit-planning/src/request.rs` and `moveit-planners-stomp/src/planner.rs` place these two fields' *consumption* at the `PlanningContext` layer and neither mentions the clamping. Measured: `rg -n 'allowed_planning_time\|num_planning_attempts' crates/ ros/ --glob '*.rs'` is 15 hits, every one a doc comment, and no site applies either normalization |
| `moveit_core/planning_interface/src/planning_response.cpp` | ported-elsewhere | `ros/moveit-ros/src/planning.rs:326` (`TryFrom<PlanningResponse<'m>> for PlanningResponseMsgOut`) -- `MotionPlanResponse::getMessage` (`:40-50`); `PORTING-PLAN.md` §NEW + `ros/moveit-ros/src/planning.rs:44-66` (`# Not ported here: MotionPlanDetailedResponse::getMessage`) -- the other function | the previous citation (`response.rs:100`) is to `moveit_py`'s same-named file, so this row carried no evidence about this file at all. The real counterpart is moveit-ros's core->msg conversion, which reproduces `getMessage`'s five writes as far as this side has sources: `trajectory` and `trajectory_start` (upstream's `getFirstWayPoint()`) directly; `group_name` from `RobotTrajectory::group_name()`, inside upstream's own `if (trajectory && !trajectory->empty())` guard -- added this round, the field was silently empty on the wire before; `error_code` structurally replaced by `Result` (the impl writes `SUCCESS`); `planning_time` absent because `PlanningResponse` has no such field (`response.rs`'s D8 audit calls it "unported, in scope", expiring when any crate implements `pipeline::Planner`). The file's other function, `MotionPlanDetailedResponse::getMessage` (`:52-79` -- 28 of the 39 lines the two functions occupy, signatures included, in a 79-line file), still has no counterpart anywhere and is not going to get one: §NEW **decides** it non-port, which is what moves this row off `gap`, since §3 makes a row `gap` only when the residue is undecided **and** most of the file is absent. The sentence also sits in the crate D6 would put the conversion in (`ros/moveit-ros/src/planning.rs:44-66`). Three measurements at `e017c91ee` behind it: D6 needs a core-side source type and the workspace's only counterpart to `planning_interface::MotionPlanDetailedResponse` is `ChompSolution` (`crates/moveit-planners-chomp/src/planner.rs:205`), whose `trajectory` and `description` are scalars rather than the parallel vectors upstream iterates, so all four boundaries of the rule (empty vector; all-empty; first-non-empty at `i > 0` supplying `trajectory_start`/`group_name`; source-indexed `description`/`processing_time` guards) are unrepresentable there; upstream calls this function from **nowhere** -- `rg -n 'getMessage'` over moveit2 less `AllowedCollisionMatrix` is 13 hits (2 declarations, 2 definitions, 9 calls) and each of the 9 passes a `moveit_msgs::msg::MotionPlanResponse&`, which does not compile against the detailed signature; and no `.srv`/`.action`/`.msg` embeds the wire type -- `rg -n 'MotionPlanDetailedResponse' third_party/moveit_msgs/` is one hit (`CMakeLists.txt:53`, the build list) against three embedders for `MotionPlanResponse`. Residual: nothing undecided; §NEW.4 states the three conditions that re-open it |
...
| `moveit_core/robot_state/src/attached_body.cpp` | gap | `crates/moveit-scene/src/attached_body.rs:25-33`, `crates/moveit-scene/src/scene.rs:1322-1325` | the four residuals now split two ways, so the file stays `gap` on the second pair. **`computeTransform` (cpp:105) and `getGlobalSubframeTransform` (cpp:157): decided-non-port.** Both exist only to maintain and read upstream's cached globals — `global_pose_`, `global_collision_body_transforms_`, `global_subframe_poses_` — and this module has none of the three. `attached_body.rs:25-33` commits to one-level `shape_poses` held in the attach link's own frame ("composing it away up front means `PlanningScene::detach` only ever needs one transform"), and `PlanningScene::frame_transform` recomputes `link_transform * local_pose` on demand there. There is no cache for `computeTransform` to refresh and none for `getGlobalSubframeTransform` to read; porting them would mean first adding the cache the design deliberately removed. **`setScale` (cpp:86) and `setPadding` (cpp:120): still `gap`** — unwritten, not blocked. The primitive they need does exist, contrary to an earlier note: `moveit_geometry::Shape::scale`/`::padd` are `crates/moveit-geometry/src/shapes.rs:1492` and `:1497`, and upstream's `shape.use_count() == 1 ? const_cast : clone` policy is exactly `Arc::make_mut` on this module's `Vec<Arc<Shape>>` (`Shape` derives `Clone`, `shapes.rs:1384`). Not added here: out of the round's scope, not out of reach |
