...
`decided-non-port` 68 / `gap` 5 / `ported-elsewhere` 14.
...
| `moveit_core/planning_interface/src/planning_interface.cpp` | decided-non-port | `crates/moveit-planners-sbp/src/lib.rs:300-391` (`# Round 6 symbol audit`) -- 8 of the 9 definitions; `PORTING-PLAN.md` §236 + `crates/moveit-planning/src/request.rs:89-105` -- the 9th | "no citation and no exclusion" was wrong. The file defines **9** members of the two classes `planning_interface.hpp` declares (`PlanningContext`'s ctor/dtor/`setPlanningScene`/`setMotionPlanRequest`; `PlannerManager`'s `initialize`/`getDescription`/`getPlanningAlgorithms`/`getPlanningContext(scene, req)`/`terminate`), and the sbp audit names and decides every one of the 9 as a declaration. **8** of the 9 bodies add nothing to that decision -- an assignment, `return true`, `return ""`, `algs.clear()`, a forward to the 3-arg overload -- or the file-local `ActiveContexts` mutex+set (declared in no header) that the ctor/dtor maintain and only the two excluded `terminate()`s ever read. The **9th** was the `gap`: `setMotionPlanRequest` (`:92-103`) normalizes every request before any `solve` -- `allowed_planning_time <= 0.0` becomes `1.0`, `num_planning_attempts` becomes `max(1, n)`. §236 decides that rule, and `request.rs`'s crate doc carries the same decision in this workspace's own words. Three grounds, each measured there: the two fields exist on no request type here (`rg -n 'allowed_planning_time\|num_planning_attempts' crates ros` is 16 hits, 15 doc comments in `.rs` and one row of `ros/moveit-ros/doc/message-mapping.md`, no code), so there is no operand to repair; the receiving site exists and takes the request by value with no setter to hang a rule on (`crates/moveit-planners-sbp/src/registry.rs:584`/`:621`); and this port's budget type `Termination` (`rrt_connect.rs:40-58`, no `Default`, no unset variant, `usize`/`Duration` unsigned) cannot represent negative, unset or NaN, so all three states the clamp repairs are unconstructible. The rule is also incomplete upstream -- `<= 0.0` is false for NaN -- recorded as `doc/upstream-bugs.md`'s `set-motion-plan-request-time-guard-polarity`. Expiry is a test, not a sentence: `allowed_planning_time_boundaries_are_not_observable_on_the_core_request`/`num_planning_attempts_boundaries_are_not_observable_on_the_core_request` (`ros/moveit-ros/src/planning.rs`) fail the moment either field becomes observable on the core request |
| `moveit_core/planning_interface/src/planning_response.cpp` | gap | `ros/moveit-ros/src/planning.rs:302` (`TryFrom<PlanningResponse<'m>> for PlanningResponseMsgOut`) -- `MotionPlanResponse::getMessage` (`:40-50`) only | the previous citation (`response.rs:100`) is to `moveit_py`'s same-named file, so this row carried no evidence about this file at all. The real counterpart is moveit-ros's core->msg conversion, which reproduces `getMessage`'s five writes as far as this side has sources: `trajectory` and `trajectory_start` (upstream's `getFirstWayPoint()`) directly; `group_name` from `RobotTrajectory::group_name()`, inside upstream's own `if (trajectory && !trajectory->empty())` guard -- added this round, the field was silently empty on the wire before; `error_code` structurally replaced by `Result` (the impl writes `SUCCESS`); `planning_time` absent because `PlanningResponse` has no such field (`response.rs`'s D8 audit calls it "unported, in scope", expiring when any crate implements `pipeline::Planner`). Still `gap`, because the file's other function is both undecided and the larger half: `MotionPlanDetailedResponse::getMessage` (`:52-79` -- 28 of the file's 39 function-body lines, with its own first-non-empty-trajectory rule for `trajectory_start`/`group_name` and per-index `description`/`processing_time` push-backs) has no counterpart anywhere. `rg -n 'MotionPlanDetailedResponse' crates/ ros/ tools/ doc/ PORTING-PLAN.md --glob '!port-coverage.md'` (excluding this file, which now names it too) is 7 hits, every one about the *struct* or `solve`'s overload, none about this function |
