...
`decided-non-port` 59 / `gap` 15 / `ported-elsewhere` 13.
...
| `moveit_planners/pilz_industrial_motion_planner/include/pilz_industrial_motion_planner/planning_context_base.hpp` | ported-elsewhere | `PORTING-PLAN.md` §226.1-§226.3; `crates/moveit-planners-pilz/src/trajectory_generator.rs:606-636`, `:352` | §226.1: `solve(MotionPlanResponse&)`의 계산은 `generator_.generate(...)` 한 줄이고 그것이 `PilzGenerator::generate`(`:606-636`, 반환 타입 `MotionPlanResponse` `:494-530`)다. 생성자의 `generator_(model, limits_, group)`는 `TrajectoryGenerator::new`(`:352`) + 명령별 `new(base, group_name)`. 잔여분 셋(`terminated_`/`terminate()`, `MotionPlanDetailedResponse` 3중 push, 빈 `clear()`)은 §226.3에서 각각 D1/D4·D6·없음으로 처분
| `moveit_planners/pilz_industrial_motion_planner/include/pilz_industrial_motion_planner/planning_context_circ.hpp` | decided-non-port | `PORTING-PLAN.md` §226.2; `crates/moveit-planners-pilz/src/lib.rs:130-139` | "zero statements, zero members, zero overrides ... all four do is bind one command type to one generator type, and here that binding is `impl PilzGenerator for TrajectoryGeneratorCIRC` itself" -- 파일 전체가 `MOVEIT_CLASS_FORWARD` 한 줄과 전달 생성자 하나. 비-ROS 잔여분 0
...
| `moveit_planners/pilz_industrial_motion_planner/include/pilz_industrial_motion_planner/trajectory_generation_exceptions.hpp` | ported-elsewhere | `PORTING-PLAN.md` §226.5-§226.6; `crates/moveit-error/src/lib.rs:100-102`, `crates/moveit-planners-pilz/src/trajectory_generator.rs:511-517` | `MoveItErrorCodeException`(코드를 실은 예외) -> `Error::Code(MoveItErrorCode)`; 네 `catch` 블록 -> `MotionPlanResponse::failure` + `PilzGenerator::generate`(`:606-636`). 여덟 catch 사이트가 예외에서 꺼내는 것은 `getErrorCode()`(-> `error_code.val`)와 `what()`(-> `RCLCPP_ERROR_STREAM` 전용) 둘뿐이라 비-로깅 잔여분 0. §226.5가 매크로 생성 클래스 46개(호출 49회, 중복 3은 `polyline-header-redeclares-lin-exceptions`) 전부를 포트 사이트에 대응시킨다. 값으로 대응하는 31개, 죽은 선언 3개, 타입 모양으로 표현 불가 10개, 그리고 실제 누락 2개 -- `MoreThanOneTipFrameException`(`tip_frame_getter.hpp` 행이 이미 기록한 잔여분)과 `JointNumberMismatch`(LIN의 관절 수 검사, 이 라운드 밖) -- 는 §226.6. `lib.rs:159-169`가 같은 결론을 문장으로 적는다
