...
3개 테마(`collision-distance-accuracy` 44, `pilz-pipeline` 19, `planner-benchmark-parity` 36)는 세션 하나로 하기엔 크다. 아래는 그 절들을 군집으로 다시 쪼갠 라운드 제안이다 — 이 하위 분할은 위 테마 표(기계적)와 달리 손으로 짠 것이고, 위 표의 절별 개수를 근거로 삼는다. 코퍼스가 바뀌면 절별 개수부터 다시 뽑아 이 제안을 다시 짜야 한다.
