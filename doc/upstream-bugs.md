...
`moveit-kinematics`, `moveit-smoothing`) are already `not-reproduced` — the
port structurally avoids each one rather than transcribing it, and each
already carries a doc comment describing the deviation, but none cites a
`PORTING-PLAN.md` D-number. Assigning one (or confirming none applies) is
unassigned work, not a decision blocked on measurement the way 1/3/5/10 are.
