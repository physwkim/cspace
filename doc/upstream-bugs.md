...
648 has no `else`.
**Port:** `crates/moveit-collision/src/parry.rs:2589`
...
`parry.rs:2589` takes `contact.dist` from `parry3d_f64::query::contact` and
...
running threshold (`parry.rs:2560`) and the discard (`:2473`), but the
...
**Port:** `crates/moveit-collision/src/parry.rs:2370`, `accumulate_collision`'s
...
**Port:** `crates/moveit-collision/src/parry.rs:2577`,
