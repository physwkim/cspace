...
648 has no `else`.
**Port:** `crates/moveit-collision/src/parry.rs:2551`
...
`parry.rs:2551` takes `contact.dist` from `parry3d_f64::query::contact` and
...
The `27,384x` figure §218.4 (`PORTING-PLAN.md:17249`) and §229.3
...
running threshold (`parry.rs:2522`) and the discard (`:2473`), but the
...
**Port:** `crates/moveit-collision/src/parry.rs:2332`, `accumulate_collision`'s
...
**Port:** `crates/moveit-collision/src/parry.rs:2539`,
