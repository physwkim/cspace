...
| `planning_time: float64` | **no field** | dropped — re-checked this round (§157 audit), still absent from `moveit-planning`; expiry condition already tracked in `src/planning.rs`'s `TryFrom<PlanningResponseMsg>` doc comment (p1-fixtures grounding the field), not restated here per round 5 brief |
