...
//! `shape_msgs/{Mesh,Plane}` <-> [`moveit_geometry::Shape`]'s `Mesh`/`Plane`
...
use moveit_geometry::{Mesh, Plane, Shape, Vector3 as CoreVector3};
...
impl TryFrom<MeshMsg> for Shape {
    ...
    /// `vertices` (moveit-geometry's own D6-shaped construction check), so
