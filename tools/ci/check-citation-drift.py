...
def class_header(head_sha, total, live, n):
    """The `#` header `doc/citation-classes.txt` carries for this tree.

    A function rather than a literal at the write site because the read path
    calls it too: the counts here are the numbers a reader quotes, and until
    they were re-derived on the read path nothing checked them -- see
    tools/ci/baseline_header.py for the two commits that shipped a header
    contradicting its own rows.
    """
    header = [
...
def in_repo_header(head_sha, rows):
    """The `#` header `doc/citation-classes-in-repo.txt` carries for this tree.
    Shared with the read path for the same reason as `class_header`."""
    by_verdict = {}
    for r in rows:
...
def report_in_repo(tracked):
    ...
    header_failed = baseline_header.report(
        IN_REPO_BASELINE, text, in_repo_header("-", rows),
        "tools/ci/check-citation-drift.py --write-classes", sys.stderr)
...
def main(write_classes=False):
    ...
    if write_classes:
        ...
        header = class_header(head_sha, total, live, n)
        ...
        rows = scan_in_repo(tracked)
        head = in_repo_header(head_sha, rows)
    ...
    # The rows below are checked one by one against a fresh derivation. The
    # header describing them was checked by nothing until this line, and a
    # merge resolves it from one side while auto-merging the rows from both.
    header_failed = baseline_header.report(
        CLASS_BASELINE, baseline_text, class_header("-", total, live, n),
        "tools/ci/check-citation-drift.py --write-classes", sys.stderr)
