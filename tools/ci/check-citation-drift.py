...
def find_anchors(
    ...
    2. Table row, first column, with no tight pairing found, IN A TABLE
       ...
       LEDGER_FIRST_COLUMN_HEADERS): every valid identifier anywhere else
       in the row THAT IS `#[test]`-attested. Production-function names
       loosely mentioned in the same row are excluded here precisely
       because they are not reliably containment claims (`do_smoothing`
       above), and every column-one citation is excluded in a table that
       heads the column something else, because only `file:line` says the
       cited line is a location inside what the row discusses.
