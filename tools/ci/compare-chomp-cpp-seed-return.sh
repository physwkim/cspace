...
# (`crates/cspace-planners/src/chomp/optimizer.rs:1873`) and never replaced,
# and that snapshot is what `optimize` copies back before returning
# (`crates/cspace-planners/src/chomp/optimizer.rs:1979-1983`). So the returned
...
# `crates/cspace-planners/examples/chomp_benchmark_port.rs:627-630`,
# whose subspace weights are built by the same rule at
# `tools/moveit-oracle/src/oracle.cpp:695` and
# `crates/cspace-planners/src/sbp/joint_model_group_space.rs:153`), so
