...
def render(entries, doc_label, closure_re):
    ...
    lines.append(
        ...
        "같은 방식으로 '부정 어휘 없음'을 규칙화해 자동으로 걸러 보면 187건 중 39건이 "
        "걸리는데, 그중 38건은 '거부한다'/'그대로다'/'미결'처럼 이 정규식이 놓친 다른 "
        "부정 표현으로 여전히 열린 진짜 잔여 claim이었다 — 부정-어휘-부재는 이 코퍼스에서 "
        "신뢰할 수 없는 신호다(38/39가 오탐). 그래서 이 도구는 918을 `measurement`에 "
    ...
    lines.append(
        ...
        "항목처럼 스레드 이름을 반복하지 않는 항목이 새어 나간다(모듈 docstring "
        "참고: 불릿 단위 72/187 대 절 단위 174/187). 매치되는 첫 테마(우선순위 순)가 "
