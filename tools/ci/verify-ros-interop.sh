...
# glob reaches it.
#
# ros/cspace-ros lives outside the root workspace (D5, PORTING-PLAN.md
...
# already bit this repo once, when ros/cspace-ros broke on `main` while
...
# ros/cspace-ros happens to use the changed symbol), no moveit_msgs
# schema-drift check, no oracle/fixture comparison. Root-workspace CI
# (ci.yml's check-*.sh glob) still never builds or tests ros/cspace-ros --
