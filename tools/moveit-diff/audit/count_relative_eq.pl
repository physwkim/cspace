...
# Same classification as crates/moveit-geometry/audit/count_relative_eq.pl
# (p3-acm's round-16 tolerance-site count), applied to p1-joints' owned
# tolerance-macro call sites (crates/moveit-kinematics, tools/moveit-diff,
# crates/moveit-state/tests/invariants.rs). Strips `//` line-comment tails
# (NOT string/char literal aware, but this repo's assert_relative_eq! calls
# contain no `//` inside string args, verified by inspection), then finds
# every `assert_relative_eq!(` / `relative_eq!(` call by bracket-matching
...
# argument text (not the surrounding doc comment, since doc comments were
# already stripped as `//` tails or never contain a real macro call to begin
# with).
use strict;
use warnings;
