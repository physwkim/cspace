// Vendored from moveit2 @ e017c91ee12984393a28ba246075c65f69cde3bf:
//   moveit_kinematics/kdl_kinematics_plugin/include/moveit/kdl_kinematics_plugin/chainiksolver_vel_mimic_svd.hpp
//
// This oracle links only moveit_core (see oracle.cpp's own top-of-file
// comment): the rest of moveit_kinematics -- kdl_kinematics_plugin.cpp,
// specifically -- pulls in rclcpp, pluginlib and moveit_ros_planning, none of
// which this binary may depend on (it starts no ROS node). KDL::
// ChainIkSolverVelMimicSVD itself has no such dependency (its own .cpp has
// exactly one #include, its own header), so it is vendored here verbatim
// instead. The ONLY textual change from upstream is the #include path below,
// updated to this vendored joint_mimic.hpp's own location; every other line
// is untouched, including the upstream license header.
//
// tools/moveit-oracle/src/oracle.cpp hand-transcribes the outer
// searchPositionIK/CartToJnt/clipToJointLimits loop that calls this solver
// (KDLKinematicsPlugin itself is not vendored, for the same ROS-dependency
// reason) -- see that transcription's own doc comment for what it
// deliberately does not replicate (the wall-clock timeout).
